...
void RainRenderer::prepareFrame(const glm::vec3& cameraPos, const glm::mat4& view, const float rainStrength,
                                const float snowStrength, float dt) {
    ...
    if (rainStrength > 0.01f) {
        ...
        buildVertices(m_rainDrops, rainStrength, RAIN_DROP_LENGTH, 0.006f, true, view, m_rainVertices);
    } else {
    ...
    if (snowStrength > 0.01f) {
        ...
        buildVertices(m_snowDrops, snowStrength, SNOW_DROP_LENGTH, 0.025f, false, view, m_snowVertices);
    } else {
...
void RainRenderer::uploadFrame(RhiCommandList& commandList) {
    ...
    commandList.bufferBarrier({m_vertexBuffer, RhiResourceState::VertexBuffer, RhiResourceState::TransferDst});
    if (!m_rainVertices.empty()) {
        commandList.updateBuffer(m_vertexBuffer, RAIN_VERTEX_OFFSET, m_rainVertices.data(),
                                 m_rainVertices.size() * sizeof(float));
    }
    if (!m_snowVertices.empty()) {
        commandList.updateBuffer(m_vertexBuffer, SNOW_VERTEX_OFFSET, m_snowVertices.data(),
                                 m_snowVertices.size() * sizeof(float));
    }
...
void RainRenderer::updateDrops(std::vector<PrecipDrop>& drops, float dt, float baseSpeed, const glm::vec3& cameraPos) {
    m_time += dt;
    if (m_time > 628.0f)
        m_time -= 628.0f; // wrap at ~100*PI to prevent float precision loss
    for (auto& d : drops) {
...
void RainRenderer::buildVertices(const std::vector<PrecipDrop>& drops, const float strength, const float dropLength,
                                 const float streakWidth, const bool proceduralLines, const glm::mat4& view,
    ...
    const glm::vec3 fallDir =
        glm::normalize(glm::vec3(-wind * std::cos(kWindAngle), -1.0f, -wind * std::sin(kWindAngle) * 0.3f));
    const int visibleCount = std::min(static_cast<int>(drops.size()), static_cast<int>(drops.size() * strength));
    ...
    for (int i = 0; i < visibleCount; ++i) {
        ...
        const glm::vec3 corners[4] = {top - rightOffset, top + rightOffset, bottom + rightOffset, bottom - rightOffset};
        ...
        for (const int cornerIndex : cornerIndices) {
            ...
            vertices.insert(vertices.end(), {position.x, position.y, position.z, u[cornerIndex], v[cornerIndex]});
        }
    }
}

void RainRenderer::renderPrecipitation(RhiCommandList& commandList, const glm::mat4& projection, const glm::mat4& view,
                                       RhiTextureHandle texture, const std::vector<float>& vertices, float strength,
                                       float skyLightAtCamera, float alphaScale, const glm::vec3& color,
                                       bool proceduralLines, const RhiTextureHandle sceneDepthTexture,
                                       const glm::vec2& screenSize, bool hardwareDepthTest) {
    ...
    const RhiBindGroupHandle bindGroup =
        depthFade ? (rain ? m_rainDepthBindGroup : m_snowDepthBindGroup) : (rain ? m_rainBindGroup : m_snowBindGroup);
    const uint64_t vertexOffset = rain ? RAIN_VERTEX_OFFSET : SNOW_VERTEX_OFFSET;
    if (!pipeline.isValid() || !bindGroup.isValid()) {
        return;
    }

    const RainPushConstants pushConstants{projection * view, glm::vec4(color, strength * skyLightAtCamera),
...
void RainRenderer::render(RhiCommandList& commandList, const glm::mat4& projection, const glm::mat4& view,
                          float rainStrength, float skyLightAtCamera, float alphaScale,
                          const RhiTextureHandle sceneDepthTexture, const glm::vec2& screenSize,
                          bool hardwareDepthTest) {
    renderPrecipitation(commandList, projection, view, m_rainTex, m_rainVertices, rainStrength, skyLightAtCamera,
                        alphaScale, glm::vec3(0.72f, 0.78f, 0.85f), // rain blue-gray
                        true, sceneDepthTexture, screenSize, hardwareDepthTest);
}

void RainRenderer::renderSnow(RhiCommandList& commandList, const glm::mat4& projection, const glm::mat4& view,
                              float snowStrength, float skyLightAtCamera, float alphaScale,
                              const RhiTextureHandle sceneDepthTexture, const glm::vec2& screenSize,
                              bool hardwareDepthTest) {
    renderPrecipitation(commandList, projection, view, m_snowTex, m_snowVertices, snowStrength, skyLightAtCamera,
                        alphaScale, glm::vec3(0.92f, 0.95f, 1.0f), // snow white
                        false, sceneDepthTexture, screenSize, hardwareDepthTest);
}

bool RainRenderer::createRhiResources() {
    const std::optional<std::string> vertexSource = renderer::rhi::loadShaderSource("assets/shaders/rain_rhi.vert");
    const std::optional<std::string> fragmentSource = renderer::rhi::loadShaderSource("assets/shaders/rain_rhi.frag");
    ...
    if (!vertexSource.has_value() || !fragmentSource.has_value() || !depthFragmentSource.has_value()) {
        destroyRhiResources();
        return false;
    }
    ...
    if (!m_vertexBuffer.isValid() || !m_rainTextureView.isValid() || !m_snowTextureView.isValid() ||
        !m_precipitationSampler.isValid() || !m_depthSampler.isValid() || !m_vertexShader.isValid() ||
        !m_fragmentShader.isValid() || !m_depthFragmentShader.isValid() || !m_bindGroupLayout.isValid() ||
        !m_depthBindGroupLayout.isValid() || !m_pipelineLayout.isValid() || !m_depthPipelineLayout.isValid() ||
        !m_depthTestPipeline.isValid() || !m_depthSamplePipeline.isValid() || !createBindGroups({})) {
        destroyRhiResources();
        return false;
    }
    return true;
...
bool RainRenderer::createBindGroups(const RhiTextureHandle sceneDepthTexture) {
    if (m_rainBindGroup.isValid() && m_snowBindGroup.isValid() &&
        ...
        return !sceneDepthTexture.isValid() || (m_rainDepthBindGroup.isValid() && m_snowDepthBindGroup.isValid());
    }
    destroyBindGroups();
...
void RainRenderer::destroyBindGroups() {
    if (m_rhiDevice != nullptr) {
        if (m_rainBindGroup.isValid())
            m_rhiDevice->destroyBindGroup(m_rainBindGroup);
        if (m_snowBindGroup.isValid())
            m_rhiDevice->destroyBindGroup(m_snowBindGroup);
        if (m_rainDepthBindGroup.isValid())
            m_rhiDevice->destroyBindGroup(m_rainDepthBindGroup);
        if (m_snowDepthBindGroup.isValid())
            m_rhiDevice->destroyBindGroup(m_snowDepthBindGroup);
        if (m_sceneDepthTextureView.isValid())
            m_rhiDevice->destroyTextureView(m_sceneDepthTextureView);
    }
    m_rainBindGroup = {};
    m_snowBindGroup = {};
...
void RainRenderer::destroyRhiResources() {
    destroyBindGroups();
    if (m_rhiDevice != nullptr) {
        if (m_depthSamplePipeline.isValid())
            m_rhiDevice->destroyPipeline(m_depthSamplePipeline);
        if (m_depthTestPipeline.isValid())
            m_rhiDevice->destroyPipeline(m_depthTestPipeline);
        if (m_depthPipelineLayout.isValid())
            m_rhiDevice->destroyPipelineLayout(m_depthPipelineLayout);
        if (m_pipelineLayout.isValid())
            m_rhiDevice->destroyPipelineLayout(m_pipelineLayout);
        if (m_depthBindGroupLayout.isValid())
            m_rhiDevice->destroyBindGroupLayout(m_depthBindGroupLayout);
        if (m_bindGroupLayout.isValid())
            m_rhiDevice->destroyBindGroupLayout(m_bindGroupLayout);
        if (m_depthFragmentShader.isValid())
            m_rhiDevice->destroyShader(m_depthFragmentShader);
        if (m_fragmentShader.isValid())
            m_rhiDevice->destroyShader(m_fragmentShader);
        if (m_vertexShader.isValid())
            m_rhiDevice->destroyShader(m_vertexShader);
        if (m_depthSampler.isValid())
            m_rhiDevice->destroySampler(m_depthSampler);
        if (m_precipitationSampler.isValid())
            m_rhiDevice->destroySampler(m_precipitationSampler);
        if (m_snowTextureView.isValid())
            m_rhiDevice->destroyTextureView(m_snowTextureView);
        if (m_rainTextureView.isValid())
            m_rhiDevice->destroyTextureView(m_rainTextureView);
        if (m_vertexBuffer.isValid())
