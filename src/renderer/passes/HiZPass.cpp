...
RgPassHandle HiZPass::addGraphPasses(RenderGraph& graph, const FrameContext& ctx, DeferredRenderTargets& targets,
    ...
    for (uint32_t mip = 0u; mip < mipCount; ++mip) {
        ...
        pass.dependsOn(previous).writeTexture(resources.hiZ, RhiResourceState::ShaderWrite, destRange);
        if (mip == 0u) {
...
bool HiZPass::recordMip(RhiCommandList& commandList, const FrameContext& ctx, DeferredRenderTargets& targets,
    ...
    const uint32_t destWidth = std::max(1u, static_cast<uint32_t>(std::max(1, targets.width())) >> mip);
...
bool HiZPass::ensurePipeline(RhiDevice& rhiDevice) {
    ...
    const std::optional<std::string> source = renderer::rhi::loadShaderSource("assets/shaders/hiz_build.comp");
    if (!source.has_value()) {
        return false;
    }
    ...
    layoutDesc.entries.push_back({1u, RhiBindingType::StorageTexture, rhiFlag(RhiShaderStage::Compute), 1u});
    ...
    pipelineLayoutDesc.pushConstantBytes = static_cast<uint32_t>(sizeof(glm::ivec4));
...
bool HiZPass::ensureMipBindGroup(RhiDevice& rhiDevice, const uint32_t mip, const RhiTextureViewHandle sourceView,
    ...
    if (binding.bindGroup.isValid() && sameTextureView(binding.boundSource, sourceView) &&
...
bool HiZPass::recordCull(RhiCommandList& commandList, const FrameContext& ctx, const RenderSettings& settings,
    ...
    RhiDevice& rhiDevice = *ctx.shared->rhiDevice;
    if (!targets.ensureHiZTextureViews(rhiDevice) || !ensurePipeline(rhiDevice) || !ensureCullPipeline(rhiDevice)) {
        return false;
    }

    const RhiBufferHandle commandBuffers[2] = {worldBuffer.opaqueIndirectBufferHandle(),
                                               worldBuffer.cutoutIndirectBufferHandle()};
    const uint64_t commandCapacities[2] = {worldBuffer.opaqueIndirectBufferCapacity(),
    ...
    commandList.updateBuffer(m_cullCounterBuffer, 0u, zeroCounts, sizeof(zeroCounts));
    ...
    for (int slot = 0; slot < 2; ++slot) {
        ...
        if (!ensureCullBindGroup(rhiDevice, slot, commandBuffers[slot], commandCapacities[slot],
                                 worldBuffer.metadataBufferHandle(), worldBuffer.metadataBufferCapacity(),
    ...
    commandList.bufferBarrier({m_cullCounterBuffer, RhiResourceState::TransferSrc, RhiResourceState::StorageBuffer});
...
void HiZPass::finishGraphExecution(const bool succeeded) {
    ...
    if (succeeded) {
        ...
        m_cullRingWriteIndex = (m_pendingCullRingIndex + 1u) % kCullStatsRingSize;
    }
...
bool HiZPass::ensureCullPipeline(RhiDevice& rhiDevice) {
    ...
    const std::optional<std::string> source = renderer::rhi::loadShaderSource("assets/shaders/hiz_cull.comp");
    if (!source.has_value()) {
        return false;
    }
    RhiShaderDesc shaderDesc;
    ...
    layoutDesc.entries.push_back({2u, RhiBindingType::CombinedTextureSampler, rhiFlag(RhiShaderStage::Compute), 1u});
    layoutDesc.entries.push_back({3u, RhiBindingType::StorageBuffer, rhiFlag(RhiShaderStage::Compute), 1u});
    ...
    pipelineLayoutDesc.pushConstantBytes = static_cast<uint32_t>(sizeof(HiZCullPushConstants));
    pipelineLayoutDesc.pushConstantStages = rhiFlag(RhiShaderStage::Compute);
...
bool HiZPass::ensureCullBindGroup(RhiDevice& rhiDevice, const int slot, const RhiBufferHandle commandBuffer,
                                  const uint64_t commandCapacity, const RhiBufferHandle metadataBuffer,
                                  const uint64_t metadataCapacity, const RhiTextureViewHandle hiZView) {
    if (!commandBuffer.isValid() || !metadataBuffer.isValid() || !hiZView.isValid() || slot < 0 || slot >= 2) {
        return false;
    }
    ...
    if (binding.bindGroup.isValid() && binding.boundCommands.index == commandBuffer.index &&
        ...
        binding.boundMetadata.generation == metadataBuffer.generation && sameTextureView(binding.boundHiZ, hiZView)) {
        return true;
    }
    if (binding.bindGroup.isValid()) {
        rhiDevice.destroyBindGroup(binding.bindGroup);
