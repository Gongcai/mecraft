...
bool WaterCompositePass::execute(const FrameContext& ctx, const RenderSettings& settings,
    ...
    if (!useMultiDrawIndirect && m_waterCompositeShader == nullptr) {
        return false;
    }
    ...
    if (useMultiDrawIndirect) {
        ...
        // water pass runs before temporal resolve. The post-TAA fallback stays
        // raw because there is no later TAA pass to accumulate it.
        const bool useJitteredWater = preTemporalResolve && settings.taa.enabled;
        ...
        // DerivativeMain shaders.properties: uniform.vec3.waterAbsorption =
        // vec3(0.4, 0.14, 0.08)
        ...
        if (m_resourceMgr) {
            ...
            m_waterCompositeShader->setFloat("uWaterStillLayerCount",
        ...
        glBindTexture(GL_TEXTURE_2D, renderer::rhi::gl::textureId(targets.sceneResolvedTextureHandle()));
        glActiveTexture(GL_TEXTURE8);
        glBindTexture(GL_TEXTURE_2D, renderer::rhi::gl::textureId(m_resourceMgr->getTexture2DHandle("shader_noise2d")));
        ...
        for (const auto& entry : transparentEntries) {
            ...
            const glm::vec3 center(static_cast<float>(offset.x) + Chunk::SIZE_X * 0.5f,
        ...
        std::sort(waterItems.begin(), waterItems.end(),
                  [](const WaterItem& a, const WaterItem& b) { return a.distanceSq > b.distanceSq; });
        for (const auto& item : waterItems) {
            ...
            glDrawArrays(GL_TRIANGLES, static_cast<GLint>(firstWaterVertex),
