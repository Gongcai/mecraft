...
public:
    ...
    void setTerrainRenderer(TerrainRenderer* renderer) { m_terrainRenderer = renderer; }
    void setWorldRenderBuffer(WorldRenderBuffer* buffer) { m_worldRenderBuffer = buffer; }
    void setBlockEntityRenderer(BlockEntityRenderer* renderer) { m_blockEntityRenderer = renderer; }
    void setStaticMeshRenderer(StaticMeshRenderer* renderer) { m_staticMeshRenderer = renderer; }
    ...
    void setSceneLights(std::vector<renderer::contracts::SceneLight> lights);
    ...
    [[nodiscard]] const std::vector<renderer::contracts::GpuLight>& resolvedLights() const { return m_resolvedLights; }

    [[nodiscard]] ConsumerResources consumerResources() const;
    [[nodiscard]] const LocalShadowFrameStats& frameStats() const { return m_frameStats; }
...
private:
    ...
    [[nodiscard]] bool ensureResources(RhiDevice& rhiDevice, uint32_t spotSlotCount, uint32_t pointSlotCount);
    [[nodiscard]] bool ensureMetadataBuffer(RhiDevice& rhiDevice);
    [[nodiscard]] bool ensureSampler(RhiDevice& rhiDevice);
    [[nodiscard]] bool ensureSpotAtlas(RhiDevice& rhiDevice, uint32_t requiredGridSize);
    [[nodiscard]] bool ensurePointCubeArray(RhiDevice& rhiDevice, uint32_t requiredCapacity);
    [[nodiscard]] bool buildPreparedShadows(const FrameContext& ctx, const IWorldView* worldView);
    [[nodiscard]] bool prepareWorldActors();
    [[nodiscard]] bool prepareWorldGeometry(RhiCommandList& commandList, const PreparedShadow& shadow,
                                            uint32_t faceIndex);
    void drawWorldGeometry(RhiCommandList& commandList, const PreparedShadow& shadow, uint32_t faceIndex) const;
    [[nodiscard]] bool recordMetadataUpload(RhiCommandList& commandList) const;
    [[nodiscard]] bool recordSpotAtlas(RhiCommandList& commandList);
    [[nodiscard]] bool recordPointCubeArray(RhiCommandList& commandList);
    [[nodiscard]] bool clearPointFace(RhiCommandList& commandList, uint32_t faceLayer) const;
    [[nodiscard]] bool renderShadowPage(RhiCommandList& commandList, const PreparedShadow& shadow, uint32_t faceIndex,
                                        RhiTextureViewHandle attachment, const RhiRect2D& renderArea,
                                        const char* debugName);
    [[nodiscard]] bool importTexture(RenderGraph& graph, RhiTextureHandle texture, RhiTextureViewHandle view,
                                     bool initialized, RgTextureHandle& graphTexture) const;
    ...
    std::array<renderer::contracts::LocalShadowMetadata, renderer::contracts::kLocalShadowMetadataCount> m_metadata{};
