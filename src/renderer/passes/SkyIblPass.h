...
public:
    ...
    struct GraphResources {
        std::array<GraphGeneration, renderer::contracts::kSkyIblGenerationCount> generations;
    ...
    [[nodiscard]] bool prepareFrame(RhiDevice& rhiDevice, RhiTextureViewHandle skyCaptureView, uint64_t frameIndex);
    ...
    /// contract.
    [[nodiscard]] RgPassHandle addGraphPasses(RenderGraph& graph, RgTextureHandle skyCapture,
    ...
    [[nodiscard]] uint64_t committedRevision() const { return m_committedRevision; }
    [[nodiscard]] uint64_t buildingRevision() const { return m_buildRevision; }
    [[nodiscard]] uint32_t completedBuildWorkItems() const { return m_nextPrefilterWorkItem; }
...
private:
    struct GenerationResources {
        ...
        std::array<RhiTextureViewHandle, renderer::contracts::kSkyIblCubeFaceCount> radianceFaceViews{};
        std::array<std::array<RhiTextureViewHandle, renderer::contracts::kSkyIblCubeFaceCount>,
    ...
    [[nodiscard]] bool createBindGroups(RhiDevice& rhiDevice, RhiTextureViewHandle skyCaptureView);
    [[nodiscard]] bool recordSkyRadiance(RhiCommandList& commandList, uint32_t generation) const;
    [[nodiscard]] bool recordSpecularPrefilter(RhiCommandList& commandList, uint32_t generation, uint32_t firstWorkItem,
    ...
    std::array<GenerationResources, renderer::contracts::kSkyIblGenerationCount> m_generations;
