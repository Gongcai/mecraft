...
bool createTextureAndView(RhiDevice& rhiDevice, const char* debugName, const RhiTextureFormat format,
                          const uint32_t width, const uint32_t height, const RhiTextureUsageFlags usage,
                          RhiTextureHandle& texture, RhiTextureViewHandle& view) {
...
void beginPostProcessColorOutput(RhiCommandList& commandList, const char* debugName, const RhiTextureViewHandle view,
                                 const int width, const int height, const bool clearColor) {
...
RhiCommandList& PostProcessPass::beginCommandList(const char* const debugName) const {
    ...
    RhiCommandList* const commandList = m_commandListPool->acquire(RhiCommandListType::Graphics);
    if (commandList == nullptr || !commandList->begin({debugName, RhiCommandListType::Graphics})) {
        std::abort();
    }
    return *commandList;
}

void PostProcessPass::submitCommandList(RhiDevice& rhiDevice, RhiCommandList& commandList,
...
bool PostProcessPass::beginSceneCapture(RhiDevice& rhiDevice, const int requestedWidth, const int requestedHeight) {
    m_sceneCaptured = false;
    if (requestedWidth <= 0 || requestedHeight <= 0 ||
        !ensureSceneCaptureTargets(rhiDevice, requestedWidth, requestedHeight)) {
        return false;
    }
    if (!setHdrInput(m_sceneColorHandle, m_sceneColorView, requestedWidth, requestedHeight)) {
        return false;
    }
...
bool PostProcessPass::setHdrInput(const RhiTextureHandle texture, const RhiTextureViewHandle view, const int width,
                                  const int height) {
    if (m_rhiDevice == nullptr || !texture.isValid() || !view.isValid() || width <= 0 || height <= 0) {
        return false;
    }
    // Exposure history is independent of the HDR extent. Temporal upscaling can
    // change that extent within a frame, so sized targets are created only when
    // the final composition extent is known.
    if (!ensureExposureState(*m_rhiDevice)) {
        return false;
    }
    if (m_hdrInputHandle.index != texture.index || m_hdrInputHandle.generation != texture.generation ||
        m_hdrInputView.index != view.index || m_hdrInputView.generation != view.generation) {
...
bool PostProcessPass::prepareTextureOutput(RhiDevice& rhiDevice, const int width, const int height) {
    return width > 0 && height > 0 && ensureProcessingTargets(rhiDevice, width, height) &&
           ensureCompositeTarget(rhiDevice, width, height);
}

bool PostProcessPass::compositeToBackbuffer(RhiDevice& rhiDevice, const RhiTextureViewHandle swapchainColorView,
...
bool PostProcessPass::executeCompositeGraph(RhiDevice& rhiDevice, const CompositeDestination destination,
    ...
    const auto importBuffer = [&](const RhiBufferHandle buffer, const RhiResourceState stableState,
    ...
    if (!importTexture(m_hdrInputHandle, m_hdrInputView, RhiResourceState::ShaderRead, resources.hdrInput) ||
        !importTexture(m_sceneDepthHandle, m_sceneDepthView, RhiResourceState::DepthRead, resources.sceneDepth) ||
        !importTexture(m_noiseTexture, m_noiseTextureView, RhiResourceState::ShaderRead, resources.noise) ||
    ...
    const RhiTextureHandle outputTexture =
        destination == CompositeDestination::Texture ? m_compositeHandle : rhiDevice.currentSwapchainColorTexture();
    ...
    const bool initializeExposure = autoExposureEnabled && !m_autoExposureInitialized;
    const bool shouldSampleExposure =
        autoExposureEnabled && (initializeExposure || m_autoExposureSampleAccumulator + elapsedFrameTime >=
                                                          kAutoExposureSampleIntervalSeconds);
    const int previousExposureReadIndex = initializeExposure ? 0 : m_exposureStateReadIndex;
    const int finalExposureReadIndex = autoExposureEnabled ? 1 - previousExposureReadIndex : m_exposureStateReadIndex;
    const double nextExposureAccumulator =
        shouldSampleExposure ? 0.0 : m_autoExposureSampleAccumulator + elapsedFrameTime;
    ...
    if (initializeExposure) {
        RenderGraphPassBuilder initialize = m_renderGraph.addPass(
            {"PostProcess.ExposureStateInitialization", RgPassType::Graphics, RhiQueueType::Graphics});
        initialize.writeTexture(resources.exposureState[0], RhiResourceState::RenderTarget)
            .writeTexture(resources.exposureState[1], RhiResourceState::RenderTarget)
            .setExecute([this, &debugService, &exposureTimer, manualExposure](RgPassContext& pass) {
                exposureTimer = debugService.beginGpuTimer(pass.commandList(), GpuTimerPass::Post);
                recordExposureStateInitialization(pass.commandList(), manualExposure);
                return true;
            });
    ...
    if (shouldSampleExposure) {
        const int exposureLod =
            std::min(kAutoExposureLod, std::max(0, static_cast<int>(std::floor(std::log2(static_cast<float>(
        ...
        for (int mip = 0; mip < m_exposureMipCount; ++mip) {
            ...
            const std::string passName = "PostProcess.ExposureDownsample." + std::to_string(mip);
            RenderGraphPassBuilder downsample =
                m_renderGraph.addPass({passName.c_str(), RgPassType::Graphics, RhiQueueType::Graphics});
            if (graphTail.isValid()) {
                downsample.dependsOn(graphTail);
            }
            if (mipSourceIsScene) {
                downsample.readTexture(resources.hdrInput, RhiResourceState::ShaderRead);
            } else {
                downsample.readTexture(resources.exposure[mip - 1], RhiResourceState::ShaderRead);
            }
            downsample.writeTexture(resources.exposure[mip], RhiResourceState::RenderTarget)
                .setExecute([this, &debugService, &exposureTimer, mip, mipSourceSize, mipSourceIsScene,
                             exposureLod](RgPassContext& pass) {
                    if (mip == 0 && !exposureTimer.valid) {
                        exposureTimer = debugService.beginGpuTimer(pass.commandList(), GpuTimerPass::Post);
                    }
                    recordExposureDownsample(pass.commandList(), mip, mipSourceSize, mipSourceIsScene, exposureLod);
                    return true;
                });
    ...
    if (autoExposureEnabled) {
        RenderGraphPassBuilder resolve =
            m_renderGraph.addPass({"PostProcess.ExposureResolve", RgPassType::Graphics, RhiQueueType::Graphics});
        if (graphTail.isValid()) {
            resolve.dependsOn(graphTail);
        }
        resolve.readTexture(resources.exposure[m_exposureMipCount - 1], RhiResourceState::ShaderRead)
            .readTexture(resources.exposureState[previousExposureReadIndex], RhiResourceState::ShaderRead)
            .writeTexture(resources.exposureState[finalExposureReadIndex], RhiResourceState::RenderTarget)
            .setExecute([this, &debugService, &exposureTimer, previousExposureReadIndex, finalExposureReadIndex,
                         elapsedFrameTime, manualExposure, historyAvailable = !initializeExposure,
                         reuseExposure = !shouldSampleExposure](RgPassContext& pass) {
                if (!exposureTimer.valid) {
                    exposureTimer = debugService.beginGpuTimer(pass.commandList(), GpuTimerPass::Post);
                }
                recordExposureResolve(pass.commandList(), previousExposureReadIndex, finalExposureReadIndex,
                                      elapsedFrameTime, manualExposure, historyAvailable, reuseExposure);
    ...
    const int bloomMipCount = std::clamp(m_effects.bloomMipCount, 1, kBloomMipCount);
    if (bloomReady) {
        for (int mip = 0; mip < bloomMipCount; ++mip) {
            const std::string extractName = "PostProcess.Bloom.Extract." + std::to_string(mip);
            RenderGraphPassBuilder extract =
                m_renderGraph.addPass({extractName.c_str(), RgPassType::Graphics, RhiQueueType::Graphics});
            if (graphTail.isValid()) {
                extract.dependsOn(graphTail);
            }
            extract.readTexture(resources.hdrInput, RhiResourceState::ShaderRead)
                .writeTexture(resources.bloom[mip][0], RhiResourceState::RenderTarget)
                .setExecute([this, &debugService, &bloomTimer, mip](RgPassContext& pass) {
                    if (mip == 0) {
                        bloomTimer = debugService.beginGpuTimer(pass.commandList(), GpuTimerPass::Post);
                    }
            ...
            const std::string horizontalName = "PostProcess.Bloom.BlurHorizontal." + std::to_string(mip);
            RenderGraphPassBuilder horizontal =
                m_renderGraph.addPass({horizontalName.c_str(), RgPassType::Graphics, RhiQueueType::Graphics});
            horizontal.dependsOn(extractHandle)
                .readTexture(resources.bloom[mip][0], RhiResourceState::ShaderRead)
                .writeTexture(resources.bloom[mip][1], RhiResourceState::RenderTarget)
            ...
            const std::string verticalName = "PostProcess.Bloom.BlurVertical." + std::to_string(mip);
            RenderGraphPassBuilder vertical =
                m_renderGraph.addPass({verticalName.c_str(), RgPassType::Graphics, RhiQueueType::Graphics});
            vertical.dependsOn(horizontalHandle)
                .readTexture(resources.bloom[mip][1], RhiResourceState::ShaderRead)
                .writeTexture(resources.bloom[mip][0], RhiResourceState::RenderTarget)
                .setExecute(
                    [this, &debugService, &bloomTimer, mip, lastMip = mip == bloomMipCount - 1](RgPassContext& pass) {
    ...
    const PostProcessCompositeParams compositeParams = buildCompositeParams(useAutoExposureTexture, bloomReady);
    RenderGraphPassBuilder params =
        m_renderGraph.addPass({"PostProcess.CompositeParameters", RgPassType::Copy, RhiQueueType::Graphics});
    if (graphTail.isValid()) {
        params.dependsOn(graphTail);
    }
    params.writeBuffer(resources.compositeParams, RhiResourceState::TransferDst)
        .setExecute([this, &debugService, &compositeTimer, compositeParams](RgPassContext& pass) {
            compositeTimer = debugService.beginGpuTimer(pass.commandList(), GpuTimerPass::Post);
            uploadCompositeParams(pass.commandList(), compositeParams);
            return true;
        });
    const RgPassHandle paramsHandle = params.handle();

    RenderGraphPassBuilder composite =
        m_renderGraph.addPass({destination == CompositeDestination::Texture ? "PostProcess.CompositeTexture"
    ...
    composite.dependsOn(paramsHandle)
        ...
        .readTexture(resources.exposureState[finalExposureReadIndex], RhiResourceState::ShaderRead)
    ...
    composite.setExecute([this, &debugService, &compositeTimer, destination, outputView, outputWidth, outputHeight,
        ...
        if (destination == CompositeDestination::Texture) {
            ...
            bindBackbufferOutput(pass.commandList(), outputView, std::max(1, outputWidth), std::max(1, outputHeight),
                                 false);
            renderComposite(pass.commandList(), m_compositeSwapchainPipeline, finalExposureReadIndex);
        }
...
bool PostProcessPass::executeBlitGraph(RhiDevice& rhiDevice, const RhiTextureHandle sourceTexture,
    ...
    if (!importTexture(sourceTexture, {}, RhiResourceState::ShaderRead, source) ||
        !importTexture(swapchainTexture, swapchainColorView, RhiResourceState::Present, swapchain)) {
        return false;
    }
...
void PostProcessPass::recordExposureStateInitialization(RhiCommandList& commandList, const float manualExposure) {
    const float initialState[4] = {std::max(manualExposure, 0.001f), m_lastAverageLum, m_lastTargetExposure, 1.0f};
    for (int index = 0; index < 2; ++index) {
        RhiColorAttachment colorAttachment;
...
void PostProcessPass::recordExposureDownsample(RhiCommandList& commandList, const int mip, const glm::ivec2& sourceSize,
                                               const bool sourceIsScene, const int sourceLod) {
    beginPostProcessColorOutput(commandList, "ExposureDownsample", m_exposureView[mip], m_exposureMipSize[mip].x,
                                m_exposureMipSize[mip].y, true);
...
void PostProcessPass::recordExposureResolve(RhiCommandList& commandList, const int readIndex, const int writeIndex,
                                            const float elapsedFrameTime, const float manualExposure,
                                            const bool historyAvailable, const bool reuseExposure) {
...
void PostProcessPass::recordBloomExtract(RhiCommandList& commandList, const int mip) {
    beginPostProcessColorOutput(commandList, "BloomExtract", m_bloomView[mip][0], m_bloomMipSize[mip].x,
                                m_bloomMipSize[mip].y, true);
...
void PostProcessPass::recordBloomBlur(RhiCommandList& commandList, const int mip, const bool horizontal) {
    beginPostProcessColorOutput(commandList, horizontal ? "BloomBlurHorizontal" : "BloomBlurVertical",
                                m_bloomView[mip][horizontal ? 1 : 0], m_bloomMipSize[mip].x, m_bloomMipSize[mip].y,
                                true);
    commandList.setGraphicsPipeline(m_bloomBlurPipeline);
    commandList.setBindGroup(0u, m_bloomBlurBindGroup[mip][horizontal ? 0 : 1]);
    const glm::vec4 pushConstants = horizontal ? glm::vec4(1.0f, 0.0f, 1.0f, 0.0f) : glm::vec4(0.0f, 1.0f, 1.0f, 0.0f);
    commandList.pushConstants(&pushConstants, sizeof(pushConstants), rhiFlag(RhiShaderStage::Fragment));
    commandList.draw(3u, 1u, 0u, 0u);
    commandList.endRendering();
}

void PostProcessPass::renderComposite(RhiCommandList& commandList, const RhiPipelineHandle pipeline,
...
void PostProcessPass::uploadCompositeParams(RhiCommandList& commandList, const PostProcessCompositeParams& params) {
    commandList.updateBuffer(m_compositeParamsBuffer, 0u, &params, sizeof(params));
}

PostProcessPass::PostProcessCompositeParams PostProcessPass::buildCompositeParams(const bool useAutoExposureTexture,
                                                                                  const bool hasBloom) const {
    const bool sunRaysEnabled = m_effects.sunRaysEnabled && hasBloom;
    const float noiseDitherStrength = m_effects.shaderpackGradingEnabled ? m_effects.noiseDitherStrength : 0.0f;
    return {glm::ivec4(hasBloom ? 1 : 0, useAutoExposureTexture ? 1 : 0, sunRaysEnabled ? 1 : 0,
                       m_effects.shaderpackGradingEnabled ? 1 : 0),
            glm::ivec4(std::clamp(m_effects.bloomMipCount, 1, kBloomMipCount), m_effects.tonemapMode,
                       m_effects.postprocessDebugMode, 0),
            glm::ivec4(m_effects.underwaterEnabled ? 1 : 0, m_effects.purkinjeShiftEnabled ? 1 : 0,
                       m_effects.bloomyFogEnabled ? 1 : 0, 0),
            glm::vec4(m_effects.bloomStrength, m_adaptedExposure, m_effects.gamma, m_effects.screenRollRadians),
            glm::vec4(m_effects.sunScreenPos, m_effects.sunVisibility, m_effects.sunRayStrength),
            glm::vec4(m_effects.colorTemperature, m_effects.vibrance, m_effects.splitToneStrength,
                      m_effects.vignetteStrength),
            glm::vec4(noiseDitherStrength, m_effects.sharpenStrength, m_effects.saturation, m_effects.contrast),
            glm::vec4(m_effects.underwaterTint, m_effects.underwaterStrength),
...
bool PostProcessPass::ensureSceneCaptureTargets(RhiDevice& rhiDevice, const int width, const int height) {
    ...
    if (m_sceneColorHandle.isValid() && m_sceneDepthHandle.isValid() && m_captureWidth == width &&
    ...
    const RhiTextureUsageFlags depthUsage =
        ...
        rhiFlag(RhiTextureUsage::TransferSrc) | rhiFlag(RhiTextureUsage::TransferDst);
    if (!createTextureAndView(rhiDevice, "PostProcess.SceneColor", RhiTextureFormat::Rgba16Float,
                              static_cast<uint32_t>(width), static_cast<uint32_t>(height), colorUsage,
                              m_sceneColorHandle, m_sceneColorView) ||
        !createTextureAndView(rhiDevice, "PostProcess.SceneDepth", RhiTextureFormat::Depth32Float,
                              static_cast<uint32_t>(width), static_cast<uint32_t>(height), depthUsage,
                              m_sceneDepthHandle, m_sceneDepthView)) {
        destroySceneCaptureTargets();
        return false;
    }

    RhiCommandList& initializeCommandList = beginCommandList("PostProcess.SceneCaptureInitialization.Commands");
    initializeCommandList.textureBarrier(
        {m_sceneColorHandle, RhiResourceState::Undefined, RhiResourceState::ShaderRead});
    initializeCommandList.textureBarrier(
        {m_sceneDepthHandle, RhiResourceState::Undefined, RhiResourceState::DepthRead});
    submitCommandList(rhiDevice, initializeCommandList, "PostProcess.SceneCaptureInitialization.Submit");
    m_captureWidth = width;
    m_captureHeight = height;
    return true;
}

bool PostProcessPass::ensureExposureState(RhiDevice& rhiDevice) {
    if (m_rhiDevice != nullptr && m_rhiDevice != &rhiDevice) {
        destroyRhiResources();
    }
    m_rhiDevice = &rhiDevice;
    if (m_exposureStateHandle[0].isValid() && m_exposureStateView[0].isValid() && m_exposureStateHandle[1].isValid() &&
        m_exposureStateView[1].isValid()) {
        return true;
    }

    destroyExposureState();
    for (int index = 0; index < 2; ++index) {
    ...
    RhiCommandList& initializeCommandList = beginCommandList("PostProcess.ExposureStateInitialization.Commands");
    for (int index = 0; index < 2; ++index) {
        initializeCommandList.textureBarrier(
            {m_exposureStateHandle[index], RhiResourceState::Undefined, RhiResourceState::RenderTarget});
        ...
        initializeCommandList.textureBarrier(
            {m_exposureStateHandle[index], RhiResourceState::RenderTarget, RhiResourceState::ShaderRead});
    }
    submitCommandList(rhiDevice, initializeCommandList, "PostProcess.ExposureStateInitialization.Submit");
...
bool PostProcessPass::ensureProcessingTargets(RhiDevice& rhiDevice, const int width, const int height) {
    if (m_rhiDevice != nullptr && m_rhiDevice != &rhiDevice) {
        destroyRhiResources();
    }
    m_rhiDevice = &rhiDevice;
    if (width <= 0 || height <= 0 || !ensureExposureState(rhiDevice)) {
        return false;
    }
    if (m_bloomHandle[0][0].isValid() && m_processingWidth == width && m_processingHeight == height) {
        return true;
    }
    ...
    for (int mip = 0; mip < kBloomMipCount; ++mip) {
        ...
        for (int ping = 0; ping < 2; ++ping) {
            if (!createTextureAndView(rhiDevice, "PostProcess.Bloom", RhiTextureFormat::Rgba16Float,
                                      static_cast<uint32_t>(m_bloomMipSize[mip].x),
                                      static_cast<uint32_t>(m_bloomMipSize[mip].y),
                                      rhiFlag(RhiTextureUsage::Sampled) | rhiFlag(RhiTextureUsage::ColorAttachment),
                                      m_bloomHandle[mip][ping], m_bloomView[mip][ping])) {
                destroyProcessingTargets();
                return false;
            }
        }
    ...
    const int exposureBaseLod =
        std::min(kAutoExposureLod,
                 std::max(0, static_cast<int>(std::floor(std::log2(static_cast<float>(std::max(width, height)))))));
    const glm::ivec2 exposureBaseSize(std::max(1, width >> exposureBaseLod), std::max(1, height >> exposureBaseLod));
    glm::ivec2 exposureSize(std::max(1, exposureBaseSize.x / 2), std::max(1, exposureBaseSize.y / 2));
    m_exposureMipCount = 0;
    for (int mip = 0; mip < kExposureMipCount; ++mip) {
        m_exposureMipSize[mip] = exposureSize;
        if (!createTextureAndView(rhiDevice, "PostProcess.Exposure", RhiTextureFormat::Rg16Float,
                                  static_cast<uint32_t>(exposureSize.x), static_cast<uint32_t>(exposureSize.y),
                                  rhiFlag(RhiTextureUsage::Sampled) | rhiFlag(RhiTextureUsage::ColorAttachment),
                                  m_exposureHandle[mip], m_exposureView[mip])) {
            destroyProcessingTargets();
            return false;
        }
        ++m_exposureMipCount;
    ...
    RhiCommandList& initializeCommandList = beginCommandList("PostProcess.ProcessingInitialization.Commands");
    for (int mip = 0; mip < kBloomMipCount; ++mip) {
        for (int ping = 0; ping < 2; ++ping) {
            initializeCommandList.textureBarrier(
                {m_bloomHandle[mip][ping], RhiResourceState::Undefined, RhiResourceState::ShaderRead});
        }
    }
    for (int mip = 0; mip < m_exposureMipCount; ++mip) {
        initializeCommandList.textureBarrier(
            {m_exposureHandle[mip], RhiResourceState::Undefined, RhiResourceState::ShaderRead});
    }
    submitCommandList(rhiDevice, initializeCommandList, "PostProcess.ProcessingInitialization.Submit");
...
bool PostProcessPass::ensureCompositeTarget(RhiDevice& rhiDevice, const int width, const int height) {
    ...
    destroyCompositeBindGroups();
    if (!createTextureAndView(rhiDevice, "PostProcess.Composite", RhiTextureFormat::Rgba8Unorm,
                              static_cast<uint32_t>(std::max(1, width)), static_cast<uint32_t>(std::max(1, height)),
                              rhiFlag(RhiTextureUsage::Sampled) | rhiFlag(RhiTextureUsage::ColorAttachment) |
                                  rhiFlag(RhiTextureUsage::TransferSrc),
                              m_compositeHandle, m_compositeView)) {
        return false;
    }
    RhiCommandList& commandList = beginCommandList("PostProcess.CompositeTargetInitialization.Commands");
    commandList.textureBarrier({m_compositeHandle, RhiResourceState::Undefined, RhiResourceState::ShaderRead});
    submitCommandList(rhiDevice, commandList, "PostProcess.CompositeTargetInitialization.Submit");
    return true;
}
...
bool PostProcessPass::ensureRhiPipelines(RhiDevice& rhiDevice) {
    ...
    if (!vertexSource.has_value() || !postProcessSource.has_value() || !bloomExtractSource.has_value() ||
        !bloomBlurSource.has_value() || !exposureDownsampleSource.has_value() || !exposureResolveSource.has_value()) {
        return false;
    }
    ...
    m_fullscreenVertexShader = createShader("PostProcess.Vertex", RhiShaderStage::Vertex, *vertexSource);
    m_postProcessFragmentShader =
        createShader("PostProcess.Composite.Fragment", RhiShaderStage::Fragment, *postProcessSource);
    m_bloomExtractFragmentShader =
        createShader("PostProcess.BloomExtract.Fragment", RhiShaderStage::Fragment, *bloomExtractSource);
    m_bloomBlurFragmentShader =
        createShader("PostProcess.BloomBlur.Fragment", RhiShaderStage::Fragment, *bloomBlurSource);
    m_exposureDownsampleFragmentShader =
        createShader("PostProcess.ExposureDownsample.Fragment", RhiShaderStage::Fragment, *exposureDownsampleSource);
    m_exposureResolveFragmentShader =
        createShader("PostProcess.ExposureResolve.Fragment", RhiShaderStage::Fragment, *exposureResolveSource);
    if (!m_fullscreenVertexShader.isValid() || !m_postProcessFragmentShader.isValid() ||
        ...
        !m_exposureDownsampleFragmentShader.isValid() || !m_exposureResolveFragmentShader.isValid()) {
        destroyRhiResources();
        return false;
    }
    ...
    if (!m_linearClampSampler.isValid() || !m_nearestClampSampler.isValid() || !m_nearestRepeatSampler.isValid()) {
        destroyRhiResources();
        return false;
    }
    ...
    bufferDesc.usage = rhiFlag(RhiBufferUsage::Uniform) | rhiFlag(RhiBufferUsage::TransferDst);
    ...
    m_singleTextureBindGroupLayout = createTextureLayout("PostProcess.SingleTexture.BindGroupLayout", 1u);
    m_twoTextureBindGroupLayout = createTextureLayout("PostProcess.TwoTexture.BindGroupLayout", 2u);
    ...
    compositeLayoutDesc.entries.push_back({12u, RhiBindingType::UniformBuffer, rhiFlag(RhiShaderStage::Fragment), 1u});
    m_compositeBindGroupLayout = rhiDevice.createBindGroupLayout(compositeLayoutDesc);
    if (!m_singleTextureBindGroupLayout.isValid() || !m_twoTextureBindGroupLayout.isValid() ||
        !m_compositeBindGroupLayout.isValid()) {
        destroyRhiResources();
        return false;
    }

    auto createPipelineLayout = [&](const char* debugName, const RhiBindGroupLayoutHandle bindGroupLayout,
        ...
        desc.pushConstantStages = pushConstantBytes > 0u ? rhiFlag(RhiShaderStage::Fragment) : 0u;
        return rhiDevice.createPipelineLayout(desc);
    };
    m_exposureDownsamplePipelineLayout =
        createPipelineLayout("PostProcess.ExposureDownsample.PipelineLayout", m_singleTextureBindGroupLayout,
                             sizeof(ExposureDownsamplePushConstants));
    m_exposureResolvePipelineLayout =
        createPipelineLayout("PostProcess.ExposureResolve.PipelineLayout", m_twoTextureBindGroupLayout,
                             sizeof(ExposureResolvePushConstants));
    m_bloomExtractPipelineLayout = createPipelineLayout("PostProcess.BloomExtract.PipelineLayout",
                                                        m_singleTextureBindGroupLayout, sizeof(glm::ivec4));
    m_bloomBlurPipelineLayout =
        createPipelineLayout("PostProcess.BloomBlur.PipelineLayout", m_singleTextureBindGroupLayout, sizeof(glm::vec4));
    m_compositePipelineLayout =
        createPipelineLayout("PostProcess.Composite.PipelineLayout", m_compositeBindGroupLayout, 0u);
    if (!m_exposureDownsamplePipelineLayout.isValid() || !m_exposureResolvePipelineLayout.isValid() ||
        !m_bloomExtractPipelineLayout.isValid() || !m_bloomBlurPipelineLayout.isValid() ||
        !m_compositePipelineLayout.isValid()) {
        destroyRhiResources();
        return false;
    }

    auto createPipeline = [&](const char* debugName, const RhiShaderHandle fragmentShader,
    ...
    m_exposureDownsamplePipeline =
        createPipeline("PostProcess.ExposureDownsample.Pipeline", m_exposureDownsampleFragmentShader,
                       m_exposureDownsamplePipelineLayout, RhiTextureFormat::Rg16Float);
    m_exposureResolvePipeline = createPipeline("PostProcess.ExposureResolve.Pipeline", m_exposureResolveFragmentShader,
                                               m_exposureResolvePipelineLayout, RhiTextureFormat::Rgba16Float);
    m_bloomExtractPipeline = createPipeline("PostProcess.BloomExtract.Pipeline", m_bloomExtractFragmentShader,
                                            m_bloomExtractPipelineLayout, RhiTextureFormat::Rgba16Float);
    m_bloomBlurPipeline = createPipeline("PostProcess.BloomBlur.Pipeline", m_bloomBlurFragmentShader,
                                         m_bloomBlurPipelineLayout, RhiTextureFormat::Rgba16Float);
    m_compositeTexturePipeline = createPipeline("PostProcess.CompositeTexture.Pipeline", m_postProcessFragmentShader,
                                                m_compositePipelineLayout, RhiTextureFormat::Rgba8Unorm);
    if (!m_exposureDownsamplePipeline.isValid() || !m_exposureResolvePipeline.isValid() ||
        !m_bloomExtractPipeline.isValid() || !m_bloomBlurPipeline.isValid() || !m_compositeTexturePipeline.isValid()) {
        destroyRhiResources();
        return false;
    }
    return true;
...
bool PostProcessPass::ensureSwapchainCompositePipeline(RhiDevice& rhiDevice, const RhiTextureFormat colorFormat) {
    if (colorFormat == RhiTextureFormat::Undefined || !m_fullscreenVertexShader.isValid() ||
        !m_postProcessFragmentShader.isValid() || !m_compositePipelineLayout.isValid()) {
        return false;
    }
    if (m_compositeSwapchainPipeline.isValid() && m_compositeSwapchainFormat == colorFormat) {
        return true;
    }
...
bool PostProcessPass::ensureGbufferDepthTextureView(RhiDevice& rhiDevice, const RhiTextureHandle texture) {
    if (m_gbufferDepthTextureView.isValid() && sameTextureHandle(m_gbufferDepthViewTexture, texture)) {
        return true;
    }
    destroyCompositeBindGroups();
...
bool PostProcessPass::rebuildTargetBindGroups() {
    if (m_bloomExtractBindGroup.isValid() && m_exposureMipCount > 0 &&
        m_exposureDownsampleBindGroup[m_exposureMipCount - 1].isValid() && m_exposureResolveBindGroup[0].isValid() &&
        m_exposureResolveBindGroup[1].isValid()) {
        return true;
    }
    if (m_rhiDevice == nullptr || !m_singleTextureBindGroupLayout.isValid() || !m_twoTextureBindGroupLayout.isValid() ||
        m_exposureMipCount <= 0) {
        return false;
    }
    destroyTargetBindGroups();

    auto createSingleTextureBindGroup = [&](const RhiTextureViewHandle view, const RhiSamplerHandle sampler) {
    ...
    m_bloomExtractBindGroup = createSingleTextureBindGroup(m_hdrInputView, m_linearClampSampler);
    if (!m_bloomExtractBindGroup.isValid()) {
        destroyTargetBindGroups();
        return false;
    }
    for (int mip = 0; mip < kBloomMipCount; ++mip) {
        m_bloomBlurBindGroup[mip][0] = createSingleTextureBindGroup(m_bloomView[mip][0], m_linearClampSampler);
        m_bloomBlurBindGroup[mip][1] = createSingleTextureBindGroup(m_bloomView[mip][1], m_linearClampSampler);
        if (!m_bloomBlurBindGroup[mip][0].isValid() || !m_bloomBlurBindGroup[mip][1].isValid()) {
            destroyTargetBindGroups();
            return false;
        }
    }
    for (int mip = 0; mip < m_exposureMipCount; ++mip) {
        const RhiTextureViewHandle source = mip == 0 ? m_hdrInputView : m_exposureView[mip - 1];
        m_exposureDownsampleBindGroup[mip] = createSingleTextureBindGroup(source, m_nearestClampSampler);
    ...
    const RhiTextureViewHandle finalExposureView = m_exposureView[m_exposureMipCount - 1];
    for (int readIndex = 0; readIndex < 2; ++readIndex) {
        RhiBindGroupDesc desc;
        desc.layout = m_twoTextureBindGroupLayout;
        const RhiTextureViewHandle views[2] = {finalExposureView, m_exposureStateView[readIndex]};
...
bool PostProcessPass::rebuildCompositeBindGroups() {
    ...
    if (m_rhiDevice == nullptr || !m_compositeBindGroupLayout.isValid() || !m_hdrInputView.isValid() ||
        !m_noiseTextureView.isValid() || !m_gbufferDepthTextureView.isValid() || !m_sceneDepthView.isValid()) {
        return false;
    }
    destroyCompositeBindGroups();

    const RhiTextureViewHandle commonViews[12] = {m_hdrInputView,
                                                  m_bloomView[0][0],
                                                  m_bloomView[1][0],
                                                  m_bloomView[2][0],
                                                  m_bloomView[3][0],
                                                  ...
                                                  {},
                                                  m_sceneDepthView};
    const RhiSamplerHandle samplers[12] = {m_linearClampSampler,  m_linearClampSampler,  m_linearClampSampler,
                                           m_linearClampSampler,  m_linearClampSampler,  m_linearClampSampler,
                                           m_linearClampSampler,  m_linearClampSampler,  m_nearestRepeatSampler,
                                           m_nearestClampSampler, m_nearestClampSampler, m_nearestClampSampler};
    for (int exposureIndex = 0; exposureIndex < 2; ++exposureIndex) {
        ...
        for (uint32_t binding = 0u; binding < 12u; ++binding) {
            ...
            entry.resource.combinedTextureSampler.textureView =
                binding == 10u ? m_exposureStateView[exposureIndex] : commonViews[binding];
...
void PostProcessPass::bindCompositeOutput(RhiCommandList& commandList, const int width, const int height) {
    beginPostProcessColorOutput(commandList, "PostProcessCompositeTexture", m_compositeView, width, height, false);
}

void PostProcessPass::bindBackbufferOutput(RhiCommandList& commandList, const RhiTextureViewHandle swapchainColorView,
                                           const int width, const int height, const bool clearColor) {
    beginPostProcessColorOutput(commandList, "PostProcessBackbuffer", swapchainColorView, width, height, clearColor);
}
...
void PostProcessPass::destroySceneCaptureTargets() {
    ...
    destroyTargetBindGroups();
    if (m_rhiDevice != nullptr) {
        auto destroyTextureAndView = [&](RhiTextureHandle& texture, RhiTextureViewHandle& view) {
            if (view.isValid()) {
...
void PostProcessPass::destroyExposureState() {
    destroyCompositeBindGroups();
    destroyTargetBindGroups();
    if (m_rhiDevice != nullptr) {
        for (int index = 0; index < 2; ++index) {
            if (m_exposureStateView[index].isValid()) {
                m_rhiDevice->destroyTextureView(m_exposureStateView[index]);
            }
            if (m_exposureStateHandle[index].isValid()) {
                m_rhiDevice->destroyTexture(m_exposureStateHandle[index]);
            }
...
void PostProcessPass::destroyProcessingTargets() {
    ...
    if (m_rhiDevice != nullptr) {
        ...
        for (int mip = 0; mip < kBloomMipCount; ++mip) {
            ...
            for (int ping = 0; ping < 2; ++ping) {
                destroyTextureAndView(m_bloomHandle[mip][ping], m_bloomView[mip][ping]);
            }
        }
        for (int mip = 0; mip < kExposureMipCount; ++mip) {
...
void PostProcessPass::destroyRhiResources() {
    ...
    if (m_rhiDevice != nullptr) {
        ...
        const RhiPipelineHandle pipelines[] = {m_exposureDownsamplePipeline, m_exposureResolvePipeline,
                                               m_bloomExtractPipeline,       m_bloomBlurPipeline,
                                               m_compositeTexturePipeline,   m_compositeSwapchainPipeline};
        for (const RhiPipelineHandle pipeline : pipelines) {
            if (pipeline.isValid()) {
                m_rhiDevice->destroyPipeline(pipeline);
            }
        }
        const RhiShaderHandle shaders[] = {m_fullscreenVertexShader,           m_postProcessFragmentShader,
                                           m_bloomExtractFragmentShader,       m_bloomBlurFragmentShader,
                                           m_exposureDownsampleFragmentShader, m_exposureResolveFragmentShader};
        ...
        const RhiPipelineLayoutHandle pipelineLayouts[] = {
            m_exposureDownsamplePipelineLayout, m_exposureResolvePipelineLayout, m_bloomExtractPipelineLayout,
            m_bloomBlurPipelineLayout, m_compositePipelineLayout};
        for (const RhiPipelineLayoutHandle layout : pipelineLayouts) {
            if (layout.isValid()) {
                m_rhiDevice->destroyPipelineLayout(layout);
            }
        }
        const RhiBindGroupLayoutHandle bindGroupLayouts[] = {m_singleTextureBindGroupLayout,
                                                             m_twoTextureBindGroupLayout, m_compositeBindGroupLayout};
        ...
        const RhiSamplerHandle samplers[] = {m_linearClampSampler, m_nearestClampSampler, m_nearestRepeatSampler};
