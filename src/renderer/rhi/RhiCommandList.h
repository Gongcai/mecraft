...
public:
    ...
    /// Draws indexed commands generated by a compute pass, using a GPU-written draw count.
    /// @param indirectBuffer Buffer containing VkDrawIndexedIndirectCommand records.
    /// @param indirectOffset Byte offset of the first command; must be four-byte aligned.
    /// @param maxDrawCount Maximum number of commands the GPU may consume.
    /// @param stride Byte distance between consecutive commands.
    /// @param countBuffer Storage/indirect buffer containing one uint32 draw count.
    /// @param countOffset Byte offset of the draw count; must be four-byte aligned.
    virtual void drawIndexedIndirectCount(RhiBufferHandle indirectBuffer, uint64_t indirectOffset,
                                          uint32_t maxDrawCount, uint32_t stride, RhiBufferHandle countBuffer,
