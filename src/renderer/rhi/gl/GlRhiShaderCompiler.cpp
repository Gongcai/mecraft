...
[[nodiscard]] bool createSpvcCompiler(SpvcContextOwner& owner,
    ...
    if (spvc_context_parse_spirv(owner.context(), spirv.data(), spirv.size(), &parsedIr) != SPVC_SUCCESS ||
        spvc_context_create_compiler(owner.context(), SPVC_BACKEND_GLSL, parsedIr,
...
[[nodiscard]] const GlRhiShaderBindingRemap* findRemap(
...
[[nodiscard]] bool remapResources(const spvc_compiler compiler,
    ...
    for (size_t i = 0u; i < resourceCount; ++i) {
        ...
        const uint32_t set = spvc_compiler_get_decoration(compiler, resource.id,
                                                          SpvDecorationDescriptorSet);
        const uint32_t binding = spvc_compiler_get_decoration(compiler, resource.id,
...
std::optional<std::string> crossCompileShaderToOpenGl(
    ...
    if (!remapResources(compiler, resources, SPVC_RESOURCE_TYPE_UNIFORM_BUFFER,
                        RhiBindingType::UniformBuffer, remaps, errorMessage) ||
        !remapResources(compiler, resources, SPVC_RESOURCE_TYPE_STORAGE_BUFFER,
                        RhiBindingType::StorageBuffer, remaps, errorMessage) ||
        !remapResources(compiler, resources, SPVC_RESOURCE_TYPE_SAMPLED_IMAGE,
                        RhiBindingType::CombinedTextureSampler, remaps, errorMessage) ||
        !remapResources(compiler, resources, SPVC_RESOURCE_TYPE_SEPARATE_IMAGE,
                        RhiBindingType::SampledTexture, remaps, errorMessage) ||
        !remapResources(compiler, resources, SPVC_RESOURCE_TYPE_SEPARATE_SAMPLERS,
                        RhiBindingType::Sampler, remaps, errorMessage) ||
        !remapResources(compiler, resources, SPVC_RESOURCE_TYPE_STORAGE_IMAGE,
                        RhiBindingType::StorageTexture, remaps, errorMessage)) {
        return std::nullopt;
    }
    ...
    if (pushConstantCount != 0u) {
        ...
        spvc_compiler_set_decoration(compiler, pushConstants[0].id,
                                     SpvDecorationBinding, *pushConstantBinding);
    }
    ...
    if (spvc_compiler_create_compiler_options(compiler, &options) != SPVC_SUCCESS ||
        ...
        spvc_compiler_options_set_bool(
            options, SPVC_COMPILER_OPTION_GLSL_EMIT_PUSH_CONSTANT_AS_UNIFORM_BUFFER,
