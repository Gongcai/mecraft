...
[[nodiscard]] inline VkImageAspectFlags toVkImageAspectFlags(
    const RhiTextureAspectFlags aspects,
    const RhiTextureFormat format) {
    if (aspects == 0u) {
        return defaultAspectForFormat(format);
    }
    VkImageAspectFlags result = 0u;
    if ((aspects & rhiFlag(RhiTextureAspect::Color)) != 0u) result |= VK_IMAGE_ASPECT_COLOR_BIT;
    if ((aspects & rhiFlag(RhiTextureAspect::Depth)) != 0u) result |= VK_IMAGE_ASPECT_DEPTH_BIT;
    if ((aspects & rhiFlag(RhiTextureAspect::Stencil)) != 0u) result |= VK_IMAGE_ASPECT_STENCIL_BIT;
    return result;
}
...
[[nodiscard]] inline VkShaderStageFlags toVkShaderStageFlags(const RhiShaderStageFlags stages) {
    ...
    if ((stages & rhiFlag(RhiShaderStage::Vertex)) != 0u) result |= VK_SHADER_STAGE_VERTEX_BIT;
    if ((stages & rhiFlag(RhiShaderStage::Fragment)) != 0u) result |= VK_SHADER_STAGE_FRAGMENT_BIT;
    if ((stages & rhiFlag(RhiShaderStage::Compute)) != 0u) result |= VK_SHADER_STAGE_COMPUTE_BIT;
    return result;
}
