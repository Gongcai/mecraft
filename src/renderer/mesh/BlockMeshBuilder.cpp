...
FaceUvRect makeTorchUvRect(const float left, const float top, const float right, const float bottom) {
...
void emitFace(std::vector<BlockVertex>& vertices, const std::array<glm::vec3, 4>& corners,
              const std::array<glm::vec2, 4>& uv, const float normal, const AnimatedTextureRef& textureRef,
              const uint8_t tintKind, const uint8_t tintU, const uint8_t tintV, const uint8_t derivativeMaterialId) {
    ...
    for (const int idx : kFaceIndices) {
        ...
        vertices.push_back(makeBlockVertex(pos.x, pos.y, pos.z, uvCoord.x, uvCoord.y, normal, 1.0f, 0.0f, 3.0f, layer,
                                           frameCount, animationFps, animated, tintKind, tintU, tintV,
...
void appendModelVertices(std::vector<BlockVertex>& vertices, const BlockStateId stateId, const BlockDef& def,
    ...
    for (const ModelElement& element : model.elements) {
        for (int faceIndex = 0; faceIndex < 6; ++faceIndex) {
            ...
            emitFace(vertices, corners, applyHorizontalUvLock(buildModelFaceUv(face), faceIndex, variant->transform),
                     static_cast<float>(transformFaceIndex(faceIndex, variant->transform)), textureRef, tintKind, tintU,
                     tintV, def.derivativeMaterialId);
        }
    }
}
...
BlockCubeMesh uploadBlockCubeMesh(const std::vector<BlockVertex>& vertices, RhiDevice& rhiDevice,
    ...
    mesh.rhiVertexBuffer = rhiDevice.createBuffer(bufferDesc, vertices.data(), vertices.size() * sizeof(BlockVertex));
    ...
    if (isModelShape(def)) {
        ...
        const auto emitQuad = [&](const std::array<glm::vec3, 4>& corners) {
            for (const int idx : kFaceIndices) {
                ...
                vertices.push_back(makeBlockVertex(pos.x, pos.y, pos.z, uvCoord.x, uvCoord.y, crossMarker, 1.0f, 0.0f,
                                                   3.0f, layer, 1.0f, 0.0f, 0.0f, tintKind, tintU, tintV,
        ...
        for (int face = 0; face < 6; ++face) {
            ...
            for (const int idx : kFaceIndices) {
                ...
                vertices.push_back(makeBlockVertex(pos.x, pos.y, pos.z, uvCoord.x, uvCoord.y, static_cast<float>(face),
                                                   1.0f, 0.0f, 3.0f, layer, 1.0f, 0.0f, 0.0f, tintKind, tintU, tintV,
                                                   def.derivativeMaterialId));
            }
        }
    }
