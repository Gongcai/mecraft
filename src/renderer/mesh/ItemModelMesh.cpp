...
void addQuad(std::vector<ItemModelVertex>& vertices, const ItemModelVertex& a, const ItemModelVertex& b,
             const ItemModelVertex& c, const ItemModelVertex& d) {
...
bool buildExtrudedItemMesh(const TextureAtlas& atlas, const std::vector<unsigned char>& atlasPixels,
                           const int tileIndex, std::vector<ItemModelVertex>& outVertices) {
    outVertices.clear();
    if (tileIndex < 0 || !atlas.texture.isValid() || atlas.tilesPerRow <= 0 || atlas.tileSize <= 0 ||
        atlas.atlasWidth <= 0 || atlas.atlasHeight <= 0) {
        return false;
    }
    ...
    for (int y = 0; y < tileSize; ++y) {
        for (int x = 0; x < tileSize; ++x) {
            ...
            addQuad(outVertices, {x0, y0, zFront, u0, v0, 1.0f, kFrontN[0], kFrontN[1], kFrontN[2]},
            ...
            addQuad(outVertices, {x1, y0, zBack, u1, v0, 0.62f, kBackN[0], kBackN[1], kBackN[2]},
            ...
            if (!sampleOpaque(x - 1, y)) {
                addQuad(outVertices, {x0, y0, zBack, u0, v0, 0.78f, kLeftN[0], kLeftN[1], kLeftN[2]},
            ...
            if (!sampleOpaque(x + 1, y)) {
                addQuad(outVertices, {x1, y0, zFront, u1, v0, 0.85f, kRightN[0], kRightN[1], kRightN[2]},
            ...
            if (!sampleOpaque(x, y + 1)) {
                addQuad(outVertices, {x0, y1, zFront, u0, v1, 0.92f, kBottomN[0], kBottomN[1], kBottomN[2]},
            ...
            if (!sampleOpaque(x, y - 1)) {
                addQuad(outVertices, {x0, y0, zBack, u0, v0, 0.70f, kTopN[0], kTopN[1], kTopN[2]},
