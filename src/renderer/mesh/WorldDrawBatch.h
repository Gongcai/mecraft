...
enum class TransparentBatchKind : uint8_t { Generic = 0, Water = 1 };
...
struct WorldDrawBatch {
    ...
    void addOpaque(const GpuMeshRange& range) { entries.push_back({range, 0.0f, TransparentBatchKind::Generic}); }

    void addCutout(const GpuMeshRange& range) { entries.push_back({range, 0.0f, TransparentBatchKind::Generic}); }
