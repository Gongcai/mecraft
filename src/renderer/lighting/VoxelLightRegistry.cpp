...
[[nodiscard]] bool sameSourceSet(
    const std::vector<CachedVoxelLightSource>& lhs,
    const std::vector<CachedVoxelLightSource>& rhs) {
    if (lhs.size() != rhs.size()) {
        return false;
    }
    for (std::size_t index = 0u; index < lhs.size(); ++index) {
        if (lhs[index].localBlockIndex != rhs[index].localBlockIndex ||
            lhs[index].blockId != rhs[index].blockId) {
            return false;
        }
    }
    return true;
...
[[nodiscard]] bool sourceEnabledForState(
    const BlockAnalyticLightDefinition& definition,
    const BlockStateId state) {
    if (definition.enabledStatePropertyIndex ==
        BlockAnalyticLightDefinition::kUnconditionalStateIndex) {
        return definition.enabledStateValueIndex ==
            BlockAnalyticLightDefinition::kUnconditionalStateIndex;
    }
    return definition.enabledStateValueIndex !=
               BlockAnalyticLightDefinition::kUnconditionalStateIndex &&
           BlockStateRegistry::getPropertyIndex(
               state, definition.enabledStatePropertyIndex) ==
...
struct VoxelLightRegistry::Impl final {
    ...
    [[nodiscard]] bool rebuildChunk(
        const CachedVoxelLightChunk* previous,
        const Chunk& chunk,
...
bool VoxelLightRegistry::Impl::rebuildChunk(
    const CachedVoxelLightChunk* previous,
    const Chunk& chunk,
    CachedVoxelLightChunk& rebuilt) {
    std::unordered_map<uint32_t, renderer::contracts::StableLightId>
        previousIds;
    if (previous != nullptr) {
        ...
        for (int z = 0; z < Chunk::SIZE_Z; ++z) {
            for (int x = 0; x < Chunk::SIZE_X; ++x) {
                ...
                const BlockID blockId =
                    BlockStateRegistry::getBlockId(state);
                const BlockDef& block = BlockRegistry::getFast(blockId);
                if (!block.analyticLight.has_value() ||
                    !sourceEnabledForState(*block.analyticLight, state)) {
                    continue;
                }
                ...
                if (previousId != previousIds.end()) {
                    ...
                    const std::optional<renderer::contracts::StableLightId>
                        allocated =
                            renderer::contracts::allocateStableSceneId<
                                renderer::contracts::StableLightIdTag>();
                    if (!allocated.has_value()) {
                        setError(
                            "stable voxel light identity space is exhausted");
                        return false;
                    }
                    lightId = *allocated;
                }
                rebuilt.sources.push_back(
                    {localIndex, blockId, lightId});
            }
        }
    }
    return true;
...
bool VoxelLightRegistry::Impl::synchronize(
    const IWorldView& currentWorldView) {
    ...
    const uint64_t currentActiveRevision =
        currentWorldView.getActiveChunkRevision();
    const uint64_t currentContentRevision =
        currentWorldView.getBlockContentRevision();
    if (currentActiveRevision == activeChunkRevision &&
        currentContentRevision == blockContentRevision) {
        error.clear();
        return true;
    }

    const IWorldView::ChunkMap& activeChunks =
        currentWorldView.getActiveChunks();
    ...
    std::sort(
        ...
        [](const ActiveChunkEntry& lhs, const ActiveChunkEntry& rhs) {
    ...
    for (const ActiveChunkEntry& entry : active) {
        ...
        const CachedVoxelLightChunk* previous =
            existing != chunks.end() ? &existing->second : nullptr;
        if (previous != nullptr && previous->instance == entry.chunk &&
            previous->blockContentRevision ==
                entry.chunk->getBlockContentRevision()) {
            continue;
        }
        CachedVoxelLightChunk rebuilt;
        ...
        sourceSetChanged = sourceSetChanged ||
            (previous == nullptr
                 ? !rebuilt.sources.empty()
                 : !sameSourceSet(previous->sources, rebuilt.sources));
    ...
    for (auto it = chunks.begin(); it != chunks.end();) {
        if (activeKeys.find(it->first) == activeKeys.end()) {
            sourceSetChanged =
                sourceSetChanged || !it->second.sources.empty();
            it = chunks.erase(it);
        } else {
            ++it;
        }
...
VoxelLightRegistry::VoxelLightRegistry()
...
bool VoxelLightRegistry::buildGpuLights(
    ...
    for (const int64_t chunkKey : m_impl->orderedChunkKeys) {
        const CachedVoxelLightChunk& chunk =
            m_impl->chunks.at(chunkKey);
        for (const CachedVoxelLightSource& source : chunk.sources) {
            const BlockDef& block = BlockRegistry::getFast(source.blockId);
            if (!block.analyticLight.has_value()) {
                m_impl->setError(
                    "cached voxel light references a block without an analytic definition");
                return false;
            }
            const BlockAnalyticLightDefinition& definition =
                *block.analyticLight;
            const uint32_t y = source.localBlockIndex /
                static_cast<uint32_t>(Chunk::SIZE_X * Chunk::SIZE_Z);
            const uint32_t horizontal = source.localBlockIndex %
                static_cast<uint32_t>(Chunk::SIZE_X * Chunk::SIZE_Z);
            const uint32_t z = horizontal /
                static_cast<uint32_t>(Chunk::SIZE_X);
            const uint32_t x = horizontal %
                static_cast<uint32_t>(Chunk::SIZE_X);
            const double worldX =
                static_cast<double>(chunk.chunkX) * Chunk::SIZE_X + x +
                definition.positionOffsetMeters.x;
            const double worldY =
                static_cast<double>(y) +
                definition.positionOffsetMeters.y;
            const double worldZ =
                static_cast<double>(chunk.chunkZ) * Chunk::SIZE_Z + z +
                definition.positionOffsetMeters.z;

            GpuLightNormalizationInput input;
            input.lightId = source.lightId;
            input.type = GpuLightType::Point;
            input.positionMeters = {
                static_cast<float>(worldX - cameraPositionMeters.x),
            ...
            input.contributionFlags =
                gpuLightContributionFlagBit(
                    GpuLightContributionFlag::Diffuse) |
                gpuLightContributionFlagBit(
                    GpuLightContributionFlag::Specular) |
                gpuLightContributionFlagBit(
                    GpuLightContributionFlag::Volumetric);
            const GpuLightNormalizationResult normalized =
                normalizeGpuLight(input);
            if (!normalized.succeeded()) {
                m_impl->setError(
                    "voxel light normalization failed [block=" +
                    BlockRegistry::getNamespacedId(source.blockId).full() +
                    ", error=" +
                    gpuLightNormalizationErrorStableId(normalized.error) +
                    ", field=" +
