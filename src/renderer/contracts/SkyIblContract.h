...
inline constexpr uint32_t kSkyIblPrefilterWorkItemCount = kSkyIblCubeFaceCount * kSkyIblCubeMipCount;
inline constexpr uint32_t kSkyIblRevisionIntervalFrames = kSkyIblPrefilterWorkItemCount;
...
[[nodiscard]] constexpr float skyIblRoughnessForMip(const uint32_t mip) {
    ...
    return static_cast<float>(boundedMip) / static_cast<float>(kSkyIblCubeMipCount - 1u);
}
...
[[nodiscard]] constexpr float skyIblMipForRoughness(const float roughness) {
    return std::clamp(roughness, 0.0f, 1.0f) * static_cast<float>(kSkyIblCubeMipCount - 1u);
}
...
[[nodiscard]] constexpr uint64_t skyIblRevisionForFrame(const uint64_t frameIndex) {
    return frameIndex / static_cast<uint64_t>(kSkyIblRevisionIntervalFrames) + 1u;
}
...
[[nodiscard]] constexpr uint32_t skyIblMipForWorkItem(const uint32_t workItem) {
    return std::min(workItem, kSkyIblPrefilterWorkItemCount - 1u) / kSkyIblCubeFaceCount;
}

/// Maps one bounded prefilter work item to its destination cubemap face.
/// @param workItem Linear work item in the complete face/mip chain.
/// @return Destination face in the GGX prefilter texture.
[[nodiscard]] constexpr uint32_t skyIblFaceForWorkItem(const uint32_t workItem) {
    return std::min(workItem, kSkyIblPrefilterWorkItemCount - 1u) % kSkyIblCubeFaceCount;
}
