...
inline constexpr const char* kCameraPathInterpolation = "linear_position_slerp_orientation";
...
[[nodiscard]] CameraPathLoadResult parseCameraPathJson(std::string_view jsonText);

/// Loads and validates one versioned Camera Path file.
/// @param path File containing a Camera Path JSON document.
/// @return Parsed path, stable semantic hash, or a structured error.
[[nodiscard]] CameraPathLoadResult loadCameraPath(const std::filesystem::path& path);
...
[[nodiscard]] CameraPathError sampleCameraPath(const CameraPath& path, double timeSeconds, CameraPathPose& pose);
