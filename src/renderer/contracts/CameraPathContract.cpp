...
[[nodiscard]] bool
expectedField(const std::string_view name,
...
[[nodiscard]] CameraPathError
validateFields(const Json &object,
               const std::initializer_list<const char *> expectedFields,
...
[[nodiscard]] bool readString(const Json &object, const char *field,
...
[[nodiscard]] bool readVersion(const Json &root, uint32_t &version,
                               CameraPathLoadResult &result) {
...
[[nodiscard]] bool readFiniteNumber(const Json &value,
                                    const CameraPathError error,
...
[[nodiscard]] bool readVector(const Json &value, const CameraPathError error,
                              const std::string &fieldPath, glm::dvec3 &output,
                              CameraPathLoadResult &result) {
  ...
  for (size_t component = 0u; component < 3u; ++component) {
    double parsed = 0.0;
    if (!readFiniteNumber(value[component], error,
                          fieldPath + "[" + std::to_string(component) + "]",
                          parsed, result)) {
      return false;
    }
...
[[nodiscard]] bool buildOrientation(CameraPathKeyframe &keyframe,
                                    const size_t keyframeIndex,
                                    CameraPathLoadResult &result) {
  ...
  if (!std::isfinite(directionLengthSquared) ||
      directionLengthSquared <= kMinimumVectorLengthSquared) {
    result.error = CameraPathError::InvalidLookAt;
    result.detail =
        "root.keyframes[" + std::to_string(keyframeIndex) + "].look_at";
    return false;
  }
  const double upLengthSquared = glm::dot(keyframe.up, keyframe.up);
  if (!std::isfinite(upLengthSquared) ||
      upLengthSquared <= kMinimumVectorLengthSquared) {
  ...
  if (!std::isfinite(rightLengthSquared) ||
      rightLengthSquared <= kMinimumVectorLengthSquared) {
    result.error = CameraPathError::InvalidUpVector;
    result.detail = "root.keyframes[" + std::to_string(keyframeIndex) + "].up";
    return false;
  }
  ...
  glm::dquat orientation =
      glm::normalize(glm::quat_cast(glm::dmat3(right, correctedUp, -forward)));
...
[[nodiscard]] CameraPathLoadResult parseCameraPathRoot(const Json &root) {
  ...
  result.error = validateFields(root,
                                {"kind", "version", "id", "coordinate_system",
  ...
  if (!readString(root, "coordinate_system", "root.coordinate_system",
  ...
  if (!readString(root, "interpolation", "root.interpolation", interpolation,
                  result)) {
    return result;
  }
  ...
  for (size_t index = 0u; index < keyframes.size(); ++index) {
    ...
    const std::string keyframePath =
        "root.keyframes[" + std::to_string(index) + "]";
    ...
    result.error = validateFields(
        keyframeJson,
        {"time_seconds", "position", "look_at", "up", "vertical_fov_degrees"},
    ...
    if (!readFiniteNumber(*keyframeJson.find("time_seconds"),
                          CameraPathError::InvalidKeyframeTime,
                          keyframePath + ".time_seconds", keyframe.timeSeconds,
                          result)) {
      return result;
    }
    if ((index == 0u && keyframe.timeSeconds != 0.0) ||
        (index != 0u && keyframe.timeSeconds <= previousTime)) {
      result.error = CameraPathError::InvalidKeyframeTime;
      result.detail = keyframePath + ".time_seconds";
      return result;
    }
    if (!readVector(*keyframeJson.find("position"),
                    CameraPathError::InvalidPosition,
                    keyframePath + ".position", keyframe.position, result) ||
        !readVector(*keyframeJson.find("look_at"),
                    CameraPathError::InvalidLookAt, keyframePath + ".look_at",
                    keyframe.lookAt, result) ||
        !readVector(*keyframeJson.find("up"), CameraPathError::InvalidUpVector,
                    keyframePath + ".up", keyframe.up, result) ||
        !readFiniteNumber(*keyframeJson.find("vertical_fov_degrees"),
                          CameraPathError::InvalidFieldOfView,
                          keyframePath + ".vertical_fov_degrees",
                          keyframe.verticalFovDegrees, result)) {
      return result;
    }
    ...
    if (keyframe.verticalFovDegrees <= 1.0 ||
        keyframe.verticalFovDegrees >= 179.0) {
...
bool CameraPath::isValid() const {
  return !id.empty() && keyframes.size() >= 2u &&
         std::isfinite(durationSeconds) && durationSeconds > 0.0 &&
         keyframes.front().timeSeconds == 0.0 &&
         keyframes.back().timeSeconds == durationSeconds;
}
...
const char *cameraPathErrorStableId(const CameraPathError error) {
  switch (error) {
  case CameraPathError::None:
    return "None";
  case CameraPathError::FileOpenFailed:
    return "FileOpenFailed";
  case CameraPathError::FileReadFailed:
    return "FileReadFailed";
  case CameraPathError::InvalidJson:
    return "InvalidJson";
  case CameraPathError::InvalidRoot:
    return "InvalidRoot";
  case CameraPathError::MissingField:
    return "MissingField";
  case CameraPathError::UnexpectedField:
    return "UnexpectedField";
  case CameraPathError::InvalidFieldType:
    return "InvalidFieldType";
  case CameraPathError::InvalidKind:
    return "InvalidKind";
  case CameraPathError::UnsupportedVersion:
    return "UnsupportedVersion";
  case CameraPathError::InvalidIdentifier:
    return "InvalidIdentifier";
  case CameraPathError::InvalidCoordinateSystem:
    return "InvalidCoordinateSystem";
  case CameraPathError::InvalidInterpolation:
    return "InvalidInterpolation";
  case CameraPathError::InvalidKeyframeCount:
    return "InvalidKeyframeCount";
  case CameraPathError::InvalidKeyframeTime:
    return "InvalidKeyframeTime";
  case CameraPathError::InvalidPosition:
    return "InvalidPosition";
  case CameraPathError::InvalidLookAt:
    return "InvalidLookAt";
  case CameraPathError::InvalidUpVector:
    return "InvalidUpVector";
  case CameraPathError::InvalidFieldOfView:
    return "InvalidFieldOfView";
  case CameraPathError::InvalidPathState:
    return "InvalidPathState";
  case CameraPathError::SampleTimeNotFinite:
    return "SampleTimeNotFinite";
  case CameraPathError::SampleTimeOutOfRange:
    return "SampleTimeOutOfRange";
  }
  std::abort();
}
...
CameraPathLoadResult parseCameraPathJson(const std::string_view jsonText) {
  const Json root =
      Json::parse(jsonText.begin(), jsonText.end(), nullptr, false);
...
CameraPathError sampleCameraPath(const CameraPath &path,
  ...
  const auto upper = std::upper_bound(
      ...
      [](const double time, const CameraPathKeyframe &keyframe) {
  ...
  const glm::dquat orientation = glm::normalize(
      glm::slerp(previous.orientation, next.orientation, factor));
  if (!finiteQuaternion(orientation)) {
    return CameraPathError::InvalidPathState;
  }
  pose.position =
      previous.position + (next.position - previous.position) * factor;
  pose.forward = orientation * glm::dvec3(0.0, 0.0, -1.0);
  pose.up = orientation * glm::dvec3(0.0, 1.0, 0.0);
  pose.verticalFovDegrees =
      previous.verticalFovDegrees +
      (next.verticalFovDegrees - previous.verticalFovDegrees) * factor;
  return CameraPathError::None;
}
