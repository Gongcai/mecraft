...
bool buildExtrudedItemMesh(const TextureAtlas& atlas,
    ...
    for (int y = 0; y < tileSize; ++y) {
        for (int x = 0; x < tileSize; ++x) {
            ...
            addQuad(outVertices,
                    {x0, y0, zFront, u0, v0, 1.0f},
                    {x1, y0, zFront, u1, v0, 1.0f},
                    {x1, y1, zFront, u1, v1, 1.0f},
                    {x0, y1, zFront, u0, v1, 1.0f});

            addQuad(outVertices,
                    {x1, y0, zBack, u1, v0, 0.62f},
                    {x0, y0, zBack, u0, v0, 0.62f},
                    {x0, y1, zBack, u0, v1, 0.62f},
                    {x1, y1, zBack, u1, v1, 0.62f});

            if (!sampleOpaque(x - 1, y)) {
                addQuad(outVertices,
                        {x0, y0, zBack, u0, v0, 0.78f},
                        {x0, y0, zFront, u0, v0, 0.78f},
                        {x0, y1, zFront, u0, v1, 0.78f},
                        {x0, y1, zBack, u0, v1, 0.78f});
            }

            if (!sampleOpaque(x + 1, y)) {
                addQuad(outVertices,
                        {x1, y0, zFront, u1, v0, 0.85f},
                        {x1, y0, zBack, u1, v0, 0.85f},
                        {x1, y1, zBack, u1, v1, 0.85f},
                        {x1, y1, zFront, u1, v1, 0.85f});
            }

            if (!sampleOpaque(x, y + 1)) {
                addQuad(outVertices,
                        {x0, y1, zFront, u0, v1, 0.92f},
                        {x1, y1, zFront, u1, v1, 0.92f},
                        {x1, y1, zBack, u1, v1, 0.92f},
                        {x0, y1, zBack, u0, v1, 0.92f});
            }

            if (!sampleOpaque(x, y - 1)) {
                addQuad(outVertices,
                        {x0, y0, zBack, u0, v0, 0.70f},
                        {x1, y0, zBack, u1, v0, 0.70f},
                        {x1, y0, zFront, u1, v0, 0.70f},
                        {x0, y0, zFront, u0, v0, 0.70f});
            }
        }
    }
