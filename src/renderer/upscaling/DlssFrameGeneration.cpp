...
[[nodiscard]] std::array<float, 16u> toRowMajorArray(
...
[[nodiscard]] StreamlineDlssFrameConstants frameConstants(
...
[[nodiscard]] StreamlineDlssResource toStreamlineResource(
...
[[nodiscard]] bool validTexture(
    ...
    return info.image != VK_NULL_HANDLE && info.view != VK_NULL_HANDLE &&
           ...
           (info.usage & requiredUsage) == requiredUsage;
}

[[nodiscard]] bool sameOptions(
    const StreamlineDlssFrameGenerationOptions& lhs,
    const StreamlineDlssFrameGenerationOptions& rhs) {
    return lhs.enabled == rhs.enabled &&
           lhs.renderWidth == rhs.renderWidth &&
           lhs.renderHeight == rhs.renderHeight &&
           lhs.outputWidth == rhs.outputWidth &&
           lhs.outputHeight == rhs.outputHeight &&
           lhs.backBufferCount == rhs.backBufferCount &&
           lhs.colorFormat == rhs.colorFormat &&
           lhs.depthFormat == rhs.depthFormat &&
           lhs.motionVectorFormat == rhs.motionVectorFormat &&
           lhs.hudlessFormat == rhs.hudlessFormat &&
           lhs.uiFormat == rhs.uiFormat;
}

class DlssFrameGenerationPresentationBackend final : public PresentationBackend {
public:
    explicit DlssFrameGenerationPresentationBackend(VkRhiDevice& device)
        : m_device(device) {
    }
    ...
    PresentationBackendPresentResult presentFrame(
        ...
        if (m_enabled && !m_renderingGameFrames &&
            !streamline.clearDlssFrameGenerationResources(
                info.trackingFrameIndex, kStreamlineViewport)) {
        ...
        if (status != RhiFrameStatus::Success &&
        ...
        if (!streamline.queryDlssFrameGenerationState(
                kStreamlineViewport, state) || state.status != 0u) {
            std::cerr << streamline.lastError() << '\n';
            return {RhiFrameStatus::Error, 0u, 0u};
        }
        if (m_renderingGameFrames) {
            if (state.inputsProcessingCompletionFence == nullptr ||
                state.inputsProcessingCompletionValue == 0u ||
                !VkRhiInterop::queueExternalTimelineWait(
                    m_device,
        ...
        const uint32_t displayedFrames = m_renderingGameFrames
    ...
    bool cancelFrame(const RhiPresentInfo& info) override {
        ...
        const bool resourcesCleared =
            !m_enabled || streamline.clearDlssFrameGenerationResources(
                info.trackingFrameIndex, kStreamlineViewport);
        if (!resourcesCleared) {
            std::cerr << streamline.lastError() << '\n';
        }
    ...
    bool setVsyncEnabled(const bool enabled) override {
        return !m_enabled && m_device.setVsyncEnabled(enabled);
    }

    [[nodiscard]] bool supportsFrameGeneration() const override {
        const StreamlineRuntime& streamline = StreamlineRuntime::instance();
        return streamline.dlssFrameGenerationSupported() &&
               streamline.reflexLowLatencyAvailable() &&
               (!m_device.vsyncEnabled() ||
                m_device.capabilities().vsyncControl);
    }
    ...
    [[nodiscard]] bool frameGenerationActive() const override {
        return m_enabled && m_renderingGameFrames;
    }
    ...
    bool setFrameGenerationEnabled(const bool enabled) override {
        ...
        if (enabled) {
            ...
            if (m_vsyncBeforeFrameGeneration &&
                !m_device.setVsyncEnabled(false)) {
                return false;
            }
            if (!VkRhiInterop::recreateFrameGenerationSwapchain(
                    m_device, true)) {
                std::cerr << streamline.lastError() << '\n';
                return false;
            }
        ...
        if (m_configured) {
            ...
            if (!streamline.configureDlssFrameGeneration(
                    kStreamlineViewport, disabledOptions)) {
                std::cerr << streamline.lastError() << '\n';
                return false;
            }
            m_renderingGameFrames = false;
            if (!streamline.releaseDlssFrameGenerationResources(
                    kStreamlineViewport)) {
                std::cerr << streamline.lastError() << '\n';
                return false;
            }
    ...
    bool setRenderingGameFrames(const bool renderingGameFrames) override {
        ...
        if (m_configured) {
            ...
            if (!StreamlineRuntime::instance().configureDlssFrameGeneration(
                    kStreamlineViewport, options)) {
                return false;
            }
            m_options = options;
        }
    ...
    bool prepareFrameGeneration(
        ...
        const auto depth = VkRhiInterop::textureInfo(
            m_device,
            temporalFrame.textures.depth,
            temporalFrame.textures.depthView);
        const auto motionVectors = VkRhiInterop::textureInfo(
            m_device,
            temporalFrame.textures.velocity,
            temporalFrame.textures.velocityView);
        const auto hudlessColor = VkRhiInterop::textureInfo(
            m_device,
            resources.hudlessColorTexture,
            resources.hudlessColorView);
        const auto uiColor = VkRhiInterop::textureInfo(
            m_device,
            resources.uiColorTexture,
            resources.uiColorView);
        ...
        if (!depth.has_value() || !motionVectors.has_value() ||
            ...
            !validTexture(*hudlessColor, VK_FORMAT_B8G8R8A8_UNORM,
                          outputExtent,
                          VK_IMAGE_USAGE_SAMPLED_BIT) ||
            !validTexture(*uiColor, VK_FORMAT_B8G8R8A8_UNORM,
                          outputExtent,
                          VK_IMAGE_USAGE_SAMPLED_BIT)) {
            return false;
        }
        ...
        dispatch.depth = toStreamlineResource(
            *depth, RhiResourceState::ShaderRead);
        dispatch.motionVectors = toStreamlineResource(
            *motionVectors, RhiResourceState::ShaderRead);
        dispatch.hudlessColor = toStreamlineResource(
            *hudlessColor, RhiResourceState::ShaderRead);
        dispatch.uiColorAndAlpha = toStreamlineResource(
            *uiColor, RhiResourceState::ShaderRead);
...
std::unique_ptr<PresentationBackend>
createDlssFrameGenerationPresentationBackend(RhiDevice& rhiDevice) {
    if (rhiDevice.backend() != RhiBackend::Vulkan) {
        return nullptr;
    }
    return std::make_unique<DlssFrameGenerationPresentationBackend>(
        static_cast<VkRhiDevice&>(rhiDevice));
}
