...
[[nodiscard]] std::optional<FfxFsr3UpscalerQualityMode> toFsrQualityMode(const TemporalUpscaleQuality quality) {
    switch (quality) {
    case TemporalUpscaleQuality::Quality: return FFX_FSR3UPSCALER_QUALITY_MODE_QUALITY;
    case TemporalUpscaleQuality::Balanced: return FFX_FSR3UPSCALER_QUALITY_MODE_BALANCED;
    case TemporalUpscaleQuality::Performance: return FFX_FSR3UPSCALER_QUALITY_MODE_PERFORMANCE;
    case TemporalUpscaleQuality::UltraPerformance: return FFX_FSR3UPSCALER_QUALITY_MODE_ULTRA_PERFORMANCE;
...
Fsr31RenderExtentResult queryFsr31RenderExtent(const TemporalUpscaleQuality quality,
    ...
    const FfxErrorCode error = ffxFsr3UpscalerGetRenderResolutionFromQualityMode(
        &renderExtent.width, &renderExtent.height, outputExtent.width, outputExtent.height, *qualityMode);
    if (error != FFX_OK) {
...
Fsr31JitterResult queryFsr31Jitter(const uint64_t frameIndex, const TemporalExtent renderExtent,
                                   const TemporalExtent outputExtent) {
    if (!renderExtent.isValid()) {
    ...
    if (!fitsSdkDimension(renderExtent.width) || !fitsSdkDimension(outputExtent.width)) {
        Fsr31JitterResult result;
        result.status = Fsr31TemporalConfigStatus::DimensionOutOfRange;
        return result;
    }

    const int32_t phaseCount = ffxFsr3UpscalerGetJitterPhaseCount(static_cast<int32_t>(renderExtent.width),
    ...
    const int32_t phaseIndex = static_cast<int32_t>(frameIndex % static_cast<uint64_t>(phaseCount));
    TemporalJitter jitter;
    const FfxErrorCode error =
        ffxFsr3UpscalerGetJitterOffset(&jitter.pixels.x, &jitter.pixels.y, phaseIndex, phaseCount);
    if (error != FFX_OK) {
        Fsr31JitterResult result;
