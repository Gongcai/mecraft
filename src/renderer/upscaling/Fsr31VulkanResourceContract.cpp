...
[[nodiscard]] std::optional<Fsr31ResourceValidationFailure> validateResource(
    ...
    if (resource.image == VK_NULL_HANDLE || resource.view == VK_NULL_HANDLE) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::MissingNativeObject};
    }
    if (resource.imageType != VK_IMAGE_TYPE_2D) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::InvalidImageType};
    }
    if (resource.viewType != VK_IMAGE_VIEW_TYPE_2D) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::InvalidViewType};
    }
    if (resource.format != requirement.format) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::InvalidFormat};
    }
    if (resource.extent.width != requirement.extent.width ||
        resource.extent.height != requirement.extent.height ||
        resource.extent.depth != 1u) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::InvalidExtent};
    }
    if (resource.aspectMask != requirement.aspectMask) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::InvalidAspectMask};
    }
    if (resource.baseMip != 0u || resource.mipCount != 1u ||
        resource.baseLayer != 0u || resource.layerCount != 1u) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::InvalidSubresourceRange};
    }
    if ((resource.usage & VK_IMAGE_USAGE_SAMPLED_BIT) == 0u) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::MissingSampledUsage};
    }
    if (requirement.storage &&
        (resource.usage & VK_IMAGE_USAGE_STORAGE_BIT) == 0u) {
        return Fsr31ResourceValidationFailure{
            requirement.role, Fsr31ResourceValidationError::MissingStorageUsage};
    }
    return std::nullopt;
}
...
Fsr31ResourceResolveResult resolveFsr31VulkanResourceSet(
    ...
    const ResourceLookup lookups[] = {
        {Fsr31ResourceRole::HdrColor, frame.textures.hdrColor,
         frame.textures.hdrColorView, &resources.hdrColor},
        {Fsr31ResourceRole::Depth, frame.textures.depth,
         frame.textures.depthView, &resources.depth},
        {Fsr31ResourceRole::Velocity, frame.textures.velocity,
         frame.textures.velocityView, &resources.velocity},
        {Fsr31ResourceRole::Exposure, frame.textures.exposure,
         frame.textures.exposureView, &resources.exposure},
        {Fsr31ResourceRole::ReactiveMask, frame.textures.reactiveMask,
         frame.textures.reactiveMaskView, &resources.reactiveMask},
        {Fsr31ResourceRole::TransparencyMask, frame.textures.transparencyMask,
         frame.textures.transparencyMaskView, &resources.transparencyMask},
        {Fsr31ResourceRole::OutputHdrColor, frame.textures.outputHdrColor,
         frame.textures.outputHdrColorView, &resources.outputHdrColor}
    };
    for (const ResourceLookup& lookup : lookups) {
        const auto resource = VkRhiInterop::textureInfo(
            device, lookup.texture, lookup.view);
