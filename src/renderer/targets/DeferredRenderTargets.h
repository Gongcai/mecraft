...
public:
    ...
    /// Returns the current NRD previous-viewport coverage diagnostic.
    /// Guide preparation writes the current slot after reconstructing the
    /// current surface in the previous camera.
    [[nodiscard]] RhiTextureHandle nrdReprojectionCoverageTextureHandle() const {
        return m_nrdReprojectionCoverageHandle[m_currentHistoryIndex];
    }
    [[nodiscard]] RhiTextureViewHandle nrdReprojectionCoverageTextureViewHandle() const {
        return m_nrdReprojectionCoverageView[m_currentHistoryIndex];
    }
    bool ensureNrdReprojectionCoverageTextureViews(RhiDevice& rhiDevice);
...
private:
    ...
    RhiTextureHandle m_nrdReprojectionCoverageHandle[2];
    ...
    RhiTextureViewHandle m_nrdReprojectionCoverageView[2];
