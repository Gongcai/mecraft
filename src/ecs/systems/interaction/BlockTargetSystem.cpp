...
void BlockTargetSystem::update(SystemContext& ctx) {
    ...
    auto view = registry.view<LocalPlayerTag, BlockActionIntentComponent, TransformComponent, CameraStateComponent,
    ...
    for (auto e : view) {
        ...
        const PhysicsInfo pickRay = {transform.position + glm::vec3(0.0f, transform.eyeHeight, 0.0f), camera.front};
        const RayHit hit = raycastWorldView(worldView, pickRay, kPickDistance);
        target.hasTarget = hit.hit && hit.kind == RayHitKind::Block;
        target.targetState = target.hasTarget ? worldView.getBlockState(hit.blockPos.x, hit.blockPos.y, hit.blockPos.z)
                                              : NULL_BLOCK_STATE;
        ...
        target.fluidTargetState =
            target.hasFluidTarget
