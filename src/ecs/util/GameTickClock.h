...
public:
    ...
    explicit GameTickClock(double tickRate = DEFAULT_TICK_RATE) : m_tickInterval(1.0 / tickRate) {}

    void advance(double deltaTime) { m_accumulator += deltaTime; }

    bool shouldTick() const { return m_accumulator >= m_tickInterval; }
