...
inline uint8_t blockTintKindFromBiomeTint(const BiomeTintKind tintKind) {
    switch (tintKind) {
    case BiomeTintKind::Grass: return BlockTintKinds::GRASS;
    case BiomeTintKind::Foliage: return BlockTintKinds::FOLIAGE;
    case BiomeTintKind::None:
    default: return BlockTintKinds::NONE;
    }
}
...
inline BlockVertex makeBlockVertex(float x, float y, float z, float u, float v, float normal, float sunlight,
                                   ...
                                   uint8_t tintKind = BlockTintKinds::NONE, uint8_t tintU = 0, uint8_t tintV = 0,
                                   uint8_t derivativeMaterialId = DerivativeMaterialIds::DEFAULT) {
    return {x,
            y,
            z,
...
public:
    ...
    [[nodiscard]] BlockStateId getBlockUnchecked(int x, int y, int z) const {
        const std::size_t index = static_cast<std::size_t>(x) + static_cast<std::size_t>(z) * SIZE +
