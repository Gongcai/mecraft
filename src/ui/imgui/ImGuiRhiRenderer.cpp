...
bool ImGuiRhiRenderer::init(const Window& window, RhiDevice& rhiDevice, const bool dockingEnabled,
                            std::string iniFile) {
    shutdown();
    IMGUI_CHECKVERSION();
...
bool ImGuiRhiRenderer::beginFrame(const int framebufferWidth, const int framebufferHeight) {
    if (m_context == nullptr || m_rhiDevice == nullptr || framebufferWidth <= 0 || framebufferHeight <= 0 ||
        m_frameStarted) {
        return false;
    }
    ImGui::SetCurrentContext(m_context);
...
bool ImGuiRhiRenderer::prepareDrawData(RhiCommandList& commandList) {
    ...
    if (drawData == nullptr || !buildPreparedDraws(*drawData) || !uploadDrawBuffers(commandList)) {
        return false;
    }
...
ImTextureID ImGuiRhiRenderer::registerTexture(const RhiTextureViewHandle textureView, const RhiSamplerHandle sampler) {
...
void ImGuiRhiRenderer::unregisterTexture(const ImTextureID textureId) {
    const auto it =
        std::find_if(m_textureBindings.begin(), m_textureBindings.end(),
                     [textureId](const TextureBinding& binding) { return binding.id == textureId && !binding.font; });
...
bool ImGuiRhiRenderer::createRhiResources() {
    ...
    if (!m_fontTexture.isValid() || !m_fontTextureView.isValid() || !m_fontSampler.isValid() ||
        ...
        !m_pipelineLayout.isValid() || !m_pipeline.isValid() || !m_vertexBuffer.isValid() || !m_indexBuffer.isValid()) {
        return false;
    }
...
RhiBindGroupHandle ImGuiRhiRenderer::findBindGroup(const ImTextureID textureId) const {
    const auto it = std::find_if(m_textureBindings.begin(), m_textureBindings.end(),
                                 [textureId](const TextureBinding& binding) { return binding.id == textureId; });
...
bool ImGuiRhiRenderer::buildPreparedDraws(const ImDrawData& drawData) {
    ...
    if (drawData.TotalVtxCount < 0 || drawData.TotalIdxCount < 0 || m_displaySize.x <= 0.0f ||
        m_displaySize.y <= 0.0f) {
        return false;
    }
    ...
    for (const ImDrawList* drawList : drawData.CmdLists) {
        ...
        for (const ImDrawCmd& command : drawList->CmdBuffer) {
            ...
            if (!bindGroup.isValid() || command.IdxOffset > static_cast<unsigned>(drawList->IdxBuffer.Size) ||
                command.ElemCount > static_cast<unsigned>(drawList->IdxBuffer.Size) - command.IdxOffset ||
                command.VtxOffset > static_cast<unsigned>(drawList->VtxBuffer.Size)) {
                return false;
            }
            const float clipMinX = (command.ClipRect.x - drawData.DisplayPos.x) * drawData.FramebufferScale.x;
            const float clipMinY = (command.ClipRect.y - drawData.DisplayPos.y) * drawData.FramebufferScale.y;
            const float clipMaxX = (command.ClipRect.z - drawData.DisplayPos.x) * drawData.FramebufferScale.x;
            const float clipMaxY = (command.ClipRect.w - drawData.DisplayPos.y) * drawData.FramebufferScale.y;
            if (!std::isfinite(clipMinX) || !std::isfinite(clipMinY) || !std::isfinite(clipMaxX) ||
                !std::isfinite(clipMaxY)) {
                return false;
            }
            const float minX = std::clamp(clipMinX, 0.0f, static_cast<float>(m_framebufferWidth));
            const float minY = std::clamp(clipMinY, 0.0f, static_cast<float>(m_framebufferHeight));
            const float maxX = std::clamp(clipMaxX, 0.0f, static_cast<float>(m_framebufferWidth));
            const float maxY = std::clamp(clipMaxY, 0.0f, static_cast<float>(m_framebufferHeight));
            ...
            PreparedDraw draw;
            draw.scissor = {scissorMinX, m_framebufferHeight - scissorMaxY,
...
bool ImGuiRhiRenderer::uploadDrawBuffers(RhiCommandList& commandList) {
    ...
    const auto ensureCapacity = [this](RhiBufferHandle& buffer, uint64_t& capacity, RhiResourceState& state,
...
void ImGuiRhiRenderer::bindRenderState(RhiCommandList& commandList) const {
    ...
    commandList.setIndexBuffer(
        m_indexBuffer, sizeof(ImDrawIdx) == sizeof(uint16_t) ? RhiIndexFormat::Uint16 : RhiIndexFormat::Uint32, 0u);
