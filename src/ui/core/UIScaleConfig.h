...
struct UIScaleConfig {
    ...
    [[nodiscard]] float getScaleForStrategy(UIScaleStrategy strategy) const {
        switch (strategy) {
            case UIScaleStrategy::None:
                return 1.0f;
            case UIScaleStrategy::Uniform:
                return effectiveScale;
            case UIScaleStrategy::TextOnly:
                return autoScale; // Ignore user GUI scale preference
            case UIScaleStrategy::PixelPerfect:
                return std::floor(effectiveScale); // Snap to integer
        }
        return effectiveScale;
    }
    ...
    [[nodiscard]] static float computeAutoScale(float actualW, float actualH) {
        ...
        if (minDim >= 2160.0f) return 3.0f;  // 4K+
        if (minDim >= 1440.0f) return 2.0f;  // 2K/1440p
        if (minDim >= 720.0f)  return 1.0f;  // 1080p/720p
    ...
    [[nodiscard]] static float getGUIScaleMultiplier(GUIScale scale, float autoScaleValue) {
        switch (scale) {
            ...
            case GUIScale::Small:
                return 0.5f;
            case GUIScale::Normal:
                return 1.0f;
            case GUIScale::Large:
                return 2.0f;
            case GUIScale::ExtraLarge:
                return 3.0f;
        }
        return 1.0f;
    }
