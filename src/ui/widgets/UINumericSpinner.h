...
private:
  ...
  [[nodiscard]] int hitTestZone(float px, float py,
                                const UIRenderContext &ctx) const;
  [[nodiscard]] UINumericSpinnerStyle
  resolveBaseStyle(const UIRenderContext &ctx) const;
  [[nodiscard]] UIResolvedNumericSpinnerStyle
  resolveStyle(const UIRenderContext &ctx) const;
