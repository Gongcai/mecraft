...
void UIPanel::renderSelf(const UIRenderContext& context) const {
    ...
    const UIResolvedStyle resolved = UIStyleResolver::resolve(
        resolveBaseStyle(context), UIStyleState_Normal);
    const bool useGlass = context.panelGlassPipeline.isValid() &&
                          context.panelGlassBindGroup.isValid() &&
                          context.backdropBlurView.isValid() &&
    ...
    const float borderWidth = std::clamp(resolved.borderWidth, 0.0f,
                                         std::min(panelWidth, panelHeight) * 0.5f);
    if (borderWidth > 0.0f) {
        std::array<float, 4> border = resolved.border;
        border[3] *= alpha;
        drawSolidRect(panelX, panelY + panelHeight - borderWidth,
                      panelWidth, borderWidth, border);
        drawSolidRect(panelX, panelY, panelWidth, borderWidth, border);
        drawSolidRect(panelX, panelY + borderWidth, borderWidth,
                      panelHeight - borderWidth * 2.0f, border);
        drawSolidRect(panelX + panelWidth - borderWidth, panelY + borderWidth,
                      borderWidth, panelHeight - borderWidth * 2.0f, border);
    }
    ...
    if (useGlass) {
        ...
        drawSolidRect(panelX + lineWidth,
                      panelY + panelHeight - lineWidth - 1.0f,
                      lineSpan, 1.0f, topHighlight);

        const Color bottomShade {0.0f, 0.0f, 0.0f, 0.18f * alpha};
        drawSolidRect(panelX + lineWidth, panelY + lineWidth,
                      lineSpan, 1.0f, bottomShade);
    }
}
