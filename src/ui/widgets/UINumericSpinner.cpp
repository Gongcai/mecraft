...
void UINumericSpinner::init(GameResources& resources, RhiDevice& rhiDevice) {
    ...
    ld.pushConstantStages = rhiFlag(RhiShaderStage::Vertex) | rhiFlag(RhiShaderStage::Fragment);
    ...
    pd.vertexInput.bindings.push_back({0, sizeof(float) * 2, RhiVertexInputRate::Vertex});
...
void UINumericSpinner::initMesh() {
    ...
    d.usage = rhiFlag(RhiBufferUsage::Vertex) | rhiFlag(RhiBufferUsage::TransferDst);
...
void UINumericSpinner::renderSelf(const UIRenderContext& ctx) const {
    ...
    if (record) {
        ...
        zone(minusX, resolved.buttonWidth, resolved.minusBackground, resolved.minusBorder);
        zone(valueX, valueW, resolved.valueBackground, resolved.valueBorder);
        zone(plusX, resolved.buttonWidth, resolved.plusBackground, resolved.plusBorder);
        if (m_editing && m_cursorVisible && isFocused()) {
            ...
            rect(cursorX, ay + resolved.cursorInset, resolved.cursorWidth, ah - 2 * resolved.cursorInset,
                 resolved.cursor);
        }
    }
    ...
    if (ctx.textRenderer) {
        ...
        // Minus sign.
        {
            ...
            ctx.textRenderer->draw(ctx, "-", minusX + (resolved.buttonWidth - m.width) * 0.5f,
        ...
        {
            const auto m = ctx.textRenderer->measureText("+", textScale);
            ctx.textRenderer->draw(ctx, "+", plusX + (resolved.buttonWidth - m.width) * 0.5f,
                                   ay + (ah - m.height) * 0.5f, textScale,
                                   {resolved.text[0], resolved.text[1], resolved.text[2], resolved.text[3] * alpha});
        }
        // Value text.
        {
            ...
            ctx.textRenderer->draw(textContext, valStr.empty() ? " " : valStr, valueX + resolved.textPadding,
                                   ay + (ah - m.height) * 0.5f, textScale,
                                   {resolved.text[0], resolved.text[1], resolved.text[2], resolved.text[3] * alpha});
