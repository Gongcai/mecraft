...
void SaveListScreen::buildUI(ResourceMgr& resourceMgr) {
    ...
    title->setText(getLocaleManager() ? getLocaleManager()->tr("select_world") : "SELECT WORLD");
    ...
    emptyText->setText(getLocaleManager() ? getLocaleManager()->tr("no_saves") : "No saves found. Create a new world!");
    ...
    createBtn->setText(getLocaleManager() ? getLocaleManager()->tr("create_new_world") : "CREATE NEW WORLD");
...
void SaveListScreen::refreshSaveList() {
        ...
        if (exists && isDir) {
            for (auto& entry : std::filesystem::directory_iterator(m_savesRoot, ec)) {
                ...
                if (sm.loadLevelMeta(meta)) {
                    save.displayName = meta.displayName.empty() ? save.folderName : meta.displayName;
    ...
    std::sort(m_saves.begin(), m_saves.end(),
              [](const SaveEntry& a, const SaveEntry& b) { return a.lastPlayedUtc > b.lastPlayedUtc; });
...
void SaveListScreen::rebuildList() {
    ...
    if (m_emptyText)
        m_emptyText->visible = false;

    const float entryWidth = m_saveListStack->width;
    const float contentHeight =
        static_cast<float>(m_saves.size()) * kEntryHeight + static_cast<float>(m_saves.size() - 1) * kEntrySpacing;
    ...
    for (std::size_t i = 0; i < m_saves.size(); ++i) {
        ...
        panel->x = 0.0f;
        panel->y = contentHeight - kEntryHeight - static_cast<float>(i) * (kEntryHeight + kEntrySpacing);
