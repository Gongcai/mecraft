...
void UIGridLayout::layout() {
    ...
    width = static_cast<float>(m_columns) * m_cellWidth + static_cast<float>(m_columns - 1) * m_spacing;
    height = static_cast<float>(totalRows) * m_cellHeight + static_cast<float>(totalRows - 1) * m_spacing;
}
