...
[[nodiscard]] uint64_t grownCapacity(const uint64_t current, const uint64_t required) {
...
[[nodiscard]] const char* presentationModeName(const PresentationMode mode) {
    ...
    case PresentationMode::Fsr31FrameGeneration: return "FSR 3.1 Frame Generation";
    case PresentationMode::DlssFrameGeneration: return "DLSS Frame Generation";
    }
    std::abort();
}
} // namespace
...
bool Dashboard::prepareFrame(RhiCommandList& commandList, const int framebufferWidth, const int framebufferHeight,
                             ecs::GameplayRegistry& registry, World& world, Camera& camera, RenderResourceHub& render,
                             RenderScene& renderScene, PostProcessPass& postProcess, UIRenderer& uiRenderer,
                             FrameProfilerStats& profilerStats, const std::function<void(int)>& renderDistanceSetter) {
    ...
    ImGuiIO& io = ImGui::GetIO();
    if (!std::isfinite(io.DisplayFramebufferScale.x) || !std::isfinite(io.DisplayFramebufferScale.y) ||
        io.DisplayFramebufferScale.x <= 0.0f || io.DisplayFramebufferScale.y <= 0.0f) {
        return false;
    }
    io.DisplaySize = {static_cast<float>(framebufferWidth) / io.DisplayFramebufferScale.x,
    ...
    if (drawData == nullptr || !buildPreparedDraws(*drawData, framebufferWidth, framebufferHeight)) {
        return false;
    }
    if (m_fontTextureState != RhiResourceState::ShaderRead) {
        commandList.textureBarrier({m_fontTexture, m_fontTextureState, RhiResourceState::ShaderRead});
...
bool Dashboard::createRhiResources(RhiDevice& rhiDevice) {
    ...
    if (!checkedMultiply(static_cast<uint64_t>(fontWidth), static_cast<uint64_t>(fontHeight), fontByteSize) ||
        !checkedMultiply(fontByteSize, static_cast<uint64_t>(bytesPerPixel), fontByteSize) ||
    ...
    const std::optional<std::string> vertexSource = renderer::rhi::loadShaderSource("assets/shaders/imgui_rhi.vert");
    const std::optional<std::string> fragmentSource = renderer::rhi::loadShaderSource("assets/shaders/imgui_rhi.frag");
    ...
    pipelineDesc.vertexInput.bindings.push_back({0u, sizeof(ImDrawVert), RhiVertexInputRate::Vertex});
    ...
    if (!m_fontTexture.isValid() || !m_fontTextureView.isValid() || !m_fontSampler.isValid() ||
        !m_vertexShader.isValid() || !m_fragmentShader.isValid() || !m_bindGroupLayout.isValid() ||
        !m_pipelineLayout.isValid() || !m_pipeline.isValid() || !m_fontBindGroup.isValid() ||
...
void Dashboard::destroyRhiResources() {
    ...
    if (m_rhiDevice != nullptr) {
        if (m_indexBuffer.isValid())
            m_rhiDevice->destroyBuffer(m_indexBuffer);
        if (m_vertexBuffer.isValid())
            m_rhiDevice->destroyBuffer(m_vertexBuffer);
        if (m_fontBindGroup.isValid())
            m_rhiDevice->destroyBindGroup(m_fontBindGroup);
        ...
        if (m_fontSampler.isValid())
            m_rhiDevice->destroySampler(m_fontSampler);
        if (m_fontTextureView.isValid())
            m_rhiDevice->destroyTextureView(m_fontTextureView);
        if (m_fontTexture.isValid())
            m_rhiDevice->destroyTexture(m_fontTexture);
    }
    m_rhiDevice = nullptr;
    m_fontTexture = {};
    m_fontTextureView = {};
...
bool Dashboard::buildPreparedDraws(const ImDrawData& drawData, const int framebufferWidth,
    ...
    if (!checkedMultiply(static_cast<uint64_t>(drawData.TotalIdxCount), sizeof(ImDrawIdx), totalIndexBytes) ||
    ...
    for (const ImDrawList* drawList : drawData.CmdLists) {
        ...
        const uint64_t drawListIndexBytes = static_cast<uint64_t>(drawList->IdxBuffer.Size) * sizeof(ImDrawIdx);
        if (drawListIndexBytes != 0u) {
            std::memcpy(m_indexBytes.data() + indexByteOffset, drawList->IdxBuffer.Data,
        ...
        for (const ImDrawCmd& drawCommand : drawList->CmdBuffer) {
            ...
            if (drawCommand.IdxOffset > static_cast<unsigned int>(drawList->IdxBuffer.Size) ||
                drawCommand.ElemCount > static_cast<unsigned int>(drawList->IdxBuffer.Size) - drawCommand.IdxOffset ||
            ...
            for (unsigned int i = 0u; i < drawCommand.ElemCount; ++i) {
                const uint64_t referencedVertex =
                    static_cast<uint64_t>(drawCommand.VtxOffset) + drawList->IdxBuffer[drawCommand.IdxOffset + i];
                if (referencedVertex >= static_cast<uint64_t>(drawList->VtxBuffer.Size)) {
                    return false;
                }
            }

            const float clipMinX = (drawCommand.ClipRect.x - drawData.DisplayPos.x) * drawData.FramebufferScale.x;
            const float clipMinY = (drawCommand.ClipRect.y - drawData.DisplayPos.y) * drawData.FramebufferScale.y;
            const float clipMaxX = (drawCommand.ClipRect.z - drawData.DisplayPos.x) * drawData.FramebufferScale.x;
            const float clipMaxY = (drawCommand.ClipRect.w - drawData.DisplayPos.y) * drawData.FramebufferScale.y;
            if (!std::isfinite(clipMinX) || !std::isfinite(clipMinY) || !std::isfinite(clipMaxX) ||
                !std::isfinite(clipMaxY)) {
                return false;
            }
    ...
    return globalVertexOffset == static_cast<uint64_t>(drawData.TotalVtxCount) &&
           globalIndexOffset == static_cast<uint64_t>(drawData.TotalIdxCount) && indexByteOffset == totalIndexBytes;
}
...
bool Dashboard::uploadDrawBuffers(RhiCommandList& commandList) {
    ...
    if (!checkedMultiply(m_vertices.size(), sizeof(ImDrawVert), vertexBytes) || vertexBytes == 0u ||
        m_indexBytes.empty()) {
        return false;
    }

    auto ensureCapacity = [this](RhiBufferHandle& buffer, uint64_t& capacity, RhiResourceState& state,
                                 const uint64_t required, const RhiBufferUsage usage, const char* debugName) {
        ...
        const RhiResourceState functionalState =
            usage == RhiBufferUsage::Vertex ? RhiResourceState::VertexBuffer : RhiResourceState::IndexBuffer;
    ...
    if (!ensureCapacity(m_vertexBuffer, m_vertexBufferCapacity, m_vertexBufferState, vertexBytes,
                        RhiBufferUsage::Vertex, "Dashboard.Vertices") ||
        !ensureCapacity(m_indexBuffer, m_indexBufferCapacity, m_indexBufferState, m_indexBytes.size(),
                        RhiBufferUsage::Index, "Dashboard.Indices")) {
        return false;
    }

    commandList.bufferBarrier({m_vertexBuffer, m_vertexBufferState, RhiResourceState::TransferDst});
    commandList.updateBuffer(m_vertexBuffer, 0u, m_vertices.data(), static_cast<size_t>(vertexBytes));
    commandList.bufferBarrier({m_vertexBuffer, RhiResourceState::TransferDst, RhiResourceState::VertexBuffer});
    m_vertexBufferState = RhiResourceState::VertexBuffer;

    commandList.bufferBarrier({m_indexBuffer, m_indexBufferState, RhiResourceState::TransferDst});
    commandList.updateBuffer(m_indexBuffer, 0u, m_indexBytes.data(), m_indexBytes.size());
    commandList.bufferBarrier({m_indexBuffer, RhiResourceState::TransferDst, RhiResourceState::IndexBuffer});
    m_indexBufferState = RhiResourceState::IndexBuffer;
    return true;
}
...
void Dashboard::bindRenderState(RhiCommandList& commandList) const {
    ...
    commandList.setViewport(
        {0.0f, 0.0f, static_cast<float>(m_framebufferWidth), static_cast<float>(m_framebufferHeight), 0.0f, 1.0f});
    ...
    const RhiIndexFormat indexFormat =
        sizeof(ImDrawIdx) == sizeof(uint16_t) ? RhiIndexFormat::Uint16 : RhiIndexFormat::Uint32;
...
void Dashboard::recordDraws(RhiCommandList& commandList) const {
    ...
    for (const PreparedDraw& draw : m_preparedDraws) {
        ...
        commandList.drawIndexed(draw.indexCount, 1u, draw.firstIndex, draw.vertexOffset, 0u);
    }
}
    ...
    if (ImGui::CollapsingHeader("Player Stats")) {
        ...
        for (auto e : view) {
            ...
            for (int i = 0; i < kCurveSamples; ++i) {
                ...
                horizontalCurve[static_cast<size_t>(i)] =
                    horizontalBobAmplitude * static_cast<float>(std::cos(phase + phaseOffset));
            }
            ...
            ImGui::PlotLines("Vertical Bob Curve", verticalCurve.data(), kCurveSamples, 0, nullptr, 0.0f,
                             kMaxVerticalPreview, ImVec2(0.0f, 90.0f));
...
void Dashboard::showPerformanceStats(World& world, RenderResourceHub& render, RenderScene& renderScene,
    ...
    if (ImGui::CollapsingHeader("Performance Stats")) {
        ...
        ImGui::Text("Presentation Mode: %s", presentationModeName(displayedProfilerStats.presentationMode));
        ...
        {
            const RenderScene::PresentationDebugInfo presentationInfo = renderScene.presentationDebugInfo();
            ...
            ImGui::Text("Render %ux%u  Output %ux%u  Swapchain Present: %s", presentationInfo.renderWidth,
                        presentationInfo.renderHeight, presentationInfo.outputWidth, presentationInfo.outputHeight,
        ...
        if (ImGui::TreeNode("Renderer Capability Contract")) {
            ...
            const renderer::contracts::RenderProfile profile = renderer::contracts::activeRenderProfile(backend);
            const renderer::contracts::RenderProfileStatus profileStatus =
                renderer::contracts::evaluateCurrentRenderProfile(profile, backend, m_rhiDevice->capabilities());
            ImGui::Text("Backend: %s", renderer::rhi::rhiBackendDisplayName(backend));
            ImGui::Text("Profile: %s (%s)", renderer::contracts::renderProfileDisplayName(profile),
                        renderer::contracts::renderProfileStableId(profile));
            ImGui::Text("Profile Status: %s [%s]", profileStatus.available() ? "Available" : "Unavailable",
                        renderer::contracts::renderFeatureStatusCodeStableId(profileStatus.code));
            if (!profileStatus.available()) {
                if (profileStatus.blockingFeature != renderer::contracts::RenderFeature::Count) {
                    ImGui::Text("Blocking Feature: %s (%s)",
                                renderer::contracts::renderFeatureDisplayName(profileStatus.blockingFeature),
                                renderer::contracts::renderFeatureStableId(profileStatus.blockingFeature));
                }
                ImGui::TextWrapped("Reason: %s", profileStatus.reason);
            }
            ImGui::Separator();
            for (size_t index = 0u; index < renderer::contracts::renderFeatureCount(); ++index) {
                const auto feature = static_cast<renderer::contracts::RenderFeature>(index);
                const renderer::contracts::RenderFeatureStatus featureStatus =
                    renderer::contracts::evaluateCurrentRenderFeature(profile, backend, m_rhiDevice->capabilities(),
                                                                      feature);
                ImGui::Text("%s: %s [%s]", renderer::contracts::renderFeatureDisplayName(feature),
                            featureStatus.available() ? "Available" : "Unavailable",
                            renderer::contracts::renderFeatureStatusCodeStableId(featureStatus.code));
        ...
        if (ImGui::TreeNode("RHI Memory")) {
            if (!m_displayRhiMemoryStats.valid) {
                ...
                ImGui::Text("Accuracy: %s", rhiMemoryStatsAccuracyStableId(m_displayRhiMemoryStats.accuracy));
                ImGui::Text("Tracked Total: %.2f MiB (%llu allocations, %llu resources)",
                            static_cast<double>(m_displayRhiMemoryStats.totalBytes) / kBytesPerMiB,
                            static_cast<unsigned long long>(m_displayRhiMemoryStats.totalAllocationCount),
                            static_cast<unsigned long long>(m_displayRhiMemoryStats.totalResourceCount));
                ImGui::Separator();
                for (size_t index = 0u; index < kRhiMemoryCategoryCount; ++index) {
                    const auto category = static_cast<RhiMemoryCategory>(index);
                    const RhiMemoryCategoryStats& entry = m_displayRhiMemoryStats.categories[index];
                    const double mebibytes = static_cast<double>(entry.bytes) / kBytesPerMiB;
                    const bool unclassifiedNonZero =
                        entry.bytes != 0u || entry.allocationCount != 0u || entry.resourceCount != 0u;
                    if (category == RhiMemoryCategory::Unclassified && unclassifiedNonZero) {
                        ...
                        ImGui::Text("%s: %.2f MiB (%llu allocations, %llu resources)",
                                    rhiMemoryCategoryDisplayName(category), mebibytes,
                                    static_cast<unsigned long long>(entry.allocationCount),
                                    static_cast<unsigned long long>(entry.resourceCount));
                    }
        ...
        showMax("App Update Dispatch", displayedProfilerStats.appUpdateDispatchMs,
                displayedProfilerStats.maxAppUpdateDispatchMs);
        showMax("App Render Dispatch", displayedProfilerStats.appRenderDispatchMs,
                displayedProfilerStats.maxAppRenderDispatchMs);
        showMax("  - Presentation Acquire", displayedProfilerStats.presentationAcquireMs,
                displayedProfilerStats.maxPresentationAcquireMs);
        showMax("  - Dispatch Other", displayedProfilerStats.renderDispatchOtherMs,
                displayedProfilerStats.maxRenderDispatchOtherMs);
        ...
        showMax("  - State Update", displayedProfilerStats.fixedStateUpdateMs,
                displayedProfilerStats.maxFixedStateUpdateMs);
        showMax("  - Particle Update", displayedProfilerStats.fixedParticleUpdateMs,
                displayedProfilerStats.maxFixedParticleUpdateMs);
        showMax("  - Drop Update", displayedProfilerStats.fixedDropUpdateMs,
                displayedProfilerStats.maxFixedDropUpdateMs);
        showMax("  - World Update", displayedProfilerStats.fixedWorldUpdateMs,
                displayedProfilerStats.maxFixedWorldUpdateMs);
        ...
        if (displayedProfilerStats.frameHistoryCount > 1) {
            ...
            ImGui::PlotLines(
                "FPS", displayedProfilerStats.fpsHistory.data(),
                static_cast<int>(displayedProfilerStats.frameHistoryCount), 0, nullptr, 0.0f,
                historyMax(displayedProfilerStats.fpsHistory.data(), displayedProfilerStats.frameHistoryCount, 1.0f),
                ImVec2(0.0f, 65.0f));
        }

        if (displayedProfilerStats.fixedHistoryCount > 1) {
            ImGui::PlotLines(
                "Render Submit", displayedProfilerStats.renderHistory.data(),
                static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                historyMax(displayedProfilerStats.renderHistory.data(), displayedProfilerStats.fixedHistoryCount, 0.1f),
                ImVec2(0.0f, 55.0f));
        }
        ...
        if (displayedProfilerStats.fixedHistoryCount > 1) {
            ...
            ImGui::PlotLines("Fixed Total", displayedProfilerStats.fixedUpdateHistory.data(),
                             static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                             historyMax(displayedProfilerStats.fixedUpdateHistory.data(),
                                        displayedProfilerStats.fixedHistoryCount, 0.1f),
                             ImVec2(0.0f, 65.0f));
            ImGui::PlotLines("Input", displayedProfilerStats.fixedInputHistory.data(),
                             static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                             historyMax(displayedProfilerStats.fixedInputHistory.data(),
                                        displayedProfilerStats.fixedHistoryCount, 0.1f),
                             ImVec2(0.0f, 55.0f));
            ImGui::PlotLines("State", displayedProfilerStats.fixedStateHistory.data(),
                             static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                             historyMax(displayedProfilerStats.fixedStateHistory.data(),
                                        displayedProfilerStats.fixedHistoryCount, 0.1f),
                             ImVec2(0.0f, 55.0f));
            ImGui::PlotLines("Particle", displayedProfilerStats.fixedParticleHistory.data(),
                             static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                             historyMax(displayedProfilerStats.fixedParticleHistory.data(),
                                        displayedProfilerStats.fixedHistoryCount, 0.1f),
                             ImVec2(0.0f, 55.0f));
            ImGui::PlotLines("Drop", displayedProfilerStats.fixedDropHistory.data(),
                             static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                             historyMax(displayedProfilerStats.fixedDropHistory.data(),
                                        displayedProfilerStats.fixedHistoryCount, 0.1f),
                             ImVec2(0.0f, 55.0f));
            ImGui::PlotLines("World", displayedProfilerStats.fixedWorldHistory.data(),
                             static_cast<int>(displayedProfilerStats.fixedHistoryCount), 0, nullptr, 0.0f,
                             historyMax(displayedProfilerStats.fixedWorldHistory.data(),
                                        displayedProfilerStats.fixedHistoryCount, 0.1f),
                             ImVec2(0.0f, 55.0f));
        }
        ...
        if (ImGui::TreeNode("Reflection Probe Capture Queue")) {
            const ReflectionProbeCaptureFrameStats& capture = m_displayReflectionProbeCaptureStats;
            ImGui::Text("Sources: %u  Active: %u  Building: %u", capture.sourceCount, capture.activeProbeCount,
                        capture.buildingProbeCount);
            ImGui::Text("Pending work: %u  Work per probe: %u  Slots: %u", capture.pendingWorkItemCount,
                        renderer::contracts::kReflectionProbeCaptureWorkItemCount, capture.slotCapacity);
            if (capture.currentProbeId.isValid()) {
                ImGui::Text("Probe %u  Work item: %u  Active revision: %u  Build revision: %u",
                            capture.currentProbeId.value, capture.currentWorkItem, capture.activeRevision,
                            capture.buildRevision);
                const bool activePublished =
                    capture.activeCubemapIndex != renderer::contracts::kReflectionProbeInvalidCubemapIndex;
                if (activePublished) {
                    ImGui::Text("Cubemap active/build: %u / %u%s", capture.activeCubemapIndex,
                                capture.buildCubemapIndex, capture.workScheduled ? " (scheduled)" : "");
                } else {
                    ImGui::Text("Cubemap active: unpublished  Build: %u%s", capture.buildCubemapIndex,
        ...
        if (gpuWindow.valid && ImGui::TreeNode("Render Graph Stage Percentiles")) {
            ImGui::Text("Latest window: %zu / %zu samples  Observed: %llu", gpuWindow.sampleCount, gpuWindow.capacity,
                        static_cast<unsigned long long>(gpuWindow.observedSampleCount));
            if (ImGui::BeginTable("GpuStagePercentileTable", 4,
                                  ImGuiTableFlags_RowBg | ImGuiTableFlags_BordersInnerV |
                                      ImGuiTableFlags_SizingStretchProp)) {
                ImGui::TableSetupColumn("Stage");
                ImGui::TableSetupColumn("p50 ms", ImGuiTableColumnFlags_WidthFixed, 70.0f);
                ImGui::TableSetupColumn("p95 ms", ImGuiTableColumnFlags_WidthFixed, 70.0f);
                ImGui::TableSetupColumn("p99 ms", ImGuiTableColumnFlags_WidthFixed, 70.0f);
                ImGui::TableHeadersRow();
                const auto showTimingRow = [](const char* name, const GpuTimingPercentiles& timing) {
                    ImGui::TableNextRow();
        ...
        if (shadowStats.supported && shadowStats.valid) {
            ImGui::Text("CSM GPU: %.3f ms  res=%d  submitted=%d  culled=%d  maxDist=%.1f", shadowStats.gpuTotalMs,
                        shadowStats.shadowResolution, shadowStats.submitted, shadowStats.culled,
            ...
            for (int cascade = 0; cascade < shadowStats.cascadeCount; ++cascade) {
                ...
                ImGui::Text("  C%d: %.3f ms (opaque %.3f / trans %.3f) split %.1f-%.1f texel %.4f radius %.1f", cascade,
                            s.gpuTotalMs, s.gpuOpaqueMs, s.gpuTransparentMs, s.splitNear, s.splitFar, s.texelWorldSize,
                            s.radius);
                ImGui::Text("      box %d/%d  dist %d/%d  entries c/t %d/%d  cmds o/c/t %zu/%zu/%zu", s.boxVisible,
                            s.boxCulled, s.distanceVisible, s.distanceCulled, s.cutoutEntries, s.transparentEntries,
                            s.opaqueCommands, s.cutoutCommands, s.transparentCommands);
            }
        ...
        if (graphStats.valid && ImGui::TreeNode("Render Graph")) {
            ImGui::Text("CPU Build: %.3f ms  Compile: %.3f ms  Record: %.3f ms  Submit: %.3f ms", graphStats.cpuBuildMs,
            ...
            if (graphStats.aliasedTextureCount > 0u) {
                ...
                ImGui::Text("Alias: %u tex  %u pages  %.1f -> %.1f MB", graphStats.aliasedTextureCount,
                            graphStats.aliasPageCount,
                            static_cast<double>(graphStats.aliasedRequestBytes) / kBytesPerMegabyte,
                            static_cast<double>(graphStats.aliasTotalPageBytes) / kBytesPerMegabyte);
            }
            if (m_displayClusteredLightingStats.valid) {
                const RenderScene::ClusteredLightingDebugInfo& clustered = m_displayClusteredLightingStats;
                ImGui::Text("Clusters: %ux%ux%u = %u  Lights: %u / %u active  Non-empty: %u", clustered.tileCountX,
                            clustered.tileCountY, clustered.depthSliceCount, clustered.clusterCount,
                            clustered.lightCount, clustered.activeLightCount, clustered.nonEmptyClusterCount);
                ImGui::Text("Cluster Indices: %u / %u  Avg: %.2f  Max: %u  Error: 0x%08x", clustered.totalIndexCount,
            ...
            if (graphStats.passes.empty()) {
                ...
                ImGui::Text("GPU Total: %.3f ms  Span: %.3f ms  Idle: %.3f ms", graphStats.gpuTotalMs,
                            graphStats.gpuSpanMs, graphStats.gpuIdleMs);
                ...
                if (ImGui::BeginTable("RenderGraphPasses", 4,
                    ...
                    ImGui::TableSetupColumn("Queue", ImGuiTableColumnFlags_WidthFixed, 80.0f);
                    ImGui::TableSetupColumn("GPU ms", ImGuiTableColumnFlags_WidthFixed, 70.0f);
                    ImGui::TableSetupColumn("Gap ms", ImGuiTableColumnFlags_WidthFixed, 70.0f);
                    ImGui::TableHeadersRow();
        ...
        const auto poolUsePercent = [](const size_t usedBytes, const size_t capacityBytes) {
            return capacityBytes > 0 ? static_cast<double>(usedBytes) * 100.0 / static_cast<double>(capacityBytes)
                                     : 0.0;
        };
        ...
        ImGui::Text("Terrain Vertex: %llu bytes (%s)", static_cast<unsigned long long>(renderWork.blockVertexBytes),
                    terrainVertexFormat);
        ...
        ImGui::Text("Frame Vertex Read: %.2f MiB (O/C/T %.2f / %.2f / %.2f)",
                    bytesToMiB(renderWork.terrainVertexReadBytes), bytesToMiB(renderWork.opaqueVertexReadBytes),
                    bytesToMiB(renderWork.cutoutVertexReadBytes), bytesToMiB(renderWork.transparentVertexReadBytes));
        ...
        ImGui::Text("Vertices O/C/T: %llu / %llu / %llu", static_cast<unsigned long long>(renderWork.opaqueVertices),
        ...
        ImGui::Text("Pool Fragmentation O/C/T: %.1f%% / %.1f%% / %.1f%%", renderWork.opaquePoolFragmentation * 100.0f,
                    renderWork.cutoutPoolFragmentation * 100.0f, renderWork.transparentPoolFragmentation * 100.0f);
        ...
        ImGui::Text("Cutout Skipped: %d / %d", renderWork.cutoutSkippedByDistance, renderWork.cutoutCandidates);
        ImGui::Text("MDI SubChunk Culled: %d / %d", renderWork.mdiSubChunksCulled, renderWork.mdiSubChunkTests);
        ...
        static constexpr const char* kTonemapModes[] = {"Reinhard [Mecraft extra]",     "AcademyFit [DerivativeMain]",
                                                        "Filmic [Mecraft extra]",       "AgX_Minimal [DerivativeMain]",
                                                        "AcademyFull [DerivativeMain]", "AgX_Full [DerivativeMain]"};
        ...
        pipelineChanged |= render_settings_imgui::showDeferredDebugView(settings);
        static constexpr const char* kLightDebugModes[] = {"0: Off",
                                                           "1: Direct Only",
                                                           "2: Skylight Only",
                                                           "3: Blocklight Only",
                                                           "4: Minimum Ambient",
        ...
        pipelineChanged |=
            ImGui::Combo("Light Debug", &lightDebugMode, kLightDebugModes, IM_ARRAYSIZE(kLightDebugModes));
        settings.debug.deferredLightDebugMode = lightDebugMode;
        static constexpr const char* kPostprocessDebugModes[] = {
            "0: Off",      "1: BloomData",  "2: FogTransmittance", "3: BloomyFog",
            "4: RainMask", "5: Time Pulse", "6: Raindrop Pattern", "7: Raindrop Distortion"};
        int ppDebugMode = settings.debug.postprocessDebugMode;
        pipelineChanged |= ImGui::Combo("Postprocess Debug", &ppDebugMode, kPostprocessDebugModes,
                                        IM_ARRAYSIZE(kPostprocessDebugModes));
        ...
        pipelineChanged |= ImGui::Combo("Reflection Debug", &reflDebugMode, kReflectionDebugModes,
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Block Parallax Depth", &settings.blockMaterialMaps.parallaxDepth, 0.0f, 0.12f, "%.3f");
        if (!settings.blockMaterialMaps.enabled) {
            ImGui::EndDisabled();
        }
        ...
        if (ImGui::Combo("Temporal Upscaler", &temporalUpscaler, temporalUpscalers.data(),
                         static_cast<int>(temporalUpscalers.size()))) {
            settings.upscale.type = temporalUpscalerTypes[static_cast<size_t>(temporalUpscaler)];
        ...
        static constexpr const char* kTemporalUpscaleQualities[] = {"Quality", "Balanced", "Performance",
                                                                    "Ultra Performance"};
        int temporalUpscaleQuality =
            settings.upscale.quality >= TemporalUpscaleQuality::Quality &&
                    settings.upscale.quality <= TemporalUpscaleQuality::UltraPerformance
                ? static_cast<int>(settings.upscale.quality) - static_cast<int>(TemporalUpscaleQuality::Quality)
                : 0;
        const bool temporalUpscalerActive = settings.upscale.type != TemporalUpscalerType::Native;
        ImGui::BeginDisabled(!temporalUpscalerActive);
        if (ImGui::Combo("Temporal Upscale Quality", &temporalUpscaleQuality, kTemporalUpscaleQualities,
                         IM_ARRAYSIZE(kTemporalUpscaleQualities))) {
            settings.upscale.quality = static_cast<TemporalUpscaleQuality>(
                static_cast<int>(TemporalUpscaleQuality::Quality) + temporalUpscaleQuality);
            pipelineChanged = true;
        }
        ImGui::EndDisabled();
        const bool fsr31Active = fsr31Supported && settings.upscale.type == TemporalUpscalerType::Fsr31;
        ImGui::BeginDisabled(!fsr31Active);
        pipelineChanged |= ImGui::Checkbox("FSR 3.1 Sharpening", &settings.upscale.sharpeningEnabled);
        pipelineChanged |=
            ImGui::SliderFloat("FSR 3.1 Sharpness", &settings.upscale.sharpeningStrength, 0.0f, 1.0f, "%.2f");
        pipelineChanged |= ImGui::Checkbox("FSR 3.1 Debug View", &settings.upscale.debugVisualizationEnabled);
        ImGui::EndDisabled();
        if (const auto& temporalFrame = renderScene.temporalFrameInput(); temporalFrame.has_value()) {
            const TemporalFrameExtents& extents = temporalFrame->extents;
            ImGui::Text("Temporal Resource: %ux%u", extents.resourceExtent.width, extents.resourceExtent.height);
            ImGui::Text("Temporal Render Rect: (%u, %u) %ux%u", extents.renderRect.x, extents.renderRect.y,
                        extents.renderRect.width, extents.renderRect.height);
            ImGui::Text("Temporal Signal Rect: (%u, %u) %ux%u", extents.signalRect.x, extents.signalRect.y,
                        extents.signalRect.width, extents.signalRect.height);
            ImGui::Text("Temporal Output: %ux%u", extents.outputExtent.width, extents.outputExtent.height);
            ...
            if (!requiresTemporalReset(temporalFrame->resetReasons)) {
                ...
                for (const TemporalResetReasonDescriptor& descriptor : temporalResetReasonDescriptors()) {
                    if (hasTemporalResetReason(temporalFrame->resetReasons, descriptor.reason)) {
        ...
        pipelineChanged |=
            ImGui::SliderFloat("FSR1 Render Scale", &settings.upscale.fsr1RenderScale, 0.50f, 1.00f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Bloom Threshold", &settings.postProcess.bloomThreshold, 0.0f, 3.0f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("DoF Focus", &settings.postProcess.dofFocusDistance, 0.5f, 50.0f, "%.1f blocks");
        ...
        pipelineChanged |= ImGui::SliderFloat("Auto Exp Min", &settings.postProcess.autoExposureMin, 0.001f, 1.0f,
                                              "%.3f", ImGuiSliderFlags_Logarithmic);
        pipelineChanged |= ImGui::SliderFloat("Auto Exp Max", &settings.postProcess.autoExposureMax, 1.0f, 64.0f,
                                              "%.2f", ImGuiSliderFlags_Logarithmic);
        ImGui::TextDisabled("DerivativeMain exposure target is unclamped; min/max are legacy UI fields.");
        pipelineChanged |=
            ImGui::SliderFloat("Auto Exp Speed", &settings.postProcess.autoExposureSpeed, 0.1f, 6.0f, "%.2f");
        pipelineChanged |=
        ...
        pipelineChanged |= ImGui::SliderFloat(
            "Scene Reflection Composite", &settings.reflection.sceneReflectionCompositeStrength, 0.0f, 1.0f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("VFog Temporal Weight", &settings.volumetric.temporalWeight, 0.0f, 0.99f, "%.2f");
        pipelineChanged |= ImGui::Checkbox("UW Volumetric Light", &settings.volumetric.uwLightEnabled);
        static constexpr const char* kVFogQualityTiers[] = {"Low: No Noise", "Medium: Cloudy Fog Lite",
                                                            "High: Cloudy Fog", "Ultra: Cloudy Sea"};
        int qualityTier = settings.volumetric.qualityTier;
        pipelineChanged |=
            ImGui::Combo("VFog Fog Type", &qualityTier, kVFogQualityTiers, IM_ARRAYSIZE(kVFogQualityTiers));
        ...
        pipelineChanged |=
            ImGui::SliderFloat("VFog Center Height", &settings.volumetric.fogCenterHeight, 0.0f, 255.0f, "%.0f");
        pipelineChanged |=
            ImGui::SliderFloat("VFog Height Spread", &settings.volumetric.fogHeightSpread, 1.0f, 200.0f, "%.0f");
        pipelineChanged |=
            ImGui::SliderFloat("VFog Noise Scale", &settings.volumetric.fogNoiseScale, 0.001f, 0.200f, "%.3f");
        pipelineChanged |=
            ImGui::SliderFloat("VFog Light Strength", &settings.volumetric.fogLightStrength, 0.0f, 1.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("VFog Density Scale", &settings.volumetric.fogDensityScale, 0.0f, 10.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("UW VL Strength", &settings.volumetric.underwaterLightStrength, 0.0f, 2.0f, "%.2f");
        if (ImGui::Combo("Weather Instant (Debug)", &weatherPresetInstant, kWeatherPresets,
                         IM_ARRAYSIZE(kWeatherPresets))) {
            world.getWeatherSystem().setDebugWeatherPresetInstant(static_cast<WeatherType>(weatherPresetInstant));
        }
        if (ImGui::Combo("Weather Smooth (Debug)", &weatherPresetSmooth, kWeatherPresets,
                         IM_ARRAYSIZE(kWeatherPresets))) {
        ...
        // Exposure diagnostics
        {
            ...
            if (settings.postProcess.autoExposureEnabled) {
                ImGui::Text("Auto Exposure: %s",
                            postProcess.isAutoExposureGpuResolved() ? "GPU texture" : "initializing");
            ...
            ImGui::Text("Direct: (%.6f, %.6f, %.6f)", skyLux.directIlluminance.r, skyLux.directIlluminance.g,
                        skyLux.directIlluminance.b);
            ImGui::Text("Sky:   (%.6f, %.6f, %.6f)", skyLux.skyIlluminance.r, skyLux.skyIlluminance.g,
                        skyLux.skyIlluminance.b);
            ImGui::Text("Sun:   (%.6f, %.6f, %.6f)", skyLux.sunIlluminance.r, skyLux.sunIlluminance.g,
                        skyLux.sunIlluminance.b);
            ImGui::Text("Moon:  (%.6f, %.6f, %.6f)", skyLux.moonIlluminance.r, skyLux.moonIlluminance.g,
                        skyLux.moonIlluminance.b);
            ...
            ImGui::Text("SunLightColor(CPU): (%.2f, %.2f, %.2f)", skyColors.sunLightColor.r, skyColors.sunLightColor.g,
                        skyColors.sunLightColor.b);
            ImGui::Text("SkyAmbientColor(CPU): (%.2f, %.2f, %.2f)", skyColors.skyAmbientColor.r,
                        skyColors.skyAmbientColor.g, skyColors.skyAmbientColor.b);
            ImGui::Text("FogColor(CPU):      (%.2f, %.2f, %.2f)", fogColor.r, fogColor.g, fogColor.b);
            ImGui::Text("HorizonScatter(CPU): (%.2f, %.2f, %.2f)", skyColors.horizonScatterColor.r,
                        skyColors.horizonScatterColor.g, skyColors.horizonScatterColor.b);
            ImGui::Text("DirectSunStrength: %.2f  SkyAmbientStrength: %.2f", settings.postProcess.directSunStrength,
                        settings.postProcess.skyAmbientStrength);
            ImGui::Text("SunWarmth: %.2f  SkyCoolness: %.2f", settings.postProcess.sunWarmth,
                        settings.postProcess.skyCoolness);
            ...
            ImGui::Text("VFog Temporal: %s (weight: %.2f)", settings.volumetric.temporalEnabled ? "ON" : "OFF",
            ...
            ImGui::Text("env.sunIlluminance: (%.2f, %.2f, %.2f)", skyLux.sunIlluminance.r, skyLux.sunIlluminance.g,
                        skyLux.sunIlluminance.b);
            ImGui::Text("env.skyIlluminance:  (%.2f, %.2f, %.2f)", skyLux.skyIlluminance.r, skyLux.skyIlluminance.g,
                        skyLux.skyIlluminance.b);
            ...
            ImGui::Text("VFog profile: center=%.0f  spread=%.0f  noise=%.3f  light=%.2f  density=%.2f",
                        settings.volumetric.fogCenterHeight, settings.volumetric.fogHeightSpread,
                        settings.volumetric.fogNoiseScale, settings.volumetric.fogLightStrength,
            ...
            ImGui::Text("VFog tier: %s  maxDist: 260  heightSpread: %.0f", tierNames[qualityTier],
                        settings.volumetric.fogHeightSpread);
            {
                ...
                float tierMultiplier =
                    qualityTier <= 0 ? 0.5f : (qualityTier <= 1 ? 1.4f : (qualityTier <= 2 ? 9.0f : 48.0f));
                ...
                ImGui::Text("VFog effective tier density: %.2f x mistGate = %.3f", tierMultiplier,
                            tierMultiplier * mistGate);
                if (weather.type == WeatherType::Clear && mistGate < 0.02f) {
                    ImGui::TextDisabled("Clear noon: DerivativeMain TIME_FADE suppresses mist density; FOG_TYPE mostly "
                                        "affects density shape.");
                }
            }
            // Active light direction (shadow system uses this)
            ImGui::TextColored(ImVec4(0.9f, 0.85f, 0.5f, 1.0f), "Active Light");
            ImGui::Text("SunDir: (%.2f, %.2f, %.2f)  MoonDir: (%.2f, %.2f, %.2f)", skyColors.sunDirection.x,
                        skyColors.sunDirection.y, skyColors.sunDirection.z, skyColors.moonDirection.x,
                        skyColors.moonDirection.y, skyColors.moonDirection.z);
            ImGui::Text("SunVis: %.3f  MoonVis: %.6f  DayFactor: %.3f", skyColors.sunVisibility,
                        skyColors.moonVisibility, world.getDayNightSystem().getSkyIntensity());
            ...
            // DerivativeMain time weights (shader-side, from sun direction)
            {
                ...
                ImGui::Text("Noon: %.3f  Midnight: %.3f  Sunrise: %.3f  Sunset: %.3f", timeNoon, timeMidnight,
                            timeSunrise, timeSunset);
                ImGui::Text("meWeight: %.3f  (sunY=%.3f, sunX=%.3f)", meWeight, sunY, sunX);
                ImGui::Text("CPU DayFactor: %.3f  vs  GPU meWeight: %.3f", world.getDayNightSystem().getSkyIntensity(),
                            meWeight);
            }
            // Weather state
            const char* tonemapNames[] = {"Reinhard [Mecraft]",      "AcademyFit [DerivMain]",  "Filmic [Mecraft]",
                                          "AgX_Minimal [DerivMain]", "AcademyFull [DerivMain]", "AgX_Full [DerivMain]"};
            ...
            ImGui::Text("Weather current: %s  wet=%.2f storm=%.2f aerialRed=%.2f",
                        weatherNames[static_cast<int>(weather.type)], weather.wetness, weather.storm,
                        weather.aerialReduction);
            ImGui::Text("Weather target:  %s  wet=%.2f storm=%.2f aerialRed=%.2f",
                        weatherNames[static_cast<int>(targetWeather.type)], targetWeather.wetness, targetWeather.storm,
                        targetWeather.aerialReduction);
            {
                ...
                ImGui::Text("Derived weather: sky=%.2f fog=%.2f cloud=%.2f surface=%.2f rain=%.2f",
                            derivedWeather.skyWetness, derivedWeather.fogWetness, derivedWeather.cloudWetness,
                            derivedWeather.surfaceWetness, derivedWeather.rainStrength);
                ImGui::Text("Weather direct shadow: %.3f  (mix(1.0, 0.03, derived.skyWetness))", overcastShadow);
                pipelineChanged |=
                    ImGui::SliderFloat("Direct Occlusion Override", &settings.weather.directWeatherOcclusion, -1.0f,
                                       1.0f, "%.2f (<0=auto, >=0=bypass all cloud shadow)");
                ImGui::TextDisabled("SkyCapture metadata stays weather-independent; direct rain dimming happens in "
                                    "deferred cloudShadow.");
            }
            ...
            // Default values (1.0 / 0.0) = no override, use DerivativeMain contract.
            {
                ...
                pipelineChanged |= ImGui::SliderFloat("Skylight Scale##weather", &settings.weather.skylightScale, 0.0f,
                                                      1.0f, "%.2f (x DerivMain)");
                pipelineChanged |= ImGui::SliderFloat("Exposure Bias##weather", &settings.weather.exposureBias, -2.0f,
                                                      2.0f, "%.2f EV");
                pipelineChanged |= ImGui::SliderFloat("Post Rain Fog##weather", &settings.weather.postRainFog, 0.0f,
                                                      2.0f, "%.2f (x DerivMain)");
                pipelineChanged |= ImGui::SliderFloat("Rain Alpha Scale##weather", &settings.weather.rainAlphaScale,
                                                      0.0f, 5.0f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Shadow Normal Offset", &settings.shadow.normalOffset, 0.0f, 0.12f, "%.3f");
        pipelineChanged |=
            ImGui::SliderFloat("Contact Shadow Strength", &settings.shadow.contactShadowStrength, 0.0f, 0.6f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Cloud Shadow Strength", &settings.cloud.shadowStrength, 0.0f, 0.8f, "%.2f");
        ...
        ImGui::TextDisabled(
            "DerivativeMain CLOUDS_SPEED adapter. Legacy Cloud Shadow Speed is ignored by the DM cloud path.");
        pipelineChanged |=
            ImGui::SliderFloat("Post Sun Ray Strength", &settings.postProcess.sunRayStrength, 0.0f, 0.6f, "%.2f");
        ImGui::TextDisabled("VFog Strength 1.00 matches DerivativeMain VOLUMETRIC_FOG_DENSITY baseline");
        pipelineChanged |=
            ImGui::SliderFloat("VFog Shadow Bias Scale", &settings.volumetric.shadowBiasScale, 0.0f, 4.0f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Color Temperature", &settings.postProcess.colorTemperature, 0.0f, 2.0f, "%.2f");
        pipelineChanged |= ImGui::SliderFloat("Vibrance", &settings.postProcess.vibrance, -0.5f, 0.8f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Highlight Compress", &settings.postProcess.highlightCompression, 0.0f, 1.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Film Emulation", &settings.postProcess.filmEmulationStrength, 0.0f, 1.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Red Modifier", &settings.postProcess.redModifierStrength, 0.0f, 1.0f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Albedo Desat", &settings.postProcess.albedoDesaturation, 0.0f, 0.8f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Shadow Desat", &settings.postProcess.shadowDesaturation, 0.0f, 1.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Split Tone", &settings.postProcess.splitToneStrength, 0.0f, 1.0f, "%.2f");
        pipelineChanged |= ImGui::SliderFloat("Vignette", &settings.postProcess.vignetteStrength, 0.0f, 0.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Shadow Tint", &settings.postProcess.shadowTintStrength, 0.0f, 0.8f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Direct Sun", &settings.postProcess.directSunStrength, 0.0f, 3.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Sky Ambient", &settings.postProcess.skyAmbientStrength, 0.0f, 1.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Minimum Ambient", &settings.postProcess.minimumAmbient, 0.0f, 0.4f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Shadow Min Light", &settings.postProcess.shadowMinLight, 0.0f, 0.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Shadow Contrast", &settings.postProcess.shadowContrast, 0.5f, 2.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Block Light", &settings.postProcess.blockLightStrength, 0.0f, 2.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Fake Bounce", &settings.postProcess.fakeBounceStrength, 0.0f, 0.3f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Aerial Strength", &settings.postProcess.aerialStrength, 0.0f, 1.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Horizon Scatter", &settings.postProcess.horizonScatterStrength, 0.0f, 1.5f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Volumetric Fog Strength", &settings.volumetric.fogStrength, 0.0f, 2.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("Noise Dither", &settings.postProcess.noiseDitherStrength, 0.0f, 0.05f, "%.3f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("SSGI Denoise Strength", &settings.ssgi.denoiseStrength, 0.0f, 1.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("SSGI Radiance Filter", &settings.ssgi.radianceFilterStrength, 0.0f, 1.0f, "%.2f");
        pipelineChanged |=
            ImGui::SliderFloat("SSGI Color Bleed", &settings.ssgi.colorBleedStrength, 0.0f, 1.0f, "%.2f");
        ...
        pipelineChanged |=
            ImGui::SliderFloat("Reflection History Weight", &settings.reflection.historyWeight, 0.0f, 0.98f, "%.2f");
        pipelineChanged |= render_settings_imgui::showPictureAdjustments(settings);
        ...
        fogChanged |=
            ImGui::SliderFloat("Auto End Offset (chunks)", &settings.fog.autoEndOffsetChunks, -2.0f, 1.0f, "%.2f");

        fogChanged |=
            ImGui::SliderFloat("Auto Fade Width (chunks)", &settings.fog.autoFadeWidthChunks, 0.25f, 4.0f, "%.2f");
        ...
        ImGui::Text("Light Queued/Dirty/Pending: %d / %d / %d", lightStats.queued, lightStats.dirty,
                    lightStats.pendingCompleted);
        ImGui::Text("Light Submitted Reasons C/B/N: %d / %d / %d", lightStats.submittedChunkLoaded,
                    lightStats.submittedBlockChanged, lightStats.submittedNeighborBoundary);
        ImGui::Text("Light Dirty Reasons C/B/N: %d / %d / %d", lightStats.dirtyChunkLoaded,
                    lightStats.dirtyBlockChanged, lightStats.dirtyNeighborBoundary);
        ImGui::Text("Light Block Changes: %d calls, %d chunks, last (%d,%d,%d) %u -> %u", lightStats.blockChangeCalls,
                    lightStats.blockChangeUniqueChunks, lightStats.lastBlockChangeX, lightStats.lastBlockChangeY,
                    lightStats.lastBlockChangeZ, static_cast<unsigned>(lightStats.lastBlockChangeOld),
        ...
        ImGui::Text("Light Capture: %.3f ms (%d jobs, avg %.3f ms)", lightStats.captureMs, lightStats.captureCount,
                    lightStats.captureCount > 0 ? lightStats.captureMs / static_cast<float>(lightStats.captureCount)
                                                : 0.0f);
        ...
        const float transparentGreedyReduction =
            transparentGreedyBefore > 0
                ? (100.0f * static_cast<float>(transparentGreedyBefore - transparentGreedyAfter) /
                   static_cast<float>(transparentGreedyBefore))
                : 0.0f;
        ImGui::Text("Transparent Faces: %u -> %u (%.1f%% fewer)", transparentGreedyBefore, transparentGreedyAfter,
        ...
        if (historyCount > 1) {
            ...
            ImGui::PlotLines("Submitted History", submittedHistory.data(), static_cast<int>(historyCount), 0, nullptr,
                             0.0f, 64.0f, ImVec2(0.0f, 60.0f));
            ImGui::PlotLines("Completed History", completedHistory.data(), static_cast<int>(historyCount), 0, nullptr,
                             0.0f, 64.0f, ImVec2(0.0f, 60.0f));
            ImGui::PlotLines("In-Flight History", inFlightHistory.data(), static_cast<int>(historyCount), 0, nullptr,
                             0.0f, 256.0f, ImVec2(0.0f, 60.0f));
        }

        const CullingFrameStats cullingStats = render.getCullingFrameStats();
        const float regionPassRate = cullingStats.regionTests > 0
                                         ? (100.0f * static_cast<float>(cullingStats.regionPassed) /
                                            static_cast<float>(cullingStats.regionTests))
                                         : 0.0f;
        const float columnPassRate = cullingStats.columnTests > 0
                                         ? (100.0f * static_cast<float>(cullingStats.columnPassed) /
                                            static_cast<float>(cullingStats.columnTests))
                                         : 0.0f;
        const float chunkPassRate =
            cullingStats.chunkTests > 0
...
void Dashboard::showGUIScaleSettings(UIRenderer& uiRenderer) {
    if (ImGui::CollapsingHeader("GUI Scale")) {
        ...
        // Scale options
        const char* scaleOptions[] = {"Auto (Based on Resolution)", "Small (0.5x)", "Normal (1.0x)", "Large (2.0x)",
                                      "Extra Large (3.0x)"};
