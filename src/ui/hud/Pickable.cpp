...
int Pickable::hitTest(const SlotInfo* slots, const int count, const float mouseX, const float mouseY) {
    if (slots == nullptr || count <= 0) {
        return -1;
    }
    for (int i = 0; i < count; ++i) {
        if (mouseX >= slots[i].x && mouseX < slots[i].x + slots[i].size && mouseY >= slots[i].y &&
...
void Pickable::render(const SlotInfo* slots, const int count, const int hoveredIndex, const RenderParams& params,
    ...
    if (context.phase == UIRenderPhase::Record && context.commandList != nullptr && context.uiRenderer != nullptr &&
        ...
        if (context.imageTexturePipeline.isValid()) {
            ...
            const auto drawAtlasItems = [&](const bool drawBakedBlockIcons, const TextureAtlas& atlas,
                ...
                for (int i = 0; i < count; ++i) {
                    ...
                    const ItemDef& itemDef = ItemRegistry::get(static_cast<ItemID>(slot.itemId));
