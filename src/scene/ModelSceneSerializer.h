...
public:
    ...
    [[nodiscard]] static bool deserialize(const nlohmann::json& value, ModelSceneDocument& document,
                                          std::string& error);

    /// Writes a scene through a temporary file and atomic rename sequence.
    [[nodiscard]] static bool saveToFile(const std::string& path, const ModelSceneDocument& document,
                                         std::string& error);

    /// Reads, parses, and validates a scene file without changing runtime state.
    [[nodiscard]] static bool loadFromFile(const std::string& path, ModelSceneDocument& document, std::string& error);
};
