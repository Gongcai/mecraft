...
int main() {
    ...
    InventoryStateContext deps{fsm, inventory, context, input, uiRenderer, dropSystem, registry};

    const glm::ivec3 blockPosition(1, 64, 2);
    std::unique_ptr<IGameState> chest = ContainerStateFactory::create(deps, "minecraft:chest", blockPosition);
    if (chest->kind() != GameStateKind::DataDrivenContainer) {
        return fail("storage handler should create a data-driven container state");
    }

    std::unique_ptr<IGameState> barrel = ContainerStateFactory::create(deps, "minecraft:barrel", blockPosition);
    ...
    std::unique_ptr<IGameState> dispenser = ContainerStateFactory::create(deps, "minecraft:dispenser", blockPosition);
    ...
    std::unique_ptr<IGameState> dropper = ContainerStateFactory::create(deps, "minecraft:dropper", blockPosition);
    ...
    std::unique_ptr<IGameState> hopper = ContainerStateFactory::create(deps, "minecraft:hopper", blockPosition);
    ...
    std::unique_ptr<IGameState> furnace = ContainerStateFactory::create(deps, "minecraft:furnace", blockPosition);
