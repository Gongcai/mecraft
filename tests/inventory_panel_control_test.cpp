...
int main() {
    ...
    panel.render({});
    if (panel.onInput({UIInputEventType::PointerMove, 10.0f, 10.0f, 0}) != UIEventResult::Ignored) {
    ...
    if (panel.onInput({UIInputEventType::PointerMove, 2.0f, 2.0f, 0}) == UIEventResult::Ignored) {
        return fail("row 1 slot should be hit");
    }

    if (panel.onInput({UIInputEventType::PointerMove, 2.0f, 35.0f, 0}) != UIEventResult::Ignored) {
        return fail("gap between row 3 and row 4 should not be hit");
    }

    if (panel.onInput({UIInputEventType::PointerMove, 22.0f, 55.0f, 0}) == UIEventResult::Ignored) {
        return fail("row 4 slot should be hit");
    }

    if (panel.onInput({UIInputEventType::PointerDown, 22.0f, 55.0f, 0}) != UIEventResult::Consumed) {
