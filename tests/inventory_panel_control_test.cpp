...
int main() {
    ...
    panel.render({});
    if (panel.onInput({UIInputEventType::PointerMove, 10.0f, 10.0f, UIPointerButton::None}) != UIEventResult::Ignored) {
    ...
    if (panel.onInput({UIInputEventType::PointerMove, 2.0f, 2.0f, UIPointerButton::None}) == UIEventResult::Ignored) {
        return fail("row 1 slot should be hit");
    }

    if (panel.onInput({UIInputEventType::PointerMove, 2.0f, 35.0f, UIPointerButton::None}) != UIEventResult::Ignored) {
        return fail("gap between row 3 and row 4 should not be hit");
    }

    if (panel.onInput({UIInputEventType::PointerMove, 22.0f, 55.0f, UIPointerButton::None}) == UIEventResult::Ignored) {
        return fail("row 4 slot should be hit");
    }

    if (panel.onInput({UIInputEventType::PointerDown, 22.0f, 55.0f, UIPointerButton::Primary}) != UIEventResult::Consumed) {
        return fail("row 4 slot click should be consumed");
    }

    if (panel.onInput({UIInputEventType::PointerUp, 22.0f, 55.0f, UIPointerButton::Primary}) == UIEventResult::Ignored) {
