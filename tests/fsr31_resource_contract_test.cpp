...
VkRhiTextureInteropInfo sampledTexture(const VkFormat format,
    ...
    resource.aspectMask = format == VK_FORMAT_D32_SFLOAT
...
Fsr31VulkanResourceSet completeResources() {
    ...
    resources.hdrColor = sampledTexture(
        VK_FORMAT_R16G16B16A16_SFLOAT, kRenderExtent, 1u);
    resources.depth = sampledTexture(VK_FORMAT_D32_SFLOAT, kRenderExtent, 2u);
    resources.velocity = sampledTexture(
        VK_FORMAT_R16G16_SFLOAT, kRenderExtent, 3u);
    resources.exposure = sampledTexture(
        VK_FORMAT_R16G16B16A16_SFLOAT, {1u, 1u}, 4u);
    resources.reactiveMask = sampledTexture(VK_FORMAT_R8_UNORM, kRenderExtent, 5u);
    resources.transparencyMask = sampledTexture(
        VK_FORMAT_R8_UNORM, kRenderExtent, 6u);
    resources.outputHdrColor = sampledTexture(
        VK_FORMAT_R16G16B16A16_SFLOAT, kOutputExtent, 7u);
    resources.outputHdrColor.usage =
...
bool testCompleteContract() {
    const auto failure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, completeResources());
    return requireTrue(!failure.has_value(),
                       "complete FSR 3.1 Vulkan resources must validate");
}
...
bool testFormatValidation() {
    ...
    const auto failure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    return requireTrue(failure.has_value() &&
                           failure->role == Fsr31ResourceRole::Velocity &&
...
bool testExtentValidation() {
    ...
    const auto failure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    return requireTrue(failure.has_value() &&
                           failure->role == Fsr31ResourceRole::OutputHdrColor &&
...
bool testUsageValidation() {
    ...
    const auto outputFailure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    if (!requireTrue(outputFailure.has_value() &&
                         outputFailure->role == Fsr31ResourceRole::OutputHdrColor &&
                         outputFailure->error ==
                             Fsr31ResourceValidationError::MissingStorageUsage,
    ...
    const auto inputFailure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    return requireTrue(inputFailure.has_value() &&
                           inputFailure->role == Fsr31ResourceRole::ReactiveMask &&
                           inputFailure->error ==
                               Fsr31ResourceValidationError::MissingSampledUsage,
...
bool testNativeObjectAndSubresourceValidation() {
    ...
    const auto nativeFailure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    if (!requireTrue(nativeFailure.has_value() &&
                         nativeFailure->role == Fsr31ResourceRole::Depth &&
                         nativeFailure->error ==
    ...
    const auto rangeFailure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    return requireTrue(rangeFailure.has_value() &&
                           rangeFailure->role == Fsr31ResourceRole::Exposure &&
                           rangeFailure->error ==
                               Fsr31ResourceValidationError::InvalidSubresourceRange,
...
bool testAspectValidation() {
    ...
    const auto failure = validateFsr31VulkanResourceSet(
        {1280u, 720u}, {1920u, 1080u}, resources);
    return requireTrue(failure.has_value() &&
                           failure->role == Fsr31ResourceRole::Depth &&
                           failure->error ==
                               Fsr31ResourceValidationError::InvalidAspectMask,
...
int main() {
    if (!testCompleteContract()) return 1;
    if (!testFormatValidation()) return 1;
    if (!testExtentValidation()) return 1;
    if (!testUsageValidation()) return 1;
    if (!testNativeObjectAndSubresourceValidation()) return 1;
    if (!testAspectValidation()) return 1;
    return 0;
}
