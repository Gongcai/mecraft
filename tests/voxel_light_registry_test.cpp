...
public:
    ...
    [[nodiscard]] BlockStateId getBlock(
        const int x, const int y, const int z) const override {
        if (y < 0 || y >= Chunk::SIZE_Y) {
            return NULL_BLOCK_STATE;
        }
        const glm::ivec2 coordinates = getChunkCoords(x, z);
        const auto found = chunks.find(
            IWorldView::chunkKey(coordinates.x, coordinates.y));
        if (found == chunks.end()) {
            return NULL_BLOCK_STATE;
        }
        return found->second->getBlock(
            x - coordinates.x * Chunk::SIZE_X,
            y,
            z - coordinates.y * Chunk::SIZE_Z);
    }
    [[nodiscard]] uint8_t getPackedLight(
        int, int, int) const override {
        return 0u;
    }
    [[nodiscard]] BlockStateId getBlockState(
        const int x, const int y, const int z) const override {
        return getBlock(x, y, z);
    }
    [[nodiscard]] BlockStateId getFluidState(
        int, int, int) const override {
        return NULL_BLOCK_STATE;
    }
    [[nodiscard]] bool isChunkLoadedForBlock(
        const int x, int, const int z) const override {
        const glm::ivec2 coordinates = getChunkCoords(x, z);
        return chunks.find(IWorldView::chunkKey(
                   coordinates.x, coordinates.y)) != chunks.end();
    }
    [[nodiscard]] int getRenderDistance() const override {
        return 2;
    }
    [[nodiscard]] glm::ivec2 getChunkCoords(
        const int worldX, const int worldZ) const override {
        return {
            static_cast<int>(std::floor(
                static_cast<double>(worldX) / Chunk::SIZE_X)),
            static_cast<int>(std::floor(
                static_cast<double>(worldZ) / Chunk::SIZE_Z))};
    }
...
bool testIncrementalVoxelLights() {
    ...
    const BlockID torchId =
        BlockRegistry::requireIdByName("minecraft:torch");
    const BlockID redstoneTorchId =
    ...
    const BlockID magmaId =
        BlockRegistry::requireIdByName("minecraft:magma_block");
    const BlockID stoneId =
        BlockRegistry::requireIdByName("minecraft:stone");

    const BlockStateId torch =
        BlockStateRegistry::getDefaultState(torchId);
    const BlockStateId redstoneTorchOn =
        BlockStateRegistry::getDefaultState(redstoneTorchId);
    const BlockStateId redstoneTorchOff =
        BlockStateRegistry::withProperty(
            redstoneTorchOn, PropIndices::LIT, PropIndices::LIT_FALSE);
    const BlockStateId redstoneLampOff =
        BlockStateRegistry::getDefaultState(redstoneLampId);
    ...
    const BlockStateId magma =
        BlockStateRegistry::getDefaultState(magmaId);
    const BlockStateId stone =
        BlockStateRegistry::getDefaultState(stoneId);

    if (!requireTrue(
            BlockRegistry::getFast(redstoneLampId).analyticLight.has_value(),
            "redstone lamps must declare an analytic light template") ||
        !requireTrue(
            BlockRegistry::getFast(redstoneLampId)
                    .analyticLight->enabledStatePropertyIndex ==
                PropIndices::LIT &&
            BlockRegistry::getFast(redstoneLampId)
                    .analyticLight->enabledStateValueIndex ==
    ...
    if (!requireTrue(
        ...
        !requireTrue(registry.lightRevision() > initialRevision,
                     "source edits must advance the light revision")) {
        return false;
    }
    ...
    return requireTrue(
        center->getBlockContentRevision() > fastRevision,
