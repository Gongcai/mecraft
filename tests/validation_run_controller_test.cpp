...
bool requireTrue(const bool condition, const char* message) {
    if (!condition) {
        std::cerr << "[validation_run_controller_test] FAIL: "
                  << message << '\n';
        return false;
    }
    return true;
...
int main() {
    ...
    if (!requireTrue(controller.configure(options),
                     "a valid Camera Path must configure validation") ||
        !requireTrue(
            controller.renderSettingsProfile().id ==
    ...
    const app::validation::ValidationFrame* frame =
    ...
    if (!requireTrue(frame != nullptr && !frame->collectPerformance &&
        ...
        !requireTrue(controller.completeFrame(true),
                     "the first warmup frame must complete") ||
        !requireTrue(controller.completeFrame(true),
                     "the second warmup frame must complete")) {
        return 1;
    }
    ...
    if (!requireTrue(frame != nullptr && frame->collectPerformance &&
        ...
        !requireTrue(controller.completeFrame(true),
                     "the first sample must complete") ||
        !requireTrue(controller.consumeCompletedSampleFrame(),
                     "each sample must publish one performance event") ||
        !requireTrue(!controller.consumeCompletedSampleFrame(),
                     "sample events must be consumed exactly once")) {
        return 1;
    }

    frame = controller.currentFrame();
    if (!requireTrue(frame != nullptr &&
                         nearlyEqual(frame->cameraPose.position.x, -6.0),
                     "the middle sample must use the exact half-duration pose") ||
        !requireTrue(controller.completeFrame(true),
                     "the middle sample must complete")) {
        return 1;
    }

    frame = controller.currentFrame();
    if (!requireTrue(frame != nullptr && frame->captureAfterRender &&
                         nearlyEqual(frame->cameraPose.position.x, -16.0),
                     "the final sample must request capture at the path endpoint") ||
        !requireTrue(!controller.completeFrame(false, "write failed") &&
                         controller.failed() &&
                         controller.error() ==
                             app::validation::ValidationRunError::CaptureFailed,
    ...
    if (!requireTrue(!validateAppLaunchOptions(invalid, error),
                     "validation must reject a single sample frame")) {
        return 1;
    }
