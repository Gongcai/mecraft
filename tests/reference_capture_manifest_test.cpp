...
constexpr std::array<ExpectedCapture, 4u> kExpectedCaptures{
    {{"voxel", "vulkan", "../scenes/m0_voxel_baseline.json",
      "m0_voxel_baseline", "ecb85fb88ef6aeef", "93b8b518406d50f9",
      "m0_voxel_render_settings", "511c4e7a0e2e2de9",
      "m0_voxel_baseline_vulkan_1280x720.png", 1511309u, "f07952a77f7a1b80",
      "e57d401434e60e967f37543721c0a753dcc02eb1103af05cdc62c2bf283d4467"},
     {"model", "vulkan", "../scenes/m0_model_damaged_helmet.json",
      "m0_model_damaged_helmet", "bde98b1cf0faca9a", "6f33df94f7766d10",
      "m0_model_render_settings", "9a8940b4590c9585",
      "m0_model_damaged_helmet_vulkan_1280x720.png", 1234823u,
      "0acff06e5a4b407b",
      "c7c20bb8820f0413dd0e4e418b9bb04d6b47689f1f07de69bf8a2896c7e25e66"},
     {"voxel", "opengl", "../scenes/m0_voxel_baseline.json",
      "m0_voxel_baseline", "ecb85fb88ef6aeef", "93b8b518406d50f9",
      "m0_voxel_render_settings", "511c4e7a0e2e2de9",
      "m0_voxel_baseline_opengl_1280x720.png", 1559880u, "c0ab8dfe38860049",
      "83491e3140815b3471cd5b1a25f576f1c727da1dc9c263dd95649bc4b4315079"},
     {"model", "opengl", "../scenes/m0_model_damaged_helmet.json",
      "m0_model_damaged_helmet", "bde98b1cf0faca9a", "6f33df94f7766d10",
      "m0_model_render_settings", "9a8940b4590c9585",
      "m0_model_damaged_helmet_opengl_1280x720.png", 1254074u,
      "fcd624333380366c",
      "4fe801feffe55134f413fcc0e512b9fc4b5fc4f61d263250914ae3e754a30278"}}};
...
bool hasExactFields(const Json &object,
...
uint32_t readBigEndianUint32(const std::array<uint8_t, 29u> &bytes,
                             const size_t offset) {
...
bool validatePngHeader(const std::filesystem::path &path) {
  ...
  constexpr std::array<uint8_t, 8u> kPngSignature{0x89u, 0x50u, 0x4eu, 0x47u,
  ...
  if (!requireTrue(input.gcount() == static_cast<std::streamsize>(bytes.size()),
      ...
      !requireTrue(readBigEndianUint32(bytes, 8u) == 13u && bytes[12u] == 'I' &&
                       bytes[13u] == 'H' && bytes[14u] == 'D' &&
                       bytes[15u] == 'R',
      ...
      !requireTrue(bytes[24u] == 8u && bytes[25u] == 6u && bytes[26u] == 0u &&
...
const ExpectedCapture *findExpectedCapture(const std::string_view scene,
                                           const std::string_view backend,
                                           size_t &index) {
  for (size_t candidateIndex = 0u; candidateIndex < kExpectedCaptures.size();
       ++candidateIndex) {
    const ExpectedCapture &candidate = kExpectedCaptures[candidateIndex];
    if (scene == candidate.scene && backend == candidate.backend) {
...
bool validateCapture(const Json &capture,
                     const std::filesystem::path &manifestDirectory,
                     std::array<bool, kExpectedCaptures.size()> &seen) {
  if (!requireTrue(
          hasExactFields(capture, {"scene", "rhi_backend", "scene_contract",
                                   "camera_path", "render_settings", "image"}),
  ...
  if (!requireTrue(sceneValue->is_string() && backendValue->is_string(),
                   "capture scene and backend must be strings")) {
    return false;
  }
  ...
  const ExpectedCapture *expected =
      findExpectedCapture(scene, backend, expectedIndex);
  if (!requireTrue(expected != nullptr,
                   "capture scene/backend pair must belong to the M0 matrix") ||
      !requireTrue(!seen[expectedIndex],
                   "capture scene/backend pairs must be unique")) {
    return false;
  }
  ...
  if (!requireTrue(
          hasExactFields(sceneContract, {"source", "id", "version",
      ...
      !requireTrue(
          hasExactFields(renderSettings, {"id", "version", "content_hash"}),
          "render settings identity must use the exact field set") ||
      !requireTrue(hasExactFields(image, {"source", "format", "byte_size",
                                          "fnv1a64", "sha256"}),
  ...
  if (!requireTrue(
          hasString(sceneContract, "source", expected->sceneSource) &&
              hasString(sceneContract, "id", expected->sceneId) &&
              hasString(sceneContract, "content_hash", expected->sceneHash) &&
              hasString(sceneContract, "hash_algorithm",
                        renderer::contracts::kStableContentHashAlgorithm) &&
              readUint32(sceneContract, "version", sceneVersion) &&
              sceneVersion == app::validation::kValidationSceneContractVersion,
          "scene identity must match its versioned descriptor") ||
      !requireTrue(
          hasString(cameraPath, "id", expected->sceneId) &&
              hasString(cameraPath, "content_hash", expected->cameraHash),
          "Camera Path identity must match the selected scene") ||
      !requireTrue(
          hasString(renderSettings, "id", expected->renderSettingsId) &&
              hasString(renderSettings, "content_hash",
  ...
  const std::filesystem::path scenePath =
      (manifestDirectory / expected->sceneSource).lexically_normal();
  const app::validation::ValidationSceneContractLoadResult loadedScene =
      app::validation::loadValidationSceneContract(scenePath);
  if (!requireTrue(loadedScene.succeeded(),
                   "manifest scene descriptors must verify") ||
      !requireTrue(
          std::string_view(validationSceneStableId(
              loadedScene.contract.scene)) == expected->scene &&
              loadedScene.contract.id == expected->sceneId &&
              renderer::contracts::stableContentHashHex(
                  loadedScene.contract.contentHash) == expected->sceneHash &&
              renderer::contracts::stableContentHashHex(
                  loadedScene.contract.cameraPath.contentHash) ==
                  expected->cameraHash &&
              loadedScene.contract.renderSettings.id ==
                  expected->renderSettingsId &&
              renderer::contracts::stableContentHashHex(
                  loadedScene.contract.renderSettings.contentHash) ==
                  expected->renderSettingsHash,
  ...
  if (!requireTrue(
          hasString(image, "source", expected->imageSource) &&
              hasString(image, "format", "png_rgba8") &&
              byteSizeValue->is_number_unsigned() &&
              byteSizeValue->get<uintmax_t>() == expected->byteSize &&
              fnvValue->is_string() &&
              fnvValue->get_ref<const std::string &>() == expected->fnv1a64 &&
              shaValue->is_string() &&
              shaValue->get_ref<const std::string &>() == expected->sha256 &&
              isLowercaseHex(shaValue->get_ref<const std::string &>(), 64u),
          "image metadata must match the locked M0 reference")) {
    return false;
  }

  const std::filesystem::path imagePath =
      manifestDirectory / expected->imageSource;
  ...
  const renderer::contracts::FileContentHashResult actualHash =
      renderer::contracts::stableFileContentHash(imagePath);
  if (!requireTrue(!fileError && byteSize == expected->byteSize,
                   "reference PNG byte size must match the manifest") ||
      !requireTrue(
          renderer::contracts::parseStableContentHashHex(expected->fnv1a64,
                                                         expectedHash),
...
int main() {
  const std::filesystem::path manifestPath =
      ...
      "assets/validation/reference_captures/manifest.json";
  std::ifstream input(manifestPath);
  const Json manifest = Json::parse(input, nullptr, false);
  if (!requireTrue(static_cast<bool>(input),
                   "reference capture manifest must open") ||
      !requireTrue(!manifest.is_discarded(),
                   "reference capture manifest must contain valid JSON") ||
      !requireTrue(hasExactFields(manifest, {"kind", "version",
                                             "capture_profile", "captures"}),
                   "manifest root must use the exact version 1 field set") ||
      !requireTrue(
          hasString(manifest, "kind", "mecraft.reference_capture_manifest"),
          "manifest kind must remain versioned")) {
    return 1;
  }

  uint32_t manifestVersion = 0u;
  if (!requireTrue(readUint32(manifest, "version", manifestVersion) &&
                       manifestVersion == 1u,
  ...
  if (!requireTrue(
          hasExactFields(profile, {"width", "height", "warmup_frame_count",
                                   "sample_frame_count"}),
          "capture profile must use the exact version 1 field set") ||
      !requireTrue(
          readUint32(profile, "width", width) &&
              readUint32(profile, "height", height) &&
              readUint32(profile, "warmup_frame_count", warmupFrames) &&
              readUint32(profile, "sample_frame_count", sampleFrames) &&
              width == kExpectedWidth && height == kExpectedHeight &&
              warmupFrames == kExpectedWarmupFrames &&
  ...
  if (!requireTrue(captures.is_array() &&
                       captures.size() == kExpectedCaptures.size(),
                   "manifest must contain exactly four M0 captures")) {
    return 1;
  }
  std::array<bool, kExpectedCaptures.size()> seen{};
  ...
  for (const bool captureSeen : seen) {
    if (!requireTrue(captureSeen,
                     "every scene/backend M0 pair must be present")) {
      return 1;
    }
  }
