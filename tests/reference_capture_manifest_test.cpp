...
constexpr std::array<ExpectedCapture, 4u> kExpectedCaptures{
    {{"voxel", "vulkan", "../scenes/m0_voxel_baseline.json", "m0_voxel_baseline", "ecb85fb88ef6aeef",
      "93b8b518406d50f9", "m0_voxel_render_settings", "511c4e7a0e2e2de9", "m0_voxel_baseline_vulkan_1280x720.png",
      1511309u, "f07952a77f7a1b80", "e57d401434e60e967f37543721c0a753dcc02eb1103af05cdc62c2bf283d4467"},
     {"model", "vulkan", "../scenes/m0_model_damaged_helmet.json", "m0_model_damaged_helmet", "bde98b1cf0faca9a",
      "6f33df94f7766d10", "m0_model_render_settings", "9a8940b4590c9585", "m0_model_damaged_helmet_vulkan_1280x720.png",
      1234823u, "0acff06e5a4b407b", "c7c20bb8820f0413dd0e4e418b9bb04d6b47689f1f07de69bf8a2896c7e25e66"},
     {"voxel", "opengl", "../scenes/m0_voxel_baseline.json", "m0_voxel_baseline", "ecb85fb88ef6aeef",
      "93b8b518406d50f9", "m0_voxel_render_settings", "511c4e7a0e2e2de9", "m0_voxel_baseline_opengl_1280x720.png",
      1559880u, "c0ab8dfe38860049", "83491e3140815b3471cd5b1a25f576f1c727da1dc9c263dd95649bc4b4315079"},
     {"model", "opengl", "../scenes/m0_model_damaged_helmet.json", "m0_model_damaged_helmet", "bde98b1cf0faca9a",
      "6f33df94f7766d10", "m0_model_render_settings", "9a8940b4590c9585", "m0_model_damaged_helmet_opengl_1280x720.png",
      1254074u, "fcd624333380366c", "4fe801feffe55134f413fcc0e512b9fc4b5fc4f61d263250914ae3e754a30278"}}};
...
const ExpectedCapture* findExpectedCapture(const std::string_view scene, const std::string_view backend,
                                           size_t& index) {
...
bool validateCapture(const Json& capture, const std::filesystem::path& manifestDirectory,
    ...
    const ExpectedCapture* expected = findExpectedCapture(scene, backend, expectedIndex);
    if (!requireTrue(expected != nullptr, "capture scene/backend pair must belong to the M0 matrix") ||
        !requireTrue(!seen[expectedIndex], "capture scene/backend pairs must be unique")) {
        return false;
    }
    ...
    if (!requireTrue(hasString(sceneContract, "source", expected->sceneSource) &&
    ...
    if (!requireTrue(loadedScene.succeeded(), "manifest scene descriptors must verify") ||
        !requireTrue(std::string_view(validationSceneStableId(loadedScene.contract.scene)) == expected->scene &&
...
int main() {
    ...
    for (const bool captureSeen : seen) {
        if (!requireTrue(captureSeen, "every scene/backend M0 pair must be present")) {
            return 1;
        }
    }
