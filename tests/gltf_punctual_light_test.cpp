...
bool testParsedPunctualLights() {
    const std::string assetPath =
        ...
        "/tests/assets/punctual_lights.gltf";
    cgltf_options options{};
    cgltf_data* rawData = nullptr;
    cgltf_result parseResult =
        cgltf_parse_file(&options, assetPath.c_str(), &rawData);
    if (!requireTrue(
            parseResult == cgltf_result_success && rawData != nullptr,
    ...
    if (!requireTrue(
            cgltf_load_buffers(&options, data.get(), assetPath.c_str()) ==
                cgltf_result_success &&
            cgltf_validate(data.get()) == cgltf_result_success,
            "the punctual-light glTF asset must satisfy cgltf validation") ||
        !requireTrue(
            data->extensions_required_count == 1u &&
            std::strcmp(data->extensions_required[0],
    ...
    for (cgltf_size index = 0u;
         index < data->scene->nodes_count; ++index) {
    ...
    for (std::size_t index = 0u; index < nodes.size(); ++index) {
        ...
        if (node->light != nullptr) {
            ...
            if (!requireTrue(decoded.succeeded(),
                             "every default-scene light node must decode")) {
                return false;
            }
            sources.push_back(decoded.source);
        }
        for (cgltf_size child = 0u;
             child < node->children_count; ++child) {
            nodes.push_back(node->children[child]);
        }
    }

    using namespace renderer::contracts;
    if (!requireTrue(sources.size() == 3u,
                     "point, spot, and directional nodes must all decode") ||
        !requireTrue(
        ...
        !requireTrue(
            ...
            near(sources[1].innerConeAngleRadians, 0.2f) &&
...
bool testStructuredDecodeFailure() {
    ...
    if (!requireTrue(
        ...
        !requireTrue(
               invalidRange.normalizationField ==
                   renderer::contracts::GpuLightField::Range,
    ...
    return requireTrue(
           ...
           requireTrue(
               invalidTransform.instantiationError ==
                   renderer::contracts::AnalyticLightInstantiationError::
                       ShearedTransform,
...
bool testStaticMeshIntegrationSource() {
    ...
    if (!requireTrue(stream.is_open(),
                     "StaticMeshRenderer source must be readable")) {
        return false;
    }
    const std::string source((std::istreambuf_iterator<char>(stream)),
                             std::istreambuf_iterator<char>());
    return requireTrue(
               source.find("KHR_lights_punctual") != std::string::npos,
               "the static asset extension contract must accept punctual lights") &&
           requireTrue(
               source.find("decodeGltfPunctualLight(*node)") !=
                   std::string::npos,
...
int main() {
    if (!testParsedPunctualLights() || !testStructuredDecodeFailure() ||
