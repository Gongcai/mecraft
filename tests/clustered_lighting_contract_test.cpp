...
bool requireTrue(const bool condition, const char* message) {
    if (!condition) {
        std::cerr << "[clustered_lighting_contract_test] FAIL: " << message << '\n';
        return false;
    }
    return true;
}
...
renderer::contracts::GpuLight makePointLight(const uint32_t id, const glm::vec3 position, const float range) {
...
bool testGridAndLogarithmicSlices() {
    using namespace renderer::contracts;
    const std::optional<ClusterGrid> grid = buildClusterGrid(1920u, 1080u, 0.1f, 1000.0f);
    return requireTrue(grid.has_value(), "valid render extents must build a cluster grid") &&
           requireTrue(grid->tileCountX == 120u && grid->tileCountY == 68u,
                       "16-pixel tiles must round the render extent upward") &&
           requireTrue(grid->depthSliceCount == 24u && grid->clusterCount == 195840u,
                       "the initial contract must use 24 logarithmic slices") &&
           requireTrue(kClusterCoverageWorkgroupSize == 64u && kClusterMaxLightCount == 65535u,
                       "coverage dispatch must use one bounded workgroup per light") &&
           requireTrue(clusterDepthSlice(*grid, grid->nearPlane) == 0u, "the near plane must map to the first slice") &&
           requireTrue(clusterDepthSlice(*grid, grid->farPlane) == 23u, "the far plane must map to the last slice") &&
...
bool testCoverageAndCapacity() {
    ...
    const glm::mat4 projection = glm::perspective(glm::radians(70.0f), 1280.0f / 720.0f, 0.1f, 500.0f);
    ...
    const auto directionalBounds = buildGpuClusterLightBounds(directional, grid, view, projection);
    if (!requireTrue(directionalBounds.has_value() && directionalBounds->minCluster.w == 1u,
                     "directional lights must intersect every cluster") ||
        !requireTrue(clusterLightCoverageCount(*directionalBounds) == grid.clusterCount,
                     "directional coverage must equal the complete lattice")) {
        return false;
    }

    const GpuLight center = makePointLight(2u, {0.0f, 0.0f, -8.0f}, 2.0f);
    const auto centerBounds = buildGpuClusterLightBounds(center, grid, view, projection);
    ...
    if (!requireTrue(outsideBounds.has_value() && outsideBounds->minCluster.w == 0u &&
                         clusterLightCoverageCount(*outsideBounds) == 0u,
                     "off-screen local lights must remain explicitly inactive")) {
        return false;
    }

    const std::vector<GpuClusterLightBounds> bounds{*directionalBounds, *centerBounds, *outsideBounds};
    const std::optional<uint32_t> capacity = requiredClusterLightIndexCount(bounds);
    const std::optional<uint32_t> inactiveCapacity = requiredClusterLightIndexCount({*outsideBounds});
    return requireTrue(capacity.has_value(), "valid coverage counts must produce a compact capacity") &&
           requireTrue(*capacity == grid.clusterCount + clusterLightCoverageCount(*centerBounds),
                       "capacity must exactly equal the sum of light coverage") &&
           requireTrue(inactiveCapacity.has_value() && *inactiveCapacity == 0u,
...
bool testReadbackCompletionContract() {
    ...
    return requireTrue(pass.find("isSubmissionComplete(token, complete)") != std::string::npos,
...
bool testEmptyLightingSteadyStateContract() {
    ...
    if (!requireTrue(readProjectFile("src/renderer/passes/ClusteredLightingPass.cpp", pass),
                     "clustered-light pass source must be readable") ||
        !requireTrue(readProjectFile("src/renderer/passes/DeferredLightingPass.cpp", lightingPass),
                     "deferred-light pass source must be readable") ||
        !requireTrue(readProjectFile("assets/shaders/deferred_lighting.frag", deferred),
    ...
    const size_t activeLightBranch = deferred.find("if (uClusterActiveLightCount > 0)");
    const size_t clusteredEvaluation = deferred.find("evaluateClusteredSurfaceLighting(");
    return requireTrue(emptySkip != std::string::npos && dependencyReturn != std::string::npos &&
...
bool testComputeShaderContracts() {
    ...
    return requireTrue(scan.find("shared uint uScan[512]") != std::string::npos,
           ...
           requireTrue(count.find("gl_WorkGroupID.x") != std::string::npos &&
                           count.find("gl_LocalInvocationID.x") != std::string::npos &&
                           count.find("CLUSTER_COVERAGE_WORKGROUP_SIZE") != std::string::npos &&
                           count.find("gl_WorkGroupID.y") != std::string::npos,
                       "count must distribute every light slice across one workgroup") &&
           requireTrue(fill.find("uCompactLightIndices") != std::string::npos,
                       "fill must write the compact light index list") &&
           requireTrue(fill.find("gl_WorkGroupID.x") != std::string::npos &&
                           fill.find("gl_LocalInvocationID.x") != std::string::npos &&
                           fill.find("bounds.maxCluster.w") != std::string::npos &&
                           fill.find("CLUSTER_COVERAGE_WORKGROUP_SIZE") != std::string::npos &&
...
bool testSharedLightingConsumers() {
    ...
    std::string pipeline;
    if (!requireTrue(readProjectFile("assets/shaders/deferred_lighting.frag", deferred),
                     "deferred lighting shader must be readable") ||
        !requireTrue(readProjectFile("assets/shaders/static_mesh_transparent_rhi.frag", staticMeshForward),
                     "static mesh Forward+ shader must be readable") ||
        !requireTrue(readProjectFile("assets/shaders/chunk_lit_common.frag", terrainForward),
                     "terrain Forward+ shader must be readable") ||
        !requireTrue(readProjectFile("assets/shaders/clustered_lighting.glsl", sharedLighting),
                     "shared clustered-light shader must be readable") ||
        !requireTrue(readProjectFile("assets/shaders/clustered_light_evaluation.glsl", lightEvaluation),
    ...
    const size_t contributionEvaluation =
        sharedLighting.find("GpuLightSurfaceContribution contribution = evaluateGpuLight(");
    const size_t shadowSampling = sharedLighting.find("shadowVisibilityValue = localShadowVisibility(");
    if (!requireTrue(deferred.find("clustered_lighting.glsl") != std::string::npos &&
                         deferred.find("evaluateClusteredSurfaceLighting") != std::string::npos,
                     "deferred lighting must use the shared clustered evaluator") ||
        !requireTrue(staticMeshForward.find("clustered_lighting.glsl") != std::string::npos &&
                         staticMeshForward.find("evaluateClusteredSurfaceLighting") != std::string::npos,
                     "static mesh Forward+ must use the shared clustered evaluator") ||
        !requireTrue(terrainForward.find("clustered_lighting.glsl") != std::string::npos &&
                         terrainForward.find("evaluateClusteredSurfaceLighting") != std::string::npos,
    ...
    constexpr const char* kBindings[] = {"layout(set=MECRAFT_CLUSTER_BIND_SET,binding=0,std430)",
    ...
    constexpr const char* kResources[] = {"lights", "records", "compactIndices", "stats"};
    for (const char* resource : kResources) {
        const std::string read = std::string("readBuffer(clusteredLightingResources.") + resource;
        const size_t firstRead = normalizedPipeline.find(read);
        const size_t secondRead =
            firstRead == std::string::npos ? std::string::npos : normalizedPipeline.find(read, firstRead + read.size());
        if (!requireTrue(firstRead != std::string::npos && secondRead != std::string::npos,
                         "deferred and Forward+ graph passes must read the same clustered buffers")) {
            return false;
        }
    }
    constexpr const char* kLocalShadowResources[] = {"readBuffer(localShadowResources.metadata",
    ...
    for (const char* resource : kLocalShadowResources) {
        ...
        const size_t secondRead = firstRead == std::string::npos
                                      ? std::string::npos
                                      : normalizedPipeline.find(resource, firstRead + std::string(resource).size());
        if (!requireTrue(firstRead != std::string::npos && secondRead != std::string::npos,
    ...
    const size_t localShadowPass = normalizedPipeline.find("m_localShadowPass->addGraphPasses(");
    const size_t clusteredPass = normalizedPipeline.find("m_clusteredLightingPass->addGraphPasses(");
