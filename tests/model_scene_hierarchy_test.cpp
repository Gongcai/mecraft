...
int main() {
    ...
    if (scene.entityId(root) == scene::kInvalidSceneEntityId ||
        scene.entityId(child) == scene::kInvalidSceneEntityId ||
    ...
    auto& rootTransform =
        scene.registry().get<ecs::LocalTransformComponent>(root);
    auto& childTransform =
        scene.registry().get<ecs::LocalTransformComponent>(child);
    rootTransform.localPosition = {3.0f, 0.0f, 0.0f};
    childTransform.localPosition = {0.0f, 2.0f, 0.0f};
    scene.syncTransforms();
    const glm::mat4 childWorldBeforeParenting =
        scene.registry().get<ecs::WorldTransformComponent>(child).worldMatrix;

    if (!scene.setParent(child, root)) {
        return fail("valid scene reparenting was rejected");
    }
    const glm::mat4 childWorldAfterParenting =
        scene.registry().get<ecs::WorldTransformComponent>(child).worldMatrix;
    ...
    scene.syncTransforms();
    const glm::mat4 movedChildWorld =
        scene.registry().get<ecs::WorldTransformComponent>(child).worldMatrix;
    ...
    const glm::mat4 requestedWorld = glm::translate(
        glm::mat4(1.0f), glm::vec3(7.0f, 4.0f, -2.0f));
