...
BlockStateId observerState(const uint16_t facing, const bool powered) {
...
uint8_t wirePower(const World& world, const int x, const int y, const int z) {
    ...
    std::cerr << "[redstone_observer_test] FAIL: Wire state does not contain a valid power value\n";
    std::abort();
}
