...
bool testGpuLayoutAndPacking() {
    ...
    input.flags = gpuMaterialFlagBit(GpuMaterialFlag::DoubleSided) |
                  gpuMaterialFlagBit(GpuMaterialFlag::Specular) |
                  gpuMaterialFlagBit(GpuMaterialFlag::Ior) |
                  gpuMaterialFlagBit(GpuMaterialFlag::Clearcoat);
    for (size_t index = 0u; index < input.textureBindings.size(); ++index) {
        input.textureBindings[index] = {static_cast<uint32_t>(100u + index),
                                        static_cast<uint32_t>(200u + index)};
    }
    ...
    if (!requireTrue(result.succeeded(), "valid glTF factors must normalize") ||
        ...
        !requireTrue(result.material.transmissionVolumeFactors ==
                         glm::vec4(0.0f, 0.0f, 0.45f, 1.6f),
    ...
    for (size_t index = 0u; index < input.textureBindings.size(); ++index) {
        ...
        if (!requireTrue(packedTexture == 100u + index,
                         "texture indices must preserve semantic order") ||
            !requireTrue(packedSampler == 200u + index,
                         "sampler indices must preserve semantic order")) {
            return false;
        }
    }
    return true;
...
bool testStructuredErrors() {
    ...
    if (!requireTrue(result.error == GpuMaterialNormalizationError::NonFiniteValue &&
        ...
        !requireTrue(std::string(gpuMaterialNormalizationErrorStableId(result.error)) ==
                         "NonFiniteValue",
    ...
    input = {};
    input.alphaMode = GpuMaterialAlphaMode::Transmission;
    input.flags = gpuMaterialFlagBit(GpuMaterialFlag::Transmission) |
    ...
    if (!requireTrue(result.succeeded(), "infinite attenuation must use an explicit flag") ||
        !requireTrue(hasGpuMaterialFlag(result.material.modesAndFlags.z,
                                        GpuMaterialFlag::InfiniteAttenuationDistance),
...
bool testGltfNumericDomains() {
    ...
    const GpuMaterialNormalizationResult result = normalizeGltfMaterial(input);
    return requireTrue(result.succeeded(),
                       "glTF normal scales and alpha cutoff must preserve their schema domains") &&
           requireTrue(result.material.materialFactors.z == -2.0f,
                       "negative normal scale must remain unchanged") &&
...
bool testTextureSemantics() {
    ...
    if (!requireTrue(gpuMaterialTextureUsesSrgb(GpuMaterialTextureSemantic::BaseColor,
                                                GpuMaterialWorkflow::MetallicRoughness),
                     "base color textures must use sRGB sampling") ||
        !requireTrue(!gpuMaterialTextureUsesSrgb(
                         GpuMaterialTextureSemantic::MetallicRoughnessOrSpecularGlossiness,
                         GpuMaterialWorkflow::MetallicRoughness),
...
bool testLabPbrReferenceVectors() {
    ...
    if (!requireTrue(
            near(neutralNormal.tangentNormal, {0.00392157f, -0.00392157f, 0.9999846f}, 1.0e-5f),
            "LabPBR normal decoding must reconstruct Z and invert Y") ||
        !requireTrue(near(neutralNormal.materialAo, 1.0f, 1.0e-6f),
                     "normal blue must decode material AO") ||
        !requireTrue(near(neutralNormal.height, 1.0f, 1.0e-6f),
                     "normal alpha must preserve encoded height")) {
        return false;
    }

    const LabPbrSpecularDecodeResult stone =
        decodeLabPbrSpecular({65u, 10u, 0u, 255u}, {0.4f, 0.4f, 0.4f});
    ...
    const LabPbrSpecularDecodeResult glowstone =
        decodeLabPbrSpecular({176u, 5u, 230u, 204u}, {0.8f, 0.6f, 0.3f});
    if (!requireTrue(glowstone.succeeded(), "emissive LabPBR vector must decode") ||
        ...
        !requireTrue(glowstone.sample.emissionProvided,
                     "alpha values through 254 must provide emission")) {
        return false;
    }

    const LabPbrSpecularDecodeResult iron =
        decodeLabPbrSpecular({225u, 230u, 0u, 255u}, {0.2f, 0.3f, 0.4f});
    if (!requireTrue(iron.succeeded() && iron.sample.metalId == 230u &&
                         iron.sample.metalness == 1.0f,
    ...
    const LabPbrSpecularDecodeResult customMetal =
        decodeLabPbrSpecular({220u, 255u, 12u, 255u}, customMetalColor);
    if (!requireTrue(customMetal.succeeded() && customMetal.sample.f0 == customMetalColor,
        ...
        !requireTrue(!customMetal.sample.emissionProvided,
                     "custom metal alpha 255 must not override emission")) {
        return false;
    }

    const LabPbrSpecularDecodeResult reserved =
        decodeLabPbrSpecular({0u, 254u, 0u, 255u}, {0.5f, 0.5f, 0.5f});
...
bool testShaderLayoutMirror() {
    ...
    if (!requireTrue(source.find("GPU_MATERIAL_CONTRACT_VERSION = 1u") != std::string::npos,
        ...
        !requireTrue(source.find("GPU_MATERIAL_FLAG_INFINITE_ATTENUATION_DISTANCE") !=
                         std::string::npos,
    ...
    constexpr std::array<const char*, 12> kOrderedFields{
        {"vec4 baseColorFactor;", "vec4 emissiveFactorAndStrength;", "vec4 materialFactors;",
         "vec4 specularGlossinessFactors;", "vec4 dielectricSpecularFactors;",
         "vec4 clearcoatFactors;", "vec4 transmissionVolumeFactors;",
         "vec4 attenuationColorAndDistance;", "vec4 gameplaySurfaceFactors;",
    ...
    for (const char* field : kOrderedFields) {
        ...
        if (!requireTrue(found != std::string::npos,
                         "GLSL material layout must mirror every CPU field")) {
            return false;
        }
