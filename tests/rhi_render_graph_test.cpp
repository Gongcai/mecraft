...
RhiTextureDesc colorTextureDesc(const uint32_t mipLevels = 1u, const uint32_t layers = 1u) {
    RhiTextureDesc desc;
...
bool executeNoop(RgPassContext&) {
    return true;
}

bool testTextureDependencyAndBarrierPlanning() {
    RenderGraph graph;
    const RgTextureHandle texture = graph.createTexture({"Lighting", colorTextureDesc(), RhiResourceState::ShaderRead});
    ...
    if (!requireTrue(result.succeeded(), result.message.c_str()) ||
        !requireTrue(graph.compiledPasses().size() == 2u, "two texture passes must compile")) {
        return false;
    }
    const auto& writePass = graph.compiledPasses()[0];
    const auto& readPass = graph.compiledPasses()[1];
    return requireTrue(writePass.textureBarriers.size() == 1u,
                       "the first texture write must transition from Undefined") &&
           requireTrue(readPass.dependencies.size() == 1u && readPass.dependencies[0] == 0u,
                       "the texture reader must depend on its writer") &&
           requireTrue(readPass.textureBarriers.size() == 1u &&
                           readPass.textureBarriers[0].oldState == RhiResourceState::RenderTarget &&
                           readPass.textureBarriers[0].newState == RhiResourceState::ShaderRead &&
                           readPass.textureBarriers[0].sourceQueue == RhiQueueType::Graphics &&
                           readPass.textureBarriers[0].destinationQueue == RhiQueueType::Compute,
                       "the reader must receive a cross-queue state transition") &&
           requireTrue(graph.epilogueTextureBarriers().size() == 1u &&
                           graph.epilogueTextureBarriers()[0].oldState == RhiResourceState::ShaderRead &&
                           graph.epilogueTextureBarriers()[0].newState == RhiResourceState::ShaderRead &&
                           graph.epilogueTextureBarriers()[0].sourceQueue == RhiQueueType::Compute &&
                           graph.epilogueTextureBarriers()[0].destinationQueue == RhiQueueType::Graphics,
                       "the final queue contract must return the texture to graphics") &&
           requireTrue(graph.textureLifetimes()[0].used && graph.textureLifetimes()[0].firstPass == 0u &&
...
bool testComputeDepthReadPlanning() {
    ...
    const RgTextureHandle depth = graph.importTexture({"ComputeDepth",
                                                       {19u, 2u},
                                                       desc,
                                                       ...
                                                       RhiQueueType::Graphics});
    graph.addPass({"ComputeDepthRead", RgPassType::Compute, RhiQueueType::Compute})
    ...
    return requireTrue(pass.textureBarriers.size() == 1u &&
                           pass.textureBarriers[0].oldState == RhiResourceState::DepthRead &&
                           pass.textureBarriers[0].newState == RhiResourceState::DepthRead &&
                           pass.textureBarriers[0].sourceQueue == RhiQueueType::Graphics &&
                           pass.textureBarriers[0].destinationQueue == RhiQueueType::Compute,
                       "compute depth sampling must preserve the depth-read layout") &&
           requireTrue(graph.epilogueTextureBarriers().size() == 1u &&
                           graph.epilogueTextureBarriers()[0].oldState == RhiResourceState::DepthRead &&
                           graph.epilogueTextureBarriers()[0].newState == RhiResourceState::DepthRead,
...
bool testSubresourceIndependence() {
    ...
    const RgTextureHandle texture =
        graph.createTexture({"MipChain", colorTextureDesc(2u, 2u), RhiResourceState::Undefined});
    ...
    return requireTrue(result.succeeded(), result.message.c_str()) &&
           requireTrue(graph.compiledPasses()[1].dependencies.empty(),
...
bool testThreeDimensionalTextureSubresources() {
    ...
    const RgTextureHandle texture = graph.createTexture({"Volume", desc, RhiResourceState::ShaderRead});
    ...
    return requireTrue(graph.compiledPasses()[0].textureBarriers.size() == 3u,
           ...
           requireTrue(hasOnlyNativeSubresourceLayers(graph.compiledPasses()[0].textureBarriers) &&
...
bool testLayeredDepthCopyPlanning() {
    ...
    const RgTextureHandle depthOpaque = graph.importTexture({"ShadowDepthOpaque",
                                                             {31u, 1u},
                                                             depthDesc,
                                                             RhiResourceState::DepthRead,
                                                             RhiResourceState::DepthRead,
                                                             ...
                                                             RhiQueueType::Graphics,
                                                             RhiQueueType::Graphics});
    const RgTextureHandle depthAll = graph.importTexture({"ShadowDepthAll",
                                                          {32u, 1u},
                                                          depthDesc,
                                                          RhiResourceState::DepthRead,
                                                          RhiResourceState::DepthRead,
                                                          ...
                                                          RhiQueueType::Graphics,
                                                          RhiQueueType::Graphics});
    const RgTextureHandle color0 = graph.importTexture({"ShadowColor0",
                                                        {33u, 1u},
                                                        colorDesc,
                                                        ...
                                                        RhiQueueType::Graphics,
                                                        RhiQueueType::Graphics});
    const RgTextureHandle color1 = graph.importTexture({"ShadowColor1",
                                                        {34u, 1u},
                                                        colorDesc,
                                                        RhiResourceState::ShaderRead,
                                                        RhiResourceState::ShaderRead,
    ...
    const char* const transparentNames[] = {"Cascade0.Transparent", "Cascade1.Transparent"};
    RgPassHandle previous;
    for (uint32_t cascade = 0u; cascade < 2u; ++cascade) {
        const RgTextureSubresourceRange range{0u, 1u, cascade, 1u, 0u};
        RenderGraphPassBuilder opaque =
            graph.addPass({opaqueNames[cascade], RgPassType::Graphics, RhiQueueType::Graphics});
        if (previous.isValid())
            opaque.dependsOn(previous);
        opaque.writeTexture(depthOpaque, RhiResourceState::DepthWrite, range).setExecute(executeNoop);
        previous = opaque.handle();

        RenderGraphPassBuilder copy = graph.addPass({copyNames[cascade], RgPassType::Copy, RhiQueueType::Graphics});
        ...
        RenderGraphPassBuilder transparent =
            graph.addPass({transparentNames[cascade], RgPassType::Graphics, RhiQueueType::Graphics});
    ...
    const RgCompileResult result = graph.compile();
    if (!requireTrue(result.succeeded(), result.message.c_str()) ||
        !requireTrue(graph.compiledPasses().size() == 6u, "two shadow cascades must compile into six passes")) {
        return false;
    }
    ...
    return requireTrue(opaque.textureBarriers.size() == 1u &&
                           opaque.textureBarriers[0].oldState == RhiResourceState::DepthRead &&
                           opaque.textureBarriers[0].newState == RhiResourceState::DepthWrite,
                       "shadow opaque pass must transition its layer to depth write") &&
           requireTrue(copy.dependencies.size() == 1u && copy.textureBarriers.size() == 2u &&
                           copy.textureBarriers[0].oldState == RhiResourceState::DepthWrite &&
                           copy.textureBarriers[0].newState == RhiResourceState::TransferSrc &&
                           copy.textureBarriers[1].oldState == RhiResourceState::DepthRead &&
                           copy.textureBarriers[1].newState == RhiResourceState::TransferDst,
                       "shadow depth copy must transition both source and destination") &&
           requireTrue(transparent.dependencies.size() == 1u && transparent.textureBarriers.size() == 3u &&
                           transparent.textureBarriers[0].oldState == RhiResourceState::TransferDst &&
                           transparent.textureBarriers[0].newState == RhiResourceState::DepthWrite,
                       "transparent shadow pass must consume the copied depth layer") &&
           requireTrue(graph.compiledPasses()[3].textureBarriers[0].range.baseLayer == 1u,
...
bool testTransientReadBeforeWriteValidation() {
    RenderGraph graph;
    const RgTextureHandle texture =
        graph.createTexture({"Uninitialized", colorTextureDesc(), RhiResourceState::Undefined});
...
bool testImportedResourceRead() {
    RenderGraph graph;
    const RgTextureHandle texture = graph.importTexture({"History",
                                                         {7u, 3u},
                                                         colorTextureDesc(),
                                                         RhiResourceState::ShaderRead,
                                                         RhiResourceState::ShaderRead,
    ...
    const RgCompileResult result = graph.compile();
    return requireTrue(result.succeeded(), result.message.c_str()) &&
           requireTrue(graph.compiledPasses()[0].textureBarriers.empty(),
...
bool testBufferRangeCoverage() {
    ...
    const RgBufferHandle buffer = graph.createBuffer({"TransientBuffer", desc, RhiResourceState::StorageBuffer});
    ...
    const RgCompileResult result = graph.compile();
    return requireTrue(result.succeeded(), result.message.c_str()) &&
           requireTrue(graph.compiledPasses()[2].dependencies.size() == 2u,
                       "a buffer consumer must depend on every overlapping writer") &&
           requireTrue(graph.epilogueBufferBarriers().size() == 1u &&
                           graph.epilogueBufferBarriers()[0].oldState == RhiResourceState::StorageBuffer &&
                           graph.epilogueBufferBarriers()[0].newState == RhiResourceState::StorageBuffer &&
                           graph.epilogueBufferBarriers()[0].sourceQueue == RhiQueueType::Compute &&
                           graph.epilogueBufferBarriers()[0].destinationQueue == RhiQueueType::Graphics,
...
bool testExplicitCycleValidation() {
    ...
    const RgPassHandle a =
        graph.addPass({"A", RgPassType::Graphics, RhiQueueType::Graphics}).setExecute(executeNoop).handle();
    const RgPassHandle b = graph.addPass({"B", RgPassType::Graphics, RhiQueueType::Graphics})
                               .dependsOn(a)
                               .setExecute(executeNoop)
                               .handle();
    graph.editPass(a).dependsOn(b);
    const RgCompileResult result = graph.compile();
    return requireTrue(a.isValid() && b.isValid(), "pass handles must remain stable while building the graph") &&
...
bool testDuplicateAccessValidation() {
    RenderGraph graph;
    const RgTextureHandle texture = graph.importTexture({"Duplicate",
                                                         {4u, 2u},
                                                         colorTextureDesc(),
                                                         RhiResourceState::ShaderRead,
                                                         RhiResourceState::ShaderRead,
                                                         ...
                                                         RhiQueueType::Graphics,
                                                         RhiQueueType::Graphics});
    graph.addPass({"DuplicateAccess", RgPassType::Graphics, RhiQueueType::Graphics})
        .readTexture(texture)
        .writeTexture(texture, RhiResourceState::RenderTarget)
        .setExecute(executeNoop);
    const RgCompileResult result = graph.compile();
...
bool testResetInvalidatesHandles() {
    RenderGraph graph;
    const RgTextureHandle stale = graph.createTexture({"BeforeReset", colorTextureDesc(), RhiResourceState::Undefined});
    graph.reset();
    const RgTextureHandle current =
        graph.createTexture({"AfterReset", colorTextureDesc(), RhiResourceState::Undefined});
    graph.addPass({"StaleHandle", RgPassType::Graphics, RhiQueueType::Graphics})
        .writeTexture(stale, RhiResourceState::RenderTarget)
        .setExecute(executeNoop);
    const RgCompileResult result = graph.compile();
    return requireTrue(stale.index == current.index && stale.generation != current.generation,
...
bool testUndefinedImportedReadValidation() {
    RenderGraph graph;
    const RgTextureHandle texture = graph.importTexture({"UndefinedImport",
                                                         {11u, 2u},
                                                         colorTextureDesc(),
                                                         ...
                                                         RhiQueueType::Graphics,
                                                         RhiQueueType::Graphics});
    graph.addPass({"ReadUndefinedImport", RgPassType::Graphics, RhiQueueType::Graphics})
        .readTexture(texture)
        .setExecute(executeNoop);
    const RgCompileResult result = graph.compile();
    return requireTrue(result.error == RgCompileError::ReadBeforeWrite,
...
bool testCrossQueueReadOwnershipDependency() {
    RenderGraph graph;
    const RgTextureHandle texture = graph.importTexture({"SharedRead",
                                                         {17u, 4u},
                                                         colorTextureDesc(),
                                                         RhiResourceState::ShaderRead,
                                                         RhiResourceState::ShaderRead,
    ...
    graph.addPass({"GraphicsRead", RgPassType::Graphics, RhiQueueType::Graphics})
        .readTexture(texture)
        .setExecute(executeNoop);
    ...
    return requireTrue(destination.dependencies.size() == 1u && destination.dependencies[0] == 0u,
           ...
           requireTrue(destination.textureBarriers.size() == 1u && destination.textureBarriers[0].sourcePass == 0u,
...
bool testConcurrentCrossQueueReads() {
    ...
    const RgTextureHandle texture = graph.importTexture({"ConcurrentSharedRead",
                                                         {18u, 4u},
                                                         desc,
                                                         RhiResourceState::ShaderRead,
                                                         RhiResourceState::ShaderRead,
    ...
    const RgCompiledPass& source = graph.compiledPasses()[0];
    const RgCompiledPass& destination = graph.compiledPasses()[1];
    return requireTrue(destination.dependencies.empty(), "concurrent cross-queue reads must remain independent") &&
           requireTrue(source.releaseTextureBarriers.empty(), "concurrent reads must not release queue ownership") &&
           requireTrue(destination.textureBarriers.empty(), "concurrent reads in one state need no acquire barrier") &&
           requireTrue(graph.epilogueTextureBarriers().empty(), "concurrent reads need no queue-only epilogue");
}

bool testAsyncLightingCriticalPathIsolation() {
    RenderGraph graph;
    RhiTextureDesc sharedDesc = colorTextureDesc();
    sharedDesc.queueSharing = RhiTextureQueueSharing::GraphicsComputeConcurrent;
    const RgTextureHandle sharedInput = graph.createTexture({"SharedLightingInput", sharedDesc});
    const RgTextureHandle ssaoOutput = graph.createTexture({"SsaoOutput", colorTextureDesc()});
    const RgTextureHandle cloudOutput = graph.createTexture({"CloudOutput", colorTextureDesc()});

    RenderGraphPassBuilder gbuffer = graph.addPass({"GBuffer", RgPassType::Graphics, RhiQueueType::Graphics});
    gbuffer.writeTexture(sharedInput, RhiResourceState::RenderTarget).setExecute(executeNoop);

    RenderGraphPassBuilder ssao = graph.addPass({"SSAO", RgPassType::Compute, RhiQueueType::Compute});
    ...
    RenderGraphPassBuilder shadow = graph.addPass({"Shadow", RgPassType::Graphics, RhiQueueType::Graphics});
    shadow.dependsOn(gbuffer.handle()).setExecute(executeNoop);

    RenderGraphPassBuilder cloud = graph.addPass({"Cloud", RgPassType::Compute, RhiQueueType::Compute});
    cloud.dependsOn(gbuffer.handle())
        .readTexture(sharedInput, RhiResourceState::ShaderRead)
        .writeTexture(cloudOutput, RhiResourceState::ShaderWrite)
        .setExecute(executeNoop);

    graph.addPass({"Lighting", RgPassType::Graphics, RhiQueueType::Graphics})
    ...
    if (!requireTrue(batches.size() == 5u, "SSAO, shadow, cloud, and lighting must form distinct queue batches")) {
        return false;
    }
    const std::vector<uint32_t>& lightingDependencies = batches[4].dependencies;
    return requireTrue(std::find(lightingDependencies.begin(), lightingDependencies.end(), 1u) !=
                           lightingDependencies.end(),
                       "lighting must wait for the SSAO compute batch") &&
           requireTrue(std::find(lightingDependencies.begin(), lightingDependencies.end(), 2u) !=
                           lightingDependencies.end(),
                       "lighting must retain its graphics shadow dependency") &&
           requireTrue(std::find(lightingDependencies.begin(), lightingDependencies.end(), 3u) ==
                           lightingDependencies.end(),
...
bool testQueueOnlyEpiloguePlanning() {
    ...
    const RgBufferHandle buffer = graph.createBuffer({"QueueOnlyEpilogue", desc, RhiResourceState::TransferDst});
    graph.addPass({"TransferWrite", RgPassType::Copy, RhiQueueType::Transfer})
    ...
    return requireTrue(graph.epilogueBufferBarriers().size() == 1u,
           ...
           requireTrue(graph.epilogueBufferBarriers()[0].oldState == RhiResourceState::TransferDst &&
                           graph.epilogueBufferBarriers()[0].newState == RhiResourceState::TransferDst &&
                           graph.epilogueBufferBarriers()[0].sourceQueue == RhiQueueType::Transfer &&
                           graph.epilogueBufferBarriers()[0].destinationQueue == RhiQueueType::Graphics &&
                           graph.epilogueBufferBarriers()[0].sourcePass == 0u,
                       "queue-only epilogue must retain state and source pass") &&
           requireTrue(graph.compiledPasses()[0].releaseBufferBarriers.size() == 1u,
...
bool testBatchSplitForLateQueueDependency() {
    ...
    desc.size = 256u;
    desc.usage = rhiFlag(RhiBufferUsage::Storage) | rhiFlag(RhiBufferUsage::TransferDst);
    const RgBufferHandle computeOutput = graph.createBuffer({"ComputeOutput", desc, RhiResourceState::Undefined});
    const RgBufferHandle graphicsOutput = graph.createBuffer({"GraphicsOutput", desc, RhiResourceState::Undefined});
    graph.addPass({"ComputeProducer", RgPassType::Compute, RhiQueueType::Compute})
        .writeBuffer(computeOutput, RhiResourceState::StorageBuffer)
        .setExecute(executeNoop);
    graph.addPass({"IndependentGraphics", RgPassType::Copy, RhiQueueType::Graphics})
        .writeBuffer(graphicsOutput, RhiResourceState::TransferDst)
        .setExecute(executeNoop);
    graph.addPass({"DependentGraphics", RgPassType::Compute, RhiQueueType::Graphics})
    ...
    if (!requireTrue(result.succeeded(), result.message.c_str()))
        return false;
    return requireTrue(graph.submissionBatches().size() == 3u, "a late cross-queue wait must start a new batch") &&
...
bool testSubmissionBatchPassLimit() {
    ...
    for (uint32_t index = 0u; index < passCount; ++index) {
        RenderGraphPassBuilder pass = graph.addPass({"BatchLimitPass", RgPassType::Copy, RhiQueueType::Graphics});
        if (previous.isValid()) {
    ...
    const uint32_t expectedBatchCount =
        (passCount + kRgMaxPassesPerSubmissionBatch - 1u) / kRgMaxPassesPerSubmissionBatch;
    ...
    for (uint32_t batchIndex = 0u; batchIndex < batches.size(); ++batchIndex) {
        const uint32_t expectedPasses =
            std::min(kRgMaxPassesPerSubmissionBatch, passCount - batchIndex * kRgMaxPassesPerSubmissionBatch);
        if (!requireTrue(batches[batchIndex].passes.size() == expectedPasses,
                         "submission batches must respect the pass limit") ||
            !requireTrue(batchIndex == 0u || (batches[batchIndex].dependencies.size() == 1u &&
...
int main() {
    const bool passed =
        testTextureDependencyAndBarrierPlanning() && testComputeDepthReadPlanning() && testSubresourceIndependence() &&
        testThreeDimensionalTextureSubresources() && testLayeredDepthCopyPlanning() &&
        testTransientReadBeforeWriteValidation() && testImportedResourceRead() && testBufferRangeCoverage() &&
        testExplicitCycleValidation() && testDuplicateAccessValidation() && testResetInvalidatesHandles() &&
        testUndefinedImportedReadValidation() && testCrossQueueReadOwnershipDependency() &&
        testConcurrentCrossQueueReads() && testAsyncLightingCriticalPathIsolation() &&
        testQueueOnlyEpiloguePlanning() && testBatchSplitForLateQueueDependency() && testSubmissionBatchPassLimit();
