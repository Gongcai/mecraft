...
layout(binding = 0) uniform samplerCube uSkyRadiance;
...
void main() {
    ...
    for (uint index = 0u; index < uSampleCount; ++index) {
        ...
        vec3 halfway = skyIblImportanceSampleGgx(samplePoint, normal, alpha);
        vec3 lightDirection = normalize(2.0 * dot(viewDirection, halfway) *
                                        halfway - viewDirection);
        float nDotL = max(dot(normal, lightDirection), 0.0);
        if (nDotL > 0.0) {
            radiance += textureLod(uSkyRadiance, lightDirection, 0.0).rgb * nDotL;
            weight += nDotL;
        }
    }
    fragColor = vec4(max(radiance / max(weight, 1e-5), vec3(0.0)), 1.0);
}
