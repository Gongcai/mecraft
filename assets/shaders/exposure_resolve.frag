...
void main() {
    ...
    float averageLum = exp(averageLogLum * 0.75);

    float K = 19.0;
    float calibration = exp2(uAutoExposureBias) * K * 1e-2;
    float a = K * 1e-2 * 18.0;
    float b = a - K * 1e-2 * 0.04;
    float targetExposure = calibration / (a - b * exp(-averageLum / b));

    float previousExposure = texelFetch(uPreviousExposureTex, ivec2(0, 0), 0).r;
    float resolvedPrevious = uInitialized ? previousExposure : uManualExposure;
