#ifndef CLOUD_TARGET_COMMON_GLSL
#define CLOUD_TARGET_COMMON_GLSL
...
// Shared cloud shading body used by both the fragment and compute entry
// points. The parameter keeps the historical varying name so the body needs
// no further edits.
vec4 renderCloudTarget(vec2 vScreenUv) {
    // The fullscreen vertex shader derives the screen UV from the clip UV via
    // an involutive flip, so the inverse mapping reuses the same helper.
...
#endif // CLOUD_TARGET_COMMON_GLSL
