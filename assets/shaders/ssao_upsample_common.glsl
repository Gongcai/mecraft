#ifndef SSAO_UPSAMPLE_COMMON_GLSL
#define SSAO_UPSAMPLE_COMMON_GLSL
...
// Shared ssao_upsample shading body used by both the fragment and compute
// entry points; fragTexel carries the integer pixel coordinate the
// calling stage derived from its native fragment position.
vec4 renderSsaoUpsample(vec2 vScreenUv, ivec2 fragTexel) {
    ivec2 fullTexel = ivec2((vec2(fragTexel) + 0.5));
    ...
    return vec4(ao, 0.0, 0.0, 1.0);
}

#endif // SSAO_UPSAMPLE_COMMON_GLSL
