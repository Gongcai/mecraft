...
void main() {
    ...
    vec2 halfCoord = vTexCoord * uHalfResSize - 0.5;
    ...
    float ao00 = texelFetch(uSsaoHalfResTex, clamp(halfBase + ivec2(0, 0), ivec2(0), ivec2(uHalfResSize) - 1), 0).r;
    float ao10 = texelFetch(uSsaoHalfResTex, clamp(halfBase + ivec2(1, 0), ivec2(0), ivec2(uHalfResSize) - 1), 0).r;
    float ao01 = texelFetch(uSsaoHalfResTex, clamp(halfBase + ivec2(0, 1), ivec2(0), ivec2(uHalfResSize) - 1), 0).r;
    float ao11 = texelFetch(uSsaoHalfResTex, clamp(halfBase + ivec2(1, 1), ivec2(0), ivec2(uHalfResSize) - 1), 0).r;

    // Depth at each half-res sample position (using full-res depth texture)
    vec2 uv00 = (vec2(halfBase + ivec2(0, 0)) + 0.5) * halfTexelSize;
    vec2 uv10 = (vec2(halfBase + ivec2(1, 0)) + 0.5) * halfTexelSize;
    vec2 uv01 = (vec2(halfBase + ivec2(0, 1)) + 0.5) * halfTexelSize;
    vec2 uv11 = (vec2(halfBase + ivec2(1, 1)) + 0.5) * halfTexelSize;

    float d00 = 2.0 * uNear / max(1.0 - texture(uDepthTex, uv00).r, 1e-7);
    float d10 = 2.0 * uNear / max(1.0 - texture(uDepthTex, uv10).r, 1e-7);
    float d01 = 2.0 * uNear / max(1.0 - texture(uDepthTex, uv01).r, 1e-7);
    float d11 = 2.0 * uNear / max(1.0 - texture(uDepthTex, uv11).r, 1e-7);
