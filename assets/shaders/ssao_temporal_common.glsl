#ifndef SSAO_TEMPORAL_COMMON_GLSL
#define SSAO_TEMPORAL_COMMON_GLSL
...
// Shared ssao_temporal shading body used by both the fragment and compute
// entry points; fragTexel carries the integer pixel coordinate the
// calling stage derived from its native fragment position.
vec4 renderSsaoTemporal(vec2 vScreenUv, ivec2 fragTexel) {
    ivec2 texel = ivec2((vec2(fragTexel) + 0.5));
    ...
    if (prevCoord.x < 0.0 || prevCoord.x > 1.0 ||
        ...
        return vec4(currentAo, 0.0, 0.0, 1.0);
    }
...
#endif // SSAO_TEMPORAL_COMMON_GLSL
