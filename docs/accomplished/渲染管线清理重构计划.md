...
1. 第一版目标是 **Mecraft-native shaderpack**：光影包基于 Mecraft 明确发布的 contracts、manifest 和 pass graph schema 编写，不承诺完整兼容 Iris/OptiFine 现有包。
2. `RenderTargets::PassIO` 当前只是内置 Deferred pipeline 的文档化表格，后续必须升级为正式 ABI：shaderpack、内置 pipeline、编辑器和诊断工具都只能依赖这个 ABI，而不能依赖旧 `Renderer` 字段或某个硬编码 pass 顺序。
3. “可替换光影包”的最低标准不是替换若干 `.fs/.vs` 文件，而是外部包可以声明自己的 targets、pass graph、shader compile requests、options 和 capabilities，并由运行时原子切换。
4. 编译或校验失败时，当前已运行 pipeline 必须保持可用；失败信息进入 `PipelineDiagnostics` / Dashboard / editor，而不是让 Game 或 RenderScene 进入半初始化状态。
...
- 明确 render target ABI：逻辑名、格式、分辨率比例、采样方式、clear policy、history/ping-pong 语义、生命周期和默认 fallback。
- 明确 pass ABI：pass 类型（terrain/entity/drop/sky/shadow/fullscreen/composite）、draw source、framebuffer 输出、depth test/write、blend、cull、viewport/scissor、debug label。
- 明确 shader ABI：稳定 include 根目录、预定义宏、sampler binding、UBO/SSBO binding、push-style uniform semantic、材质/GBuffer schema 版本。
- 明确 capability ABI：是否需要 GBuffer、velocity、history、shadow、weather mask、sky capture、volumetric、transparent composite、postprocess handoff。

验收：

- 内置 Forward/Deferred pipeline 都能声明自己使用的 contract。
- contract 校验能发现 pass 读取未写入 target、格式不匹配、sampler 冲突等错误。
- contract 校验能发现 pass 状态缺失、history 资源未声明、depth/color attachment 不兼容、capability 与 graph 不一致等错误。
- shaderpack 只包含 contract 声明时可以 dry-run 校验，不需要创建 GL 资源。
...
- 诊断信息：manifest/schema/contract/shader include/options 的错误、警告和来源位置。

建议拆分：

- Phase 13a：pack discovery、manifest schema、版本检查、pack options。
- Phase 13b：`PipelineAsset` 数据模型、target/pass/resource/capability 声明、contract dry-run 校验。
- Phase 13c：shader include/define/directive 解析，生成 shader compile requests，但不接入运行时。
- Phase 13d：内置 DerivativeMain-like pipeline 迁移为同一 asset 模型的 built-in package。
...
- 明确声明第一版 shaderpack 格式为 Mecraft-native，不把 Iris/OptiFine 完整兼容列入本阶段验收。
...
- `PipelineResourceAllocator`
- `PipelineShaderCache`
- `RuntimePass` / `ExecutablePass`
- `PipelineSwapTransaction`
- `PipelineDiagnostics`
...
- `PipelineResourceAllocator` 根据 compiled graph 分配 FBO、texture、history、ping-pong 和 transient targets，负责 resize、shadow resolution、target layout 变化后的重建。
- `PipelineShaderCache` 负责编译、热重载、include dependency tracking、旧 program 保留和错误诊断。
- `RuntimePass` / `ExecutablePass` 是 graph 执行节点，封装 GL 状态、resource binding、shader program、draw source 和 debug marker，避免 `GraphRenderPipeline` 重新膨胀成巨型 Renderer。
- `PipelineSwapTransaction` 必须保证原子切换：新 graph 的 contract 校验、资源分配和 shader 编译全部成功后，才替换当前运行 pipeline。
...
- 能加载一个外部 Mecraft-native shaderpack：声明 targets、pass graph、shaders、options 和 capabilities；编译成功后运行时切换生效。
- shaderpack 编译或 contract 校验失败时，上一条可用 pipeline 继续渲染，错误信息可定位到 pack 文件、pass、target 或 shader。
...
## 最终架构验收：No Legacy Compatibility Layer

> 这是渲染管线重构的最终硬性门槛。兼容桥只允许服务迁移阶段，不能成为新架构的永久组成部分。

### 必须成立

- `Game` 不 include、不持有、不调用 legacy `Renderer`。
- `RenderScene` 不再有 `setLegacyRenderer()` / `syncFrameOutputFromLegacyRenderer()`。
- `Renderer::RenderPipelineSettings` 删除，只保留 `RenderSettings`。
- `Renderer::RenderFrameData` 删除，只保留 `FrameContext`。
- `Renderer::renderWorldDeferred()`、`renderWorldForward()`、`renderOpaqueAndCutout()`、`renderTransparentAndOverlays()` 删除。
- `FrameOutput` 由当前 active `RenderPipeline` / `GraphRenderPipeline` 直接生成，不从旧 renderer 同步。
- Dashboard/UI/held item/postprocess/debug overlay 只通过 `RenderScene`、`RenderPipelineService`、`RenderSettings`、`FrameOutput`、debug facade 访问渲染状态。
- 光影包和 pipeline editor 只依赖 contracts、`PipelineAsset`、`PipelineGraphCompiler`、`RenderPipelineService`，不依赖旧 `Renderer` 私有状态。
- `Renderer` 这个类要么删除，要么改名/收敛为 `RenderResourceHub`，且不包含任何管线编排、legacy settings 或兼容查询。

### 源码搜索验收

以下命令在 `src/` 中不应命中，除非命中的是明确标记为 one-shot 旧配置迁移器的代码：
...
- 历史文档中的旧名称。
- 面向旧配置文件的单次迁移代码。
- 纯资源所有权对象，例如 `RenderResourceHub`，但它不能暴露旧管线 API。

### 不允许存在

- 为新系统继续同步旧 renderer 状态的桥接代码。
- 通过 legacy settings 驱动新 pipeline 的适配层。
- shaderpack / pipeline editor 依赖旧 `Renderer` 字段或旧 pass 顺序。
- `Game` 或 UI 为了渲染状态查询绕过 `RenderScene` / `RenderPipelineService`。
