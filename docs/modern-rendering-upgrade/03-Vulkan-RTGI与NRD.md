...
定位并修复该跨边界输运。
V02 Cave Turn 已完成同口径 300+32/64 Vulkan 静态运行：方差降低 `99.985878%`，AS Pending/Invalid 为 `0`；
但 SSIM `0.788408`、HDR p95 `0.507056` 和 Leakage 最大带宽 `3 Pixels` 均失败，ROI 边界像素为 `10092`。
这把下一轮根因定位从 V01 局部问题提升为洞穴边界与一次反弹局部光输运的共同问题。
