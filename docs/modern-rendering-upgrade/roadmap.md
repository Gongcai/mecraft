...
Profile/ROI/64 spp 目标。V01 已以 1 帧预热完成真实 32+32 EXR 链路验证；首轮画面复核发现原 v1 ROI 覆盖中心
...
推进 RTGI 低差异相位。Reference 采用每像素独立旋转、XOR 扰动的周期 64 点 Hammersley 集合，以 6-bit
索引奇偶校验划分前后两个 32 点子集，使两半在两个维度的全部二进制分层区间中保持均衡；任意连续 64 个
实际渲染帧枚举同一完整集合，不再让 300 帧预热把
采集区间切到两个不同批次。帧首重置条件只在既没有
...
质量报告 schema v3 已增加不改变 Gate 的误差分布诊断。固定 64 spp 的采样 A/B 中，8×8 分层序列使最终
HDR p95 恶化到 `0.782297`；scrambled Hammersley 得到约 `0.47` 的 HDR p95，因此替换旧 R2。随后发现旧的
偶/奇索引划分让每个 32 点半序列的 Radical Inverse 维只覆盖旋转后的半域，与契约注释矛盾；当前改为 6-bit
奇偶校验子集，并以 CPU 最大环形间距测试锁定两半在两个维度都覆盖完整域。该重排不改变完整 64 点集合。
RTX 4060 Laptop 的最新 300+64 实跑得到 SSIM `0.997339`、HDR p95 `0.474772`、Reference 均值
`0.002609265`；相对误差 p50 `0.165041`、绝对误差 p95 `0.001317`、分母下限像素 `0%`。Reference 前后半
相对误差 p50/p95 为 `0.289572/1.294182`，绝对误差 p95 `0.003587`。两半的采样域覆盖已修正但图像差异未
...
Caustica 对照评估和第一轮策略契约已经完成：

- 新增 `DynamicBlasPolicyContract`，决策固定为 `RigidReuse`、`Update`、`Rebuild` 三类。Rigid Reuse 锁定
  Geometry/Topology、姿态关系和 Shading Hash；UPDATE 锁定 Build Flags、Geometry 顺序与类别、Vertex/Index
  格式、Vertex/Primitive/Index 数和 Index Topology，并在连续 120 次 UPDATE 后要求完整重建。当前没有动态实体
  BLAS 生产者，报告中的 Action 与 Reject Counter 准确为零。
- 当前 Scene TLAS 仍使用 `snapshot_generations`：场景快照变化时分配一代完整资源，报告独立统计 Allocation、
  Reuse、Reuse Wait 和 Retired Generation。最新 3 帧窗口中体素分配 0 代，Sponza 分配 1 代，两者的 Reuse
  与 Reuse Wait 都是 0。动态 Transform 尚未形成每帧 TLAS 压力，因此不引入固定四槽 Ring。
