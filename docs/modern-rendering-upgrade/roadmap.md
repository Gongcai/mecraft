...
质量报告 schema v3 已增加不改变 Gate 的误差分布诊断。Anti-firefly 关闭组的相对误差 p50 为 `0.172629`、
绝对误差 p95 为 `0.001329`，使用 `1e-4` 分母下限的像素为 `0%`；因此失败不是近零像素放大。Reference
前后半相对误差 p50 为 `0.436918`、绝对误差 p95 为 `0.004012`，锚点像素两半亮度分别为
`0.007697/0.002500`，证明当前周期 R2 的 64 spp 对 V01 可见性积分未收敛。
...
1. 在 64 spp 规格不变的前提下，对当前周期 R2 与固定样本数分层低差异序列做 Reference 实采 A/B；再对
