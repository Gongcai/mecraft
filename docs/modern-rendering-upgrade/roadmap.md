...
推进 RTGI 低差异相位。Reference 采用每像素独立旋转、XOR 扰动的周期 64 点 Hammersley 集合，偶/奇交错
让前后两个 32 帧半序列都覆盖完整二维域；任意连续 64 个实际渲染帧枚举同一完整集合，不再让 300 帧预热把
...
HDR p95 恶化到 `0.782297`；偶/奇交错 scrambled Hammersley 得到最优的 SSIM `0.997353`、HDR p95
